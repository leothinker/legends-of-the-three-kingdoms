...
export default {
  ...
  card: {
    ...
    huogong: {
      ...
      async content(event, trigger, player) {
        const target = event.target
        const showPosition =
          event.showPosition === undefined ? "h" : event.showPosition
        ...
        event.chooseToShow ??= async (event, player, target) => {
          const showPosition =
            event.showPosition === undefined ? "h" : event.showPosition
          const filterShow =
            event.filterShow === undefined ? () => true : event.filterShow
          let result
        ...
        event.chooseToDiscard ??= async (event, player, target) => {
          const discardPostion =
            event.discardPostion === undefined ? "h" : event.discardPostion
          const cards2 = event.cards2
          const filterDiscard =
            event.filterDiscard === undefined
              ? { suit: get.suit(cards2[0]) }
              : event.filterDiscard
  ...
  translate: {
    ...
    leisha: "雷杀",
    leisha_info:
      "出牌阶段限一次，对你攻击范围内的一名其他角色使用。你对其造成1点雷电伤害。",
    huosha: "火杀",
    huosha_info:
      "出牌阶段限一次，对你攻击范围内的一名其他角色使用。你对其造成1点火焰伤害。",
