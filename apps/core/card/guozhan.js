...
game.import("card", () => ({
  ...
  card: {
            ...
            const { control } = await current
              ...
              .set("ai", () => {
                ...
                const num = Math.max(
                  0,
                  choices.includes("弃牌")
                    ? get.effect(target, guohe, player)
                    : 0,
                  choices.includes("得牌")
                    ? get.effect(target, shunshou, player)
                    : 0,
                )
                if (
                  choices.includes("出杀") &&
                  get.effect(player, guohe, player) +
                    get.effect(target, sha, player) >
                    num
                ) {
                  return "出杀"
                }
    ...
    gz_kefuzhongyuan: {
      ...
      ai: {
        wuxie(_target, _card, player, viewer) {
          if (get.mode() === "guozhan") {
            if (!_status._aozhan) {
              if (!player.isMajor()) {
                if (!viewer.isMajor()) {
    ...
    gz_guguoanbang: {
      ...
      async content(event, _trigger, _player) {
        ...
        const targets = game.filterPlayer(
          (current) => current !== target && current[judge] === "wu",
        )
        const result = await target
          .chooseToDiscard(
            "请弃置至少六张手牌",
            [6, target.countCards("h")],
            true,
            "h",
            "allowChooseAll",
          )
          .set("ai", (card) => {
            ...
            if (
              ...
              targetx.filter((current) => get.attitude(player, current) > 0)
                .length
            ) {
              return 0
            }
        ...
        while (targets.length && give_cards.length) {
          ...
          result = await target
            ...
            .set("ai", (button) => {
              ...
              const { link } = button
              if (
                targetx.some((current) => get.attitude(player, current) > 0)
              ) {
          ...
          if (result?.bool && result?.links?.length) {
            ...
            result = await target
              .chooseTarget(
                true,
                `选择获得${get.translation(links)}的角色`,
              ...
              .set("ai", (target) => {
                ...
                return Math.max(
                  0.1,
                  att / Math.sqrt(1 + target.countCards("h")),
                )
              })
        ...
        result: {
          target(_player, target) {
            ...
            return Math.max(
              ...
              Math.min(
                8,
                2 * game.countPlayer((current) => current[judge] === "wu"),
              ),
            )
          },
        },
    ...
    gz_wenheluanwu: {
      ...
      filterTarget: true,
      selectTarget: -1,
      ignoreTarget(_card, _player, target) {
      ...
      async content(event, _trigger, player) {
        ...
        if (result.index === 0) {
            ...
            result2 = await target
              .chooseToDiscard(
                ...
                (card, _player) => {
                  ...
                  return (
                    get.type2(card, target) !==
                    get.type2(ui.selected.cards[0], target)
                  )
                },
              )
          ...
          result2 = await player
            .discardPlayerCard(target, "h", true, "visible")
            .forResult()
        }
        if (
          target[judge] === "qun" &&
      ...
      ai: {
        equipValue(_card, player) {
          ...
          if (
            game.hasPlayer(
              (current) =>
                current.hasSkill("jubao") && get.attitude(player, current) <= 0,
            )
          ) {
            return 0
          }
    ...
    shuiyanqijunx: {
        ...
        if (event.card.yingbian_all) {
          target.discard(
            target.getCards("e", (card) =>
              lib.filter.cardDiscardable(card, target, "shuiyanqijunx"),
            ),
          )
          ...
          target
            .chooseControl("discard_card", "take_damage", (event, player) => {
              ...
              if (
                ...
                !player.hasCard(
                  (card) =>
                    get.value(card, player) > Math.max(7, 12 - player.hp),
                  "e",
                )
              ) {
                return "discard_card"
            ...
            .set(
              "prompt2",
              `请选择一项：⒈弃置装备区里的所有牌；⒉受到${get.translation(player)}造成的1点雷电伤害。`,
            )
        }
        ;("step 1")
      ...
      ai: {
        canLink(player, target, _card) {
          ...
          if (
            0.15 * val >
            2 * get.sgn(get.damageEffect(target, player, target, "thunder"))
          ) {
            return false
          }
          return true
        ...
        result: {
          target(player, target, _card, isLink) {
    ...
    lulitongxin: {
      ...
      ai: {
        ...
        value: 4,
        useful: 2,
        wuxie: (target, _card, player, viewer, _status) => {
          if (
    ...
    lianjunshengyan: {
      ...
      content() {
        ...
        if (target !== player) {
          ...
          var draw = parseInt(result.control.slice(1, index), 10)
          var recover = parseInt(result.control.slice(index + 1), 10)
          if (draw) {
            target.draw(draw)
          }
          if (recover) {
    ...
    chiling: {
      ...
      content() {
        ...
        if (index === 0) {
          target
            .chooseControl("主将", "副将", () => Math.floor(Math.random() * 2))
      ...
      destroy(_card, targetPosition, _player, event) {
        if (
    ...
    diaohulishan: {
      ...
      ai: {
        order(_item, player) {
          ...
          if (player.hasCard((card) => get.name(card) === "taoyuan", "hs")) {
        ...
        result: {
          player(player, target) {
            ...
            if (
              game.hasPlayer((current) => get.attitude(player, current) < att)
            ) {
              var num = 1
        ...
        if (
          ...
          return !game.hasPlayer((current) => {
            if (
              ...
              if (
                dist === distance &&
                parseInt(current.dataset.position, 10) <
        ...
        result: {
          target(player, target, card) {
            ...
            return game.countPlayer((current) => {
              if (current.isLinked()) {
                return get.sgn(
                  get.damageEffect(current, player, target, "fire"),
                )
              }
            })
          },
    ...
    zhibi: {
      ...
      content() {
        ...
        if (controls.length > 1) {
          player.chooseControl(controls).set("ai", () => 1)
        }
      ...
      ai: {
        ...
        result: {
          player(player, target) {
            ...
            if (player.storage.zhibi?.includes(target)) {
              return 0
            }
    ...
    wuliu: {
      ...
      ai: {
        equipValue(_card, player) {
          ...
          return (
            2 + game.countPlayer((current) => current.isFriendOf(player)) / 2
          )
        },
        basic: {
          equipValue: 3,
  ...
  skill: {
    zhaoshu_skill: {
      ...
      onremove(player, skill) {
        var cards = player
          .getExpansions(skill)
          .concat(player.getExpansions("zhaoshu_cards"))
        if (cards.length) {
          player.loseToDiscardpile(cards)
        }
      },
    ...
    zhaoshu_global: {
      ...
      filter(_event, player) {
        ...
        return game.hasPlayer(
          (current) =>
            current.hasSkill("zhaoshu_skill") && current.isFriendOf(player),
        )
      },
      filterCard: true,
      selectCard() {
        ...
        var player = _status.event.player,
          cards = ui.selected.cards.concat(
            game
              .findPlayer(
                (current) =>
                  current.hasSkill("zhaoshu_skill") &&
      ...
      selectTarget() {
        if (
          game.countPlayer(
            (current) =>
              current.hasSkill("zhaoshu_skill") &&
              current.isFriendOf(_status.event.player),
          ) === 1
        ) {
          return -1
        }
    ...
    g_taipingyaoshu_ai: {
      ai: {
        effect: {
          ...
          target_use(card, _player, target) {
    ...
    feilongduofeng2: {
      ...
      filter(event, player) {
        if (event.reason?.card && event.reason.card.name === "sha") {
          return (
            event.player.isDead() &&
            lib.group.includes(player.identity) &&
            player.isMinor()
          )
        }
        return false
      ...
      mod: {
        maxHandcard(player, num) {
          if (get.mode() === "guozhan") {
            ...
            return (
              num + game.countPlayer((current) => current.isFriendOf(player))
            )
          }
    ...
    yuxi_skill2: {
      ...
      async content(_event, _trigger, player) {
        await player.chooseUseTarget("玉玺：选择知己知彼的目标", {
          name: "zhibi",
          isCard: true,
        })
      },
    ...
    g_chiling3: {
      ...
      content() {
        ...
        var targets = game.filterPlayer((target) => target.isUnseen())
      ...
      filter(event, player) {
        ...
        return game.hasPlayer(
          (current) =>
            ...
            get.distance(event.player, current) <= 1,
        )
      },
      content() {
        "step 0"
    ...
    jilinqianyi_skill: {
      ...
      filter(event, player) {
        ...
        return game.hasPlayer(
          (current) =>
            player.inRange(current) && !event.targets?.includes(current),
        )
      },
      ...
      mod: {
        attackRange(player, distance) {
          ...
          if (
            player.hasSkill("jilinqianyi_skill", null, false) ||
            cards.some((card) => !ui.selected.cards.includes(card))
          ) {
      ...
      subSkill: {
        global: {
          mod: {
            cardEnabled(_card, player) {
              let evt = _status.event?.getParent("useCard", true, true)
              while (evt) {
                if (
                  evt?.card?.name === "sha" &&
                  evt.player?.getEquips("jilinqianyi").length
                ) {
                  break
                }
              ...
              if (
                ...
                evt.player.inRange(player)
              ) {
                return false
              }
            },
            cardSavable(_card, player) {
              let evt = _status.event?.getParent("useCard", true, true)
              while (evt) {
                if (
                  evt?.card?.name === "sha" &&
  ...
  translate: {
    ...
    jilinqianyi: "戢鳞潜翼",
    jilinqianyi_info:
      "锁定技。①此武器的攻击范围为你已损失的体力值。②你使用【杀】的结算过程中，你攻击范围内的非目标角色不能使用牌。",
    jilinqianyi_skill: "戢鳞潜翼",
    jilinqianyi_skill_info:
      "你使用【杀】的结算过程中，你攻击范围内的非目标角色不能使用牌。",
    yuxi_skill: "玉玺",
    yuxi_skill2: "玉玺",
    yuxi: "玉玺",
    yuxi_info:
    ...
    diaohulishan: "调虎离山",
    diaohulishan_info:
      "出牌阶段，对至多两名其他角色使用。目标角色于此回合视为移出游戏。",
    ...
    yuanjiao: "远交近攻",
    yuanjiao_info:
      "出牌阶段，对一名与你势力不同的其他角色使用。其摸一张牌，然后你摸三张牌。",
    ...
    yiyi: "以逸待劳",
    yiyi_info_guozhan:
      "出牌阶段，对所有己方角色使用。目标角色摸两张牌，然后弃置两张牌。",
    yiyi_info_combat:
      "出牌阶段，对所有己方角色使用。目标角色摸两张牌，然后弃置两张牌。",
    yiyi_info:
      "出牌阶段，对至多三名角色使用。目标角色摸两张牌，然后弃置两张牌。",
    yiyi_bg: "逸",
    wuliu: "吴六剑",
