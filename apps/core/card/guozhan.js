            ...
            if (choices.length > 1) {
              const { control } = await current
                ...
                .set("ai", () => {
                  ...
                  if (
                    choices.includes("出杀") &&
                    get.effect(player, guohe, player) + get.effect(target, sha, player) > num
                  ) {
                    return "出杀"
                  }
              ...
              if (control != "cancel2") {
                if (control == "出杀") {
                  ...
                  await current[current[judge] == "wei" ? "gainPlayerCard" : "discardPlayerCard"](
                    target,
                    true,
                    "he",
                  ).set("boolline", true)
          ...
          const result = await target
            ...
            .set("ai", (card) => {
              ...
              if (
                ...
                targetx.filter((current) => get.attitude(player, current) > 0).length
              ) {
                return 0
              }
            ...
            result = await target
              ...
              .set("ai", (button) => {
                ...
                if (
                  ...
                  get.name(link) == "du" &&
                  targetx.some(
                    (current) => get.attitude(player, current) < 0 && !current.hasSkillTag("nodu"),
                  )
                ) {
                  return 1
                }
        ...
        ai: {
          ...
          result: {
            target(player, target) {
              ...
              return Math.max(
                3,
                Math.min(8, 2 * game.countPlayer((current) => current[judge] == "wu")),
              )
            },
          },
        },
      ...
      jilinqianyi_skill: {
        ...
        filter(event, player) {
          ...
          return game.hasPlayer(
            (current) => player.inRange(current) && !event.targets?.includes(current),
          )
        },
