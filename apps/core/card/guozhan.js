...
export default {
  ...
  card: {
    ...
    gz_haolingtianxia: {
      ...
      async content(event, trigger, player) {
        ...
        for (const current of targets) {
          ...
          if (!current?.isIn() || current.hasSkill("diaohulishan")) {
            continue
          }
          const choiceList = [
          ...
          if (choices.length > 1) {
            const { control } = await current
              ...
              .set("ai", () => {
                ...
                const shunshou = new lib.element.VCard({
                  name: "shunshou_copy2",
                })
    ...
    shuiyanqijunx: {
      ...
      async content(event, trigger, player) {
        const target = event.target
        const discardableEquips = target.getCards("e", (card) =>
    ...
    lianjunshengyan: {
      ...
      async content(event, trigger, player) {
        ...
        if (typeof result.control === "string") {
          ...
          const draw = parseInt(result.control.slice(1, index), 10)
          const recover = parseInt(result.control.slice(index + 1), 10)
          if (draw) {
        ...
        result: {
          player(player, target) {
            ...
            return player.countCards("hs", (card) => {
              ...
              const eff1 = player.getUseValue(cardx, null, true)
  ...
  skill: {
    zhaoshu_skill: {
      ...
      intro: {
        ...
        mark(dialog, content, player) {
          ...
          dialog.addText(
            "<ul><li>与你势力相同的角色的出牌阶段限一次，其可以将一张手牌（小势力角色改为至多两张）置于【诏书】上，称为“应”。</li><li>出牌阶段限一次，若你的“应”中包含至少四种花色，则你可以发动“锦囊召唤”，将所有“应”置入弃牌堆，然后随机获得一张未加入游戏的势力锦囊牌。</li></ul>",
            false,
          )
          const cards = player.getExpansions("zhaoshu_cards")
          if (cards.length) {
    ...
    zhaoshu_global: {
      ...
      check(card) {
        ...
        const cards = ui.selected.cards.concat(
          game
            .findPlayer(
              (current) =>
                current.hasSkill("zhaoshu_skill") && current.isFriendOf(player),
