  ...
  card: {
      ...
      content() {
        ...
        if (e1.length && e2.length) {
          ...
          target
            ...
            .set("choiceList", [
              `弃置${get.translation(e1)}`,
              `弃置${get.translation(e2)}`,
            ])
        } else {
          if (e1.length) {
            target.discard(e1)
    ...
    shengdong: {
      ...
      content() {
        ...
        if (!player.countCards("h")) {
          ...
          player.chooseCard(
            "h",
            `将一张手牌交给${get.translation(event.target1)}`,
            true,
          )
        }
        ;("step 1")
        ...
        ;("step 2")
        if (!event.target1.countCards("h") || !event.target2?.isIn()) {
          event.finish()
        } else {
    ...
    zengbin: {
      ...
      content() {
        ...
        ;("step 1")
        if (
          target.countCards("he", { type: "basic" }) < target.countCards("he")
        ) {
          target
            .chooseToDiscard(
              "增兵减灶",
            ...
            .set(
              ...
              target.countCards(
                "h",
                (card) => get.value(card, target, "raw") < 8,
              ) > 1,
            )
  ...
  skill: {
    ...
    lanyinjia2: {
      ...
      filter(event, player) {
        return (
          event.card &&
          event.card.name === "sha" &&
          player.getEquips("lanyinjia").length > 0
        )
      },
      content() {
    ...
    g_jinchan: {
      ...
      content() {
        ...
        player
          .chooseToUse(
            `是否对${get.translation(trigger.card)}使用【金蝉脱壳】？`,
          )
          .set("ai1", (card) => _status.event.bool)
          .set(
            "bool",
    ...
    yinyueqiang: {
      ...
      filter(event, player) {
        ...
        return (
          ...
          player.hasHistory(
            "lose",
            (evt) => evt.getParent() === event && evt.hs?.length === 1,
          )
        )
      },
      direct: true,
      ...
      async content(event, trigger, player) {
        await player
          .chooseToUse(
            `###${get.prompt(event.name)}###对你攻击范围内的一名角色使用一张【杀】`,
          )
    ...
    caomu_skill2: {
      ...
      content() {
        var targets = game.filterPlayer(
          (current) => get.distance(player, current) <= 1 && player !== current,
        )
        if (targets.length) {
