...
export default {
  ...
  card: {
        ...
        while (true) {
          ...
          if (
            ...
            const discardResult = await target
              ...
              .forResult()
            if (!discardResult?.bool && !event.unhurt) {
              await damageTarget()
              return
            }
          ...
          while (event.shaRequired > 0) {
            ...
            if (!event.directHit) {
              ...
              next.set(
                ...
                (card, player) =>
                  get.name(card) === "sha" &&
                  lib.filter.cardRespondable(card, player),
              )
              if (event.shaRequired > 1) {
                next.set("prompt2", `共需打出${event.shaRequired}张杀`)
              }
              next.set("ai", (card) =>
                get.event().toRespond ? get.order(card) : -1,
              )
    ...
    jiedao: {
        ...
        return (
          ...
          game.hasPlayer(
            (current) =>
              ...
              lib.filter.targetEnabled(sha, target, current),
          )
        )
      },
      filterAddedTarget(card, player, target, preTarget) {
        ...
        return (
          ...
          lib.filter.targetEnabled(sha, preTarget, target)
        )
      },
      async content(event, trigger, player) {
        const target = event.target
        ...
        result: {
          player: (player, target) => {
            ...
            return (
              ...
              target
                .getEquips(1)
                .reduce((num, i) => num + get.value(i, player), 0)
            )
          },
          target: (player, target, card) => {
            ...
            if (targets.length) {
              ...
              const pre = _status.event.getTempCache(
                "jiedao_result",
                preTarget.playerid,
              )
              if (
            ...
            let res =
              target
                .getEquips(1)
                .reduce((num, i) => num + get.value(i, target), 0) /
              (target.hasSkillTag("noe") ? -2 : -4)
    ...
    wugu: {
      ...
      async content(event, trigger, player) {
        const { target } = event
        const dialog = ui.dialogs.find(
          (dialog) => dialog.videoId === event.preResult,
        )
          ...
          game.broadcast(
            (card, id, name, capt) => {
              ...
              if (dialog) {
                ...
                const button = dialog.buttons.find(
    ...
    wuxie: {
      ...
      async contentBefore(event, trigger, player) {
        ...
        if (
          triggerEvent.name === "phaseJudge" ||
          triggerEvent.card.name === "wuxie" ||
          triggerEvent.targets.length <= 1
        ) {
          return
        }
        const result = await player
            ...
            triggerEvent
              ...
              .excluded.addArray(
                game.filterPlayer((current) =>
                  current.isFriendOf(triggerEvent.target),
                ),
              )
          }
        }
  ...
  skill: {
          ...
          event.send = (player, map, skillState, eventData) => {
            ...
            if (
              ...
              if (translation.length >= 4) {
                translation =
                  lib.translate[`${map.card.name}_ab`] ||
            ...
            const next = player.chooseToUse({
              ...
              ai1() {
                if (map.isJudge) {
                  ...
                  if (
                    Math.abs(get.attitude(_status.event.player, source)) < 3
                  ) {
                    return 0
                  }
                  if (
                ...
                if (evtmap.target) {
                  ...
                  if (
                    Math.abs(get.attitude(_status.event.player, target)) < 3
                  ) {
                    return 0
                  }
                  return (
            ...
            (event.nature &&
              ...
              game.hasPlayer(
                (current) =>
                  current !== target &&
                  current.isLinked() &&
                  get.damageEffect(current, player, player, event.nature) > 0,
              ))
          ) {
            return false
          }
    ...
    guanshi_skill: {
      ...
      filter(event, player) {
        ...
        if (!player.hasSkill("guanshi_skill", null, false)) {
          min += player.hasCards("e", (card) => get.name(card) === "guanshi")
            ? 1
            : 0
        }
    ...
    qinglong_skill: {
      ...
      filter(event, player) {
        ...
        return (
          ...
          (player.hasSha() || (_status.connectMode && player.hasCards("hs")))
        )
      },
      async content(event, trigger, player) {
        await player
        ...
        trigger: {
          ...
          async content(event, trigger, player) {
            if (
              ...
              trigger
                .getParent()
                ?.excluded.addArray(trigger.getParent()?.targets)
            } else {
    ...
    bagua_skill: {
      ...
      async content(event, trigger, player) {
        ...
        const judgeEvent = player.judge("bagua", (card) =>
