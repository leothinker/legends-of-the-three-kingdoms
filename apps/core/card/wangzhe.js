...
game.import("card", () => ({
  ...
  card: {
    ...
    sha: {
      ...
      ai: {
        ...
        tag: {
          ...
          fireDamage(card, _nature) {
            if (game.hasNature(card, "fire")) {
              return 1
            }
          },
          thunderDamage(card, _nature) {
            if (game.hasNature(card, "thunder")) {
              return 1
            }
          },
          poisonDamage(card, _nature) {
            if (game.hasNature(card, "poison")) {
              return 1
            }
          },
    ...
    tao: {
      ...
      modTarget(_card, _player, target) {
    ...
    guohe: {
        ...
        if (get.is.single()) {
          ...
          if (bool1 && bool2) {
            player
              ...
              .set(
                "prompt",
                `弃置${get.translation(target)}装备区的一张牌，或观看其手牌并弃置其中的一张牌。`,
              )
          } else {
      ...
      ai: {
        wuxie: (target, _card, player, viewer, status) => {
          if (
    ...
    shunshou: {
      ...
      ai: {
        wuxie(target, _card, player, viewer) {
          if (
            !target.countCards("hej") ||
    ...
    shunshou_copy2: {
      ai: {
        ...
        result: {
          ...
          player(player, target, _card, isLink) {
    ...
    jiedao: {
      ...
      filterAddedTarget(_card, _player, target, preTarget) {
        var card = { name: "sha", isCard: true }
        return (
          target !== preTarget &&
      ...
      content() {
        ...
        if (
          ...
          target
            .chooseToUse(
              `对${get.translation(event.addedTarget)}使用一张杀，或令${get.translation(player)}获得你的武器牌`,
              function (card, _player) {
                if (get.name(card) !== "sha") {
                  return false
                }
    ...
    wugu: {
      ...
      contentBefore() {
        ...
        if (card.storage?.chooseDirection || get.is.versus()) {
          player
            .chooseControl("顺时针", "逆时针", (_event, player) => {
              if (
      ...
      contentAfter() {
        for (var _i = 0; i < ui.dialogs.length; i++) {
          if (ui.dialogs[i].videoId === event.preResult) {
    ...
    jiu: {
      ...
      type: "basic",
      toself: true,
      enable(_event, _player) {
        //return !player.hasSkill('jiu');
        return true
      },
      lianheng: true,
      logv: false,
      savable(_card, player, dying) {
        return (
          dying === player || player.hasSkillTag("jiuOther", null, dying, true)
        )
      },
      usable: 1,
      selectTarget: -1,
      ...
      content() {
        ...
        if (target.isDying() || event.getParent(2).type === "dying") {
          ...
          if (
            ...
            typeof target.getStat().card.jiu === "number"
          ) {
    ...
    huogong: {
      ...
      async content(event, _trigger, player) {
        ...
        //返回目标选展示牌的结果，给出默认逻辑
        event.chooseToShow ??= async (event, _player, target) => {
        ...
        const { cards } = result
        event.cards2 = cards
        const showEvent = target
          .showCards(cards, `${get.translation(target)}因【火攻】展示的牌`)
          .set("closeDialog", false)
        await showEvent
        const videoId = showEvent.videoId
        event.videoId = videoId
        //返回玩家弃牌/其他操作的结果，给出默认逻辑
        event.chooseToDiscard ??= async (event, player, _target) => {
          const {
            discardPostion = "h",
            cards2,
            filterDiscard = { suit: get.suit(cards2[0]) },
          } = event
      ...
      ai: {
        ...
        wuxie(target, card, player, viewer, status) {
          ...
          if (
            _status.event.getRand("huogong_wuxie") * 4 >
            player.countCards("h")
          ) {
            return 0
          }
        ...
        result: {
          target: (player, target) => {
            ...
            const curs = game.filterPlayer((current) => {
              if (
                current.hasSkillTag("noLink") ||
                current.hasSkillTag("nodamage")
              ) {
                return false
              }
              return (
                !current.hasSkillTag("nofire") ||
                !current.hasSkillTag("nothunder")
              )
            })
            if (curs.length < 2) {
              return 0
    ...
    tengjia: {
      ...
      subtype: "equip2",
      ai: {
        value(card, player, _index, method) {
  ...
  skill: {
    _wuxie: {
      ...
      content() {
        ...
        event.send = (player, map, skillState, eventData) => {
          ...
          if (
            ...
            if (translation.length >= 4) {
              translation =
                lib.translate[`${map.card.name}_ab`] || translation.slice(0, 2)
            }
          ...
          var next = player.chooseToUse({
            ...
            ai1() {
              ...
              if (evtmap.target) {
                ...
                return (
                  -get.effect(target, card, source, _status.event.player) *
                  state
                )
              }
              var triggerevent = _status.event.getTrigger()
        ...
        var _list = game.filterPlayer((current) => {
        ...
        event.withol = withol
        ;("step 5")
        if (result?.bool && !event.wuxieresult) {
        ...
        ;("step 8")
        if (event.wuxieresult2?._sendskill) {
          lib.skill[event.wuxieresult2._sendskill[0]] =
            event.wuxieresult2._sendskill[1]
        }
        if (event.wuxieresult && event.wuxieresult2?.skill) {
    ...
    cixiong_skill: {
      ...
      content() {
        ...
        if (!trigger.target.countCards("h")) {
          ...
          trigger.target
            .chooseToDiscard(
              `弃置一张手牌，或令${get.translation(player)}摸一张牌`,
            )
            .set("ai", (card) => {
            ...
            .set(
              ...
              (() => {
                ...
                const shans = trigger.target.mayHaveShan(
                  trigger.target,
                  "use",
                  true,
                  "count",
    ...
    zhangba_skill: {
      ...
      check(card) {
        const player = _status.event.player
        if (player.hasCard((card) => get.name(card) === "sha")) {
          return 0
        }
        if (
          _status.event &&
          ...
          !player.countCards(
            "hs",
            (card) => get.name(card) !== "tao" && get.name(card) !== "jiu",
          )
        ) {
    ...
    guanshi_skill: {
      ...
      content() {
        ...
        var next = player
          .chooseToDiscard(get.prompt("guanshi"), 2, "he", (card, player) => {
    ...
    bagua_skill: {
      ...
      content() {
        ...
        player.judge("bagua", (card) =>
          get.color(card) === "red" ? 1.5 : -0.5,
    ...
    tengjia2: {
      ...
      ai: {
        effect: {
          target(card, player, _target, current) {
            if (card.name === "sha") {
              if (game.hasNature(card, "fire")) {
                return 2
              }
    ...
    baiyin_skill: {
      ...
      subSkill: {
        lose: {
          ...
          filter: (_event, player, _name, card) => {
  ...
  translate: {
    sha: "杀",
    sha_info:
      "出牌阶段，对你攻击范围内的一名其他角色使用。若命中，则对目标角色造成1点伤害。",
    sha_notshan: "invisible",
    shan: "闪",
    shan_info:
      "当一张【杀】对你生效前，可响应此【杀】——对此【杀】使用，抵消其的效果。",
    tao: "桃",
    tao_info:
    ...
    shunshou: "顺手牵羊",
    shunshou_info:
      "出牌阶段，对距离为1的一名其他角色使用。获得其所属区域里的一张牌。",
    juedou: "决斗",
    juedou_bg: "斗",
    juedou_info:
    ...
    nanman_bg: "蛮",
    nanman_info:
      "出牌阶段，对所有其他角色使用。每人需打出一张【杀】，否则受到1点伤害。",
    wanjian: "万箭齐发",
    wanjian_bg: "箭",
    wanjian_info:
      "出牌阶段，对所有其他角色使用。每人需打出一张【闪】，否则受到1点伤害。",
    taoyuan: "桃园结义",
    taoyuan_bg: "园",
    ...
    wugu_bg: "谷",
    wugu_info:
      "出牌阶段，对所有角色使用。从牌堆顶亮出等同于目标角色数的牌，每人获得其中一张。",
    wuxie: "无懈可击",
    wuxie_bg: "懈",
    wuxie_info:
    ...
    hanbing_skill: "寒冰剑",
    hanbing_info:
      "你的【杀】造成伤害时，你可以防止此伤害改为弃置目标角色两张牌。",
    hanbing_skill_info:
      "你的【杀】造成伤害时，你可以防止此伤害改为弃置目标角色两张牌。",
    qinglong: "青龙偃月刀",
    qinglong_bg: "偃",
    ...
    guanshi_skill: "贯石斧",
    guanshi_skill_info:
      "你的【杀】被抵消时，你可以弃置两张牌使此【杀】依然造成伤害。",
    guanshi_info:
      "你的【杀】被抵消时，你可以弃置两张牌使此【杀】依然造成伤害。",
    fangtian: "方天画戟",
    fangtian_skill: "方天画戟",
    fangtian_skill_info:
      "若你使用的【杀】是你最后的手牌，则你可以多选择两个目标。",
    fangtian_info: "若你使用的【杀】是你最后的手牌，则你可以多选择两个目标。",
    qilin: "麒麟弓",
    qilin_bg: "弓",
    qilin_skill: "麒麟弓",
    qilin_skill_info:
      "你的【杀】造成伤害时，你可以弃置目标角色装备区里的一张坐骑牌。",
    qilin_info:
      "你的【杀】造成伤害时，你可以弃置目标角色装备区里的一张坐骑牌。",
    ...
    guding: "古锭刀",
    guding_info:
      "锁定技，当你使用【杀】对目标角色造成伤害时，若其没有手牌，则此伤害+1。",
    guding_skill: "古锭刀",
    guding_append:
