...
game.import("card", function () {
  return {
            ...
            value: (card, player) => {
              ...
              game.countPlayer((cur) => {
                ...
                if (dist > 1 || card.yingbian_all) {
                  max = Math.max(
                    max,
                    lib.card.shunshou.ai.result.target(player, cur) * get.attitude(player, cur),
                  )
                } else {
                  max = Math.max(
                    max,
            ...
            target(player, target) {
              ...
              if (get.attitude(player, target) <= 0) {
                return target.countCards("he", function (card) {
                  return (
                    get.value(card, target) > 0 && (discard || card != target.getEquip("jinhe"))
                  )
                }) > 0
                  ? -1.5
                  : 1.5
    ...
    translate: {
      ...
      wutiesuolian_info: "当你使用【杀】指定一名角色为目标后，若其未横置，你令其横置。",
      wutiesuolian_append:
      ...
      tianjitu: "天机图",
      tianjitu_skill: "天机图",
      tianjitu_info: "锁定技，当此牌进入/离开你的装备区时，你弃置一张其他牌/将手牌摸至五张。",
      tianjitu_append:
        '<span class="text" style="font-family: yuanli">其耆欲深者，其天机浅。——《庄子·大宗师》</span>',
      tongque: "铜雀",
      tongque_skill: "铜雀",
      tongque_info: "每回合限一次，你使用具有应变效果的牌无视条件直接发动该牌的应变效果。",
      tongque_append:
