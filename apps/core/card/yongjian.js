...
export default {
  ...
  card: {
    ...
    guaguliaodu: {
      ...
      async content(event, trigger, player) {
        ...
        if (
          target.hasCard(
            (card) => _status.connectMode || get.name(card, target) === "du",
            "h",
          )
        ) {
          await target
        ...
        const controlResult = await target
          ...
          .set("ai", () => {
            ...
            return get.value(card, current) >
              6 + (Math.max(current.maxHp, 3) - current.hp) * 1.5
              ? 1
              : 0
          })
          .forResult()
    ...
    tuixinzhifu: {
      ...
      async content(event, trigger, player) {
        ...
        const result = await player
          .gainPlayerCard(target, "hej", true, [1, 2])
          .forResult()
  ...
  skill: {
    ...
    qixingbaodao: {
      ...
      filter(event, player) {
        ...
        return (
          ...
          player.hasCard((card) => {
            return (
              !event.card.cards.includes(card) &&
              lib.filter.cardDiscardable(card, player, "qixingbaodao")
            )
          }, "ej")
        )
      },
      async content(event, trigger, player) {
        const cards = player.getCards("ej", (card) => {
          return (
            !trigger.card.cards.includes(card) &&
            lib.filter.cardDiscardable(card, player, "qixingbaodao")
          )
        })
        if (cards.length > 0) {
    ...
    g_du: {
      ...
      filter(event, player, name) {
        ...
        if (!evt?.hs?.filter((i) => get.name(i, player) === "du").length) {
          return false
        }
      ...
      async content(event, trigger, player) {
        ...
        if (typeof trigger.getl === "function") {
          num = trigger
            .getl(player)
            .hs.filter((i) => get.name(i, player) === "du").length
        }
    ...
    g_du_give: {
      ...
      async content(event, trigger, player) {
        ...
        const cards =
          trigger.name === "phase"
            ? player._start_cards.filter(
                (card) => get.name(card, player) === "du" && hs.includes(card),
              )
        ...
        while (cards.length) {
          const result = await player
            .chooseCardTarget({
              ...
              ai2(target) {
                if (
                  ["usedu", "keepdu"].some((tag) => target.hasSkillTag(tag))
                ) {
    ...
    _gifting: {
      ...
      selectTargetAi(event, player) {
        const cache = _status.event.getTempCache("_gifting", player.playerid)
      ...
      ai: {
        ...
        result: {
          target(player, target) {
            const cache = _status.event.getTempCache(
              "_gifting",
              player.playerid,
            )
            if (Array.isArray(cache)) {
              for (const arr of cache) {
                if (target === arr[0]) {
                  const element = arr[1].find(
                    (item) => item.card === ui.selected.cards[0],
                  )
  ...
  translate: {
    ...
    nvzhuang: "女装",
    nvzhuang_info:
      "锁定技，当你成为【杀】的目标后，若你为男性，你进行判定：若结果为黑色，此【杀】伤害+1。",
    nvzhuang_append:
