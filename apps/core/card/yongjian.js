...
game.import("card", () => ({
  ...
  skill: {
    ...
    serafuku: {
      ...
      content() {
        "step 0"
        player.judge((card) => (get.color(card) === "black" ? -2 : 0)).judge2 =
    ...
    g_du: {
      ...
      filter(event, player, name) {
        ...
        if (
          !evt ||
          !evt.hs ||
          !evt.hs.filter((i) => get.name(i, player) === "du").length
        ) {
          return false
        }
    ...
    g_du_give: {
      ...
      content() {
        ...
        if (trigger.name === "phase") {
          event.cards = player._start_cards.filter(
            (card) => get.name(card, player) === "du" && hs.includes(card),
          )
        } else {
          event.cards = trigger.cards.filter(
            (card) => get.name(card, player) === "du" && hs.includes(card),
          )
        }
  ...
  translate: {
    ...
    yonglv_append:
      '<span class="text" style="font-family: yuanli">它旁边的就是王仲宣。</span>',
    zhanxiang: "战象",
    zhanxiang_info:
      "锁定技。①其他角色至你的距离+1。②其他角色对你赠予的牌视为赠予失败。",
    xinge: "信鸽",
    xinge_info: "出牌阶段限一次。你可以将一张手牌交给一名其他角色。",
    xinge_append:
      '<span class="text" style="font-family: yuanli">咕咕咕。</span>',
