...
const skills = {
    ...
    onChooseToUse(event) {
      if (!game.online && !event.clanshixi) {
        ...
        const list = Object.entries(storage).filter(([suit, name]) => {
          if (
            !player.hasCards("he", (card) => get.suit(card, player) === suit)
          ) {
            return false
          }
          const card = get.autoViewAs({ name, isCard: true }, "unsure")
          if (
            !lib.skill.clanshixi.filterx(card) ||
    ...
    chooseButton: {
      dialog(event, player) {
        ...
        dialog.add([
          [
            [
              `###拾昔###<div class="text center">将一种花色的所有牌置入弃牌堆视为使用对应花色记录的普通锦囊牌<br>当前花色及对应的牌：${str}</div>`,
            ],
            "addNewRow",
          ],
          [
        ...
        init(player, skill) {
          const history = player.getAllHistory(
            ...
            (evt) =>
              ...
              lib.skill.clanshixi.filterx(evt.card),
          )
          if (history.length) {
  ...
  clanjianbai: {
    ...
    filter(event, player) {
      ...
      if (
        player
          .getHistory("useCard", (evt) => get.type2(evt.card) === type)
          .indexOf(event) !== 0
      ) {
        return false
      }
    ...
    async content(event, trigger, player) {
      ...
      const result =
        suits.length === 1
          ...
          : await player
              .chooseControl(
                lib.suits.slice().filter((suit) => suits.includes(suit)),
              )
      ...
      const keepCards = player.getCards(
        "he",
        (card) => get.suit(card) === keepSuit,
      )
      for (const card of keepCards) {
        ...
        if (tag) {
          ...
          tag = skill + (parseInt(tag.slice(skill.length), 10) + 1)
        } else {
          tag = `${skill}1`
        }
      ...
      const rescastCards = player.getCards(
        "he",
        (card) => get.suit(card) !== keepSuit && player.canRecast(card),
      )
    ...
    subSkill: {
      ...
      effect: {
        ...
        charlotte: true,
        onremove(player, skill) {
          let tags = player.getCards("h", (card) =>
            card.gaintag?.some((tag) => tag.startsWith(skill)),
          )
          if (tags.length) {
            tags = tags
              .slice()
          ...
          if (
            ...
            const result = await player
              .chooseCardTarget({
                ...
                forced: true,
                ai1(card) {
                  const tag = card.gaintag?.find((tag) =>
                    tag.startsWith("clanjianbai_effect"),
                  )
                  let count = 0
                  if (tag) {
                    count =
                      parseInt(tag.slice("clanjianbai_effect".length), 10) || 0
                  }
            ...
            if (
              ...
              let count = 0
              const tag = giveCard.gaintag?.find((tag) =>
                tag.startsWith(event.name),
              )
              if (tag) {
                count = parseInt(tag.slice(event.name.length), 10) || 0
              }
  ...
  clanqingjue: {
    isOnlySuit(card, player) {
      return !player.hasCard(
        (cardx) => cardx !== card && get.suit(cardx) === get.suit(card),
        "h",
      )
    },
    init(player, skill) {
    ...
    filter(event, player) {
      ...
      return (
        game
          .getGlobalHistory(
            "changeHp",
            (evt) => evt.player === player && evt.changedHp !== 0,
          )
          .indexOf(event) === 0 &&
        player.hasDiscardableCards(
          player,
          "h",
          (card) => !get.info("clanqingjue").isOnlySuit(card, player),
        )
      )
    },
    forced: true,
    async content(event, trigger, player) {
      const result = await player
        ...
        .set(
          "filterCard",
          (card, player) => !get.info("clanqingjue").isOnlySuit(card, player),
        )
        .forResult()
      const { cards } = result
      const resultx =
                ...
                if (button.link === "give") {
                  if (
                    game.hasPlayer(
                      (target) =>
                        target !== get.player() &&
                        get.attitude(get.player(), target) > 0,
                    )
                  ) {
                    return 2
                  }
        ...
        const result = await player
          ...
          .set(
            ...
            (target) =>
              ...
              get.value(get.event().toGive, target),
          )
          .forResult()
        const {
          targets: [target],
        ...
        init(player, skill) {
          ...
          player.addGaintag(
            player.getCards("h", (card) =>
              get.info("clanqingjue").isOnlySuit(card, player),
            ),
            skill,
          )
        },
    ...
    filter(event, player) {
      ...
      return game.hasPlayer((target) => {
        ...
        return evt?.hs?.some((card) =>
          evt.gaintag_map?.[card.cardid]?.includes("clanxsyingxiang"),
        )
      })
    },
    ...
    subSkill: {
      used: {
        ...
        init(player, skill) {
          player.addTip(
            skill,
            `${get.translation(skill)} 已${get.translation("clanqingjue")}`,
          )
        },
        onremove(player, skill) {
        ...
        getIndex(event, player) {
          return game
            .filterPlayer((target) => {
              ...
              return (
                gain.length > 0 &&
                (event.giver === player || lose.some((i) => gain.includes(i)))
              )
            })
            .sortBySeat()
        },
  ...
  clanfenjian: {
      ...
      if (!game.online && !event.clanfenjian) {
        ...
        event.targetprompt2.push((target) => {
          if (
            ...
            event.skill !== "clanfenjian"
          ) {
            return false
          }
          const num =
            target.getAttackRange() - get.event().player.getAttackRange()
          if (num < 0) {
            return "小于你"
          }
          if (num > 0) {
      ...
      return game.hasPlayer((target) => {
        return (
          !player
            .getStorage(name)
            .includes(
              Math.sign(target.getAttackRange() - player.getAttackRange()),
            ) && player.canUse(card, target, void 0, false)
        )
      })
    },
    filterTarget(card, player, target) {
      return (
        !player
          .getStorage("clanfenjian_used")
          .includes(
            Math.sign(target.getAttackRange() - player.getAttackRange()),
          ) &&
        lib.filter.filterTarget(
          get.autoViewAs(get.info("clanfenjian").viewAs),
          player,
          target,
        )
      )
  ...
  clandongxu: {
    ...
    ignoreMod: true,
    viewAsFilter(player) {
      return !player.storage.clandongxu
        ? player.hasCards("he", { type: "equip" })
    ...
    ai: {
      ...
      skillTagFilter(player, tag, arg) {
        ...
        if (
          tag === "respondShan" &&
          ((!bool &&
            !player.hasCards("he", (card) => get.type(card) === "equip")) ||
            (bool &&
              player.countCards("h") >= Math.min(player.getAttackRange(), 5)))
        ) {
          return false
            ...
            event.result = await player
              .chooseCardTarget({
                ...
                filterTarget(card, player, target) {
                  return (
                    target !== player &&
                    target.canEquip(ui.selected.cards[0], true)
                  )
                },
                position: "he",
                ai1(card) {
  ...
  clangezhi: {
    ...
    filter(event, player) {
      return (
        ...
        (player.countDiscardableCards(player, "he") > 0 ||
          event.source.countDiscardableCards(player, "he") > 0)
      )
    },
    check: (event, player) =>
    ...
    async assignCards(event, trigger, player) {
      ...
      if (
        cards.some(
          (card) =>
            get.owner(card) !== player || !"he".includes(get.position(card)),
        )
      ) {
        event.type = "button"
      } else {
      ...
      ai2 ??= (target) => {
        const player = get.player(),
          card =
            get.event().name === "chooseCardTarget"
              ? ui.selected.cards[0]
        ...
        while (assigned.length < cards.length && goon(event)) {
          const result = await player
            .chooseButtonTarget({
              createDialog: [
                prompt,
                cards.filter((i) => !assigned.includes(i)),
              ],
              forced: typeof forced === "boolean" ? forced : forced(event),
      ...
      debuff: {
        ...
        mod: {
          cardEnabled(card, player) {
            if (
              !player.getStorage("clangezhi_debuff").includes(get.type2(card))
            ) {
              return false
            }
          },
    ...
    filter(event, player) {
      return player.hasCard(
        (card) =>
          ...
          !player.getStorage("clanmingdian_used").includes(get.name(card)),
        "he",
      )
    },
    filterCard(card) {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseCard(
          ...
          [1, Infinity],
          "he",
          get.info(event.skill).filterCard,
        )
    ...
    async content(event, trigger, player) {
      ...
      if (
        player.hasHistory("useCard", (evt) =>
          names.includes(get.name(evt.card)),
        )
      ) {
        const list = player
          .getCards("h")
        ...
        const card = get.cardPile(
          (card) =>
            get.type(card) === "basic" && !list.includes(get.name(card)),
          void 0,
          "random",
        )
        if (card) {
  ...
  clanshize: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        game.hasPlayer(
          (target) => target.hasClan("颍川陈氏") || target === player,
        )
      )
    },
    async cost(event, trigger, player) {
    ...
    filter(event, player) {
      return (
        game
          .getGlobalHistory(
            "useCard",
            (evt) => evt.card.name === event.card.name,
          )
          .indexOf(event) === 1 &&
        game.hasPlayer(
          (target) =>
            !event.targets.includes(target) &&
            !player.getStorage("clandingan_used").includes(target),
        ) &&
        !game.getGlobalHistory("everything", (evt) => evt.name === "dying")
          .length
      )
    },
    forced: true,
    async content(event, trigger, player) {
      ...
      const pre_targets = game.filterPlayer((target) => {
        return (
          ...
          !player.getStorage(skill).includes(target)
        )
      })
      let result =
        pre_targets.length > 1
          ? await player
              ...
              .set(
                "prompt2",
                "然后你令一名手牌最多的其他角色执行一项：1.受到你造成的1点伤害；2.弃置手牌中最多的同名牌。",
              )
              .set("ai", (target) => {
      ...
      const currents = game.filterPlayer(
        (target) =>
          target !== player &&
          target.isMaxHandcard(false, (current) => current !== player),
      )
      if (!currents?.length) {
        return
      }
      ...
      result = await player
        ...
        .set("ai", (button) => {
          ...
          return (
            get.sgnAttitude(player, target) * Math.sqrt(target.countCards("h"))
          )
        })
        .forResult()
  ...
  clanfuning: {
    ...
    filter(event, player) {
      ...
      const evts = game.getRoundHistory(
        "changeHp",
        (evt) => evt.player === player && evt.changedHp !== 0,
      )
    ...
    async cost(event, trigger, player) {
      const num = Math.max(1, player.getDamagedHp()),
        count = game.countPlayer2(
          (current) => current.hasHistory("damage"),
          true,
        )
      event.result = await player
        .chooseCardTarget({
          ...
          ai1(card) {
            ...
            if (
              !player.isDamaged() ||
              ui.selected.cards.every((cardx) => get.color(cardx) === color)
            ) {
              const num = ui.selected.cards.length,
                num2 = player.countCards(
                  "h",
                  (cardx) => !ui.selected.cards.includes(cardx),
                )
    ...
    async content(event, trigger, player) {
      ...
      const bool1 =
          cards.map((card) => get.color(card)).toUniqued().length === 1,
        bool2 =
          cards.length >
          game.countPlayer2((current) => current.hasHistory("damage"), true)
      await player.give(cards, target)
      if (bool1) {
    ...
    subSkill: {
      effect: {
        ...
        filter(event, player) {
          if (
            !player.isLinked() ||
            ["equip", "delay"].includes(get.type(event.card))
          ) {
            return false
          }
  ...
  clanyingxiang: {
    ...
    async content(event, trigger, player) {
      ...
      if (cards.some((card) => card.name === name)) {
        if (
          player.hasSkill("clanxumin", null, null, false) &&
          !player.hasSkill("clanxumin")
        ) {
  ...
  clanfennu: {
    ...
    intro: {
      ...
      mark(dialog, storage, player) {
        ...
        const num1 = list.reduce(
          (sum, card) => sum + get.number(card, false),
          0,
        )
    ...
    subSkill: {
      record: {
        ...
        filter(event, player) {
          if (
            !player.getExpansions("clanfennu").length ||
            !event.isFirstTarget
          ) {
            return false
          }
      ...
      gain: {
        ...
        filter(event, player) {
          if (
            !player.getExpansions("clanfennu").length ||
            !player.countMark("clanfennu_record")
          ) {
            return false
          }
          const num1 = player
              .getExpansions("clanfennu")
              .reduce((sum, card) => sum + get.number(card, false), 0),
      ...
      return game
        .filterPlayer((current) => {
          if (
            ...
            (!current.hasClan("吴郡陆氏") && current !== player)
          ) {
            return false
          }
        ...
        .step(async (event, trigger, player) => {
          if (
            ...
            const { bool } = await target
              .chooseBool(
                "探锋",
                `是否视为对${get.translation(player)}使用一张【杀】？`,
              )
  ...
  clanjuewei: {
    ...
    filter(event, player, name) {
      ...
      return player.countCards("he", (card) => {
        ...
        return (
          lib.filter.cardDiscardable(card, player, "clanjuewei") ||
          lib.filter.cardRecastable(card, player)
        )
      })
    },
    usable: 1,
      ...
      const result = await player
        ...
        .set("ai", (button) => {
          ...
          if (button.link === "Recast") {
            if (
              !trigger.targets ||
              trigger.targets.every((target) => target === player)
            ) {
              return 0
            }
        ...
        .chooseCard(
          ...
          (card) => {
            ...
            return (
              ...
              lib.filter[type](card, player, source)
            )
          },
          "he",
        )
        ...
        player
          ...
          .step(async (event, trigger, player) => {
            const targets = trigger.targets.filter(
              (target) => target !== player && target.isIn(),
            )
            if (targets?.length) {
    ...
    subSkill: {
      ...
      effect: {
        ...
        onremove(player) {
          game
            .filterPlayer()
            .forEach((target) => target.removeGaintag("clanjiannan"))
        },
        charlotte: true,
        filter(event, player, triggername, target) {
          if (
            _status?.dying?.length ||
            player.getStorage("clanjiannan_used").length > 3
          ) {
            return false
          }
          return true
        ...
        getIndex(event, player, triggername) {
          const targets = game.filterPlayer((target) => {
            ...
            if (
              !evt?.cards2?.length ||
              target.countCards("h", (card) => card.hasGaintag("clanjiannan"))
            ) {
              return false
            }
            if (event.name === "lose") {
              return Object.values(event.gaintag_map)
                .flat()
                .includes("clanjiannan")
            }
            return target.hasHistory("lose", (evtx) => {
              if (event !== evtx.getParent()) {
                return false
              }
              return Object.values(evtx.gaintag_map)
                .flat()
                .includes("clanjiannan")
            })
          })
            ...
            .chooseButtonTarget({
              ...
              ai1: (button) => {
                ...
                switch (button.link) {
                  ...
                  case "recast": {
                    if (
                      player.hasCard((card) => get.type(card) === "equip", "he")
                    ) {
                      return 3
                    }
                    break
        ...
        .set("ai", (target) => {
          const player = get.player(),
            num = player.getHistory("useSkill", (evt) =>
              ["clanjiannan", "clanjiannan_effect"].includes(evt.skill),
            ).length
          return num > 1
  ...
  clanjuetu: {
    ...
    phaseDiscard: [
      ...
      async (event, trigger, player) => {
        ...
        if (
          lib.suits.some(
            (suit) => suits.filter((suitx) => suitx === suit).length > 1,
          )
        ) {
          const result = await player
            .chooseCard(
              "请保留每种花色的手牌各一张，将其余手牌置入弃牌堆",
              [1, Infinity],
              true,
              (card, player) => {
        ...
        if (
          game.hasPlayer((target) => target.countDiscardableCards(target, "h"))
        ) {
          ...
          next.setContent(async (event, trigger, player) => {
            const result = await target
              .chooseToDiscard("绝途：请弃置一张手牌", true, "h")
              .forResult()
            if (!result?.cards?.length) {
              event.finish()
              return
            ...
            if (
              !player.hasCard((cardx) => get.suit(cardx, player) === suit, "h")
            ) {
              await target.damage()
            }
          })
  ...
  clankudu: {
    ...
    async content(event, trigger, player) {
      ...
      const cards = event.cards,
        num = Math.min(
          5,
          Math.abs(get.number(cards[0]) - get.number(cards[1])),
        ),
    ...
    subSkill: {
      effect: {
        intro: {
          content:
            "<li>下#个回合结束时摸一张牌<br><li>第#个回合后执行一个额外回合",
        },
        onremove: true,
        charlotte: true,
        forced: true,
  ...
  clanyangji: {
    ...
    async content(event, trigger, player) {
      const next = player
        .chooseToUse(get.prompt2(event.name))
      ...
      if (
        !player.hasHistory(
          "sourceDamage",
          (evt) => evt.getParent(next.name) === next,
        ) &&
        target?.canAddJudge("lebu")
      ) {
        await player
          .chooseToUse()
          .set(
            "openskilldialog",
            `佯疾：是否将一张黑桃牌当作【乐不思蜀】对${get.translation(target)}使用？`,
          )
          .set("norestore", true)
    ...
    subSkill: {
      backup: {
        ...
        filterTarget(card, player, target) {
          return (
            lib.filter.judge(card, player, target) &&
    ...
    filter(event, player) {
      return (
        game
          .getAllGlobalHistory(
            "everything",
            (evt) => evt.name === "judge" && evt.player === event.player,
          )
          .indexOf(event) === 0
      )
    },
  ...
  clanjiewu: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          const items = target.getCards("h")
          const count = [
            ...new Set(items.map((item) => get.suit(item, target))),
          ].length
          const player = get.player()
          return (
            (get.effect(target, { name: "draw" }, target, player) * items) /
            (count + 1)
          )
        })
        .forResult()
    ...
    async content(event, trigger, player) {
      ...
      const func = (target) =>
        target.markSkill("clanjiewu_view", null, null, true)
      event.isMine()
        ? func(target)
        : player.isOnline2() && player.send(func, target)
      player
      ...
      effect: {
        ...
        onremove(player, skill) {
          if (player.storage[skill]) {
            Array.isArray(player.storage[skill]) &&
              player.storage[skill].forEach((i) =>
                i.removeSkill("clanjiewu_view"),
              )
        ...
        async cost(event, trigger, player) {
          event.result = await player
            ...
            .set(
              "filterTarget",
              (card, player, target) =>
                target.hasCard(true, "h") &&
                player.getStorage("clanjiewu_effect").includes(target),
            )
            .set("ai", (target) => {
              const items = target.getCards("h")
              const count = [
              ...
              return (
                (4 - count) *
                get.effect(target, { name: "draw" }, target, player)
              )
            })
            .forResult()
        },
        async content(event, trigger, player) {
            ...
            cards = (
              await player
                .choosePlayerCard(
                  ...
                  true,
                  "h",
                  `捷悟：展示${get.translation(target)}的一张手牌`,
                )
                .forResult()
            ).cards
          }
          ...
          await player
            .showCards(
              card,
              `${get.translation(player)}对${get.translation(target)}发动了【捷悟】`,
            )
            .set("clanjiewu", true)
          ...
          if (
            ...
            if (target.countCards("h") !== player.countCards("h")) {
              ...
              if (player !== putee) {
                cardsx = (
                  await player
                    .choosePlayerCard(
                      ...
                      true,
                      "he",
                      `捷悟：将${get.translation(putee)}的一张牌置于牌堆顶`,
                    )
                    .forResult()
                ).cards
              } else {
                cardsx = (
                  await player
                    .chooseCard("he", true, "捷悟：将你的一张牌置于牌堆顶")
                    .forResult()
                ).cards
              }
              const card = cardsx[0]
  ...
  clangaoshi: {
    ...
    filter: (event, player) =>
      player.hasHistory("useSkill", (evt) =>
        ["clanjiewu", "clanjiewu_effect"].includes(evt.skill),
      ),
    prompt(event, player) {
  ...
  clanqieyi: {
    ...
    filter(event, player) {
      if (event.name === "useCard") {
        if (
          !player.hasSkill("clanqieyi_effect") ||
          !lib.suits.includes(get.suit(event.card))
        ) {
          return false
        }
        const history = player.getHistory(
          "useCard",
          (evt) => get.suit(evt.card) === get.suit(event.card),
        )
    ...
    async content(event, trigger, player) {
      if (trigger.name === "phaseUse") {
        ...
        if (
          get.color(card) === get.color(trigger.card) ||
          get.type2(card) === get.type2(trigger.card)
        ) {
          await player.gain(card, "gain2")
        } else {
          ...
          const result = await player
            .chooseCard(`切议：将一张牌置于牌堆顶`, "he", true)
            .forResult()
        ...
        init(player, skill) {
          const suits = player
            .getHistory("useCard", (evt) =>
              lib.suits.includes(get.suit(evt.card)),
            )
          ...
          if (suits.length) {
            player.addTip(
              ...
              get.translation(skill) +
                suits.reduce((str, i) => str + get.translation(i), ""),
            )
          }
        },
        onremove(player, skill) {
  ...
  clanjianzhi: {
    ...
    filter(event, player) {
      const num = player.getHistory(
        "useSkill",
        (evt) => evt.skill === "clanqieyi",
      ).length
      return num > 0
    },
    async content(event, trigger, player) {
      const list = Array.from({
        length: player.getHistory(
          "useSkill",
          (evt) => evt.skill === "clanqieyi",
        ).length,
      ...
      const result = await player
        ...
        .set(
          "prompt",
          `###谏直###进行至多${get.cnNumber(list.length)}次判定，并执行后续效果`,
        )
      ...
      const suits = player
        ...
        .map((evt) => get.suit(evt.card))
        .unique()
      const cards = judge.filter((card) =>
        suits.includes(get.suit(card, false)),
      )
      if (cards.length) {
        if (_status.connectMode) {
          game.broadcastAll(() => {
                ...
                ai2(target) {
                  ...
                  if (att > 0) {
                    ...
                    return Math.max(
                      0.1,
                      att /
                        Math.sqrt(1 + target.countCards("h") + cards.length),
                    )
                  }
                  return 0
                },
            ...
            result = await player
              ...
              .set("ai", (target) => {
                ...
                if (att > 0) {
                  ...
                  return Math.max(
                    0.1,
                    att / Math.sqrt(1 + target.countCards("h") + cards.length),
                  )
                }
                return 0
              })
  ...
  clanquhuo: {
    ...
    filter(event, player) {
      ...
      if (
        !hs.some(
          (card) => get.type(card) === "equip" || get.name(card) === "jiu",
        )
      ) {
        return false
      }
      ...
      return (
        ...
        game.hasPlayer((target) => {
          return (
            target.isDamaged() &&
            (target.hasClan("弘农杨氏") || target === player)
          )
        })
      )
    },
  ...
  clangaojin: {
    ...
    filter(event, player) {
      return (
        player.getHistory("useCard", (evt) => evt.card).indexOf(event) ===
        player.getHandcardLimit() - 1
      )
    },
    async content(event, trigger, player) {
      player.addTempSkill(`${event.name}_effect`)
  ...
  clananran: {
      ...
      const result = await player
        ...
        .set("ai", (button) => {
          ...
          return game
            ...
            .sort((a, b) => {
              return (
                get.effect(b, { name: "draw" }, player, player) -
                get.effect(a, { name: "draw" }, player, player)
              )
            })
            .slice(0, count)
            .reduce(
              (sum, target) =>
                sum + get.effect(target, { name: "draw" }, player, player),
              0,
            )
        })
        .forResult()
    ...
    async content(event, trigger, player) {
      ...
      if (cost_data === "draw") {
        ...
        const result = await player
          .chooseTarget(
            `岸然：令至多${get.cnNumber(count)}名角色各摸一张牌`,
            [1, count],
            true,
          )
          .forResult()
          ...
          cardEnabled(card) {
            if (
              [card]
                ...
                .some(
                  (c) =>
                    get.itemtype(c) === "card" && c.hasGaintag("clananran_tag"),
                )
            ) {
              return false
            }
  ...
  clankaiji: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer((current) =>
        get.info("clankaiji").filterTarget(null, player, current),
      )
    },
    onChooseToUse(event) {
      ...
      const player = event.player
      const chosen = player
        .getRoundHistory("useSkill", (evt) => evt.skill === "clankaiji")
        .reduce((list, evt) => list.add(evt.targets[0]), [])
      event.set("clankaiji_enabledTargets", chosen)
    },
    filterTarget(card, player, target) {
      return (
        !get.event().clankaiji_enabledTargets.includes(target) &&
        player.countDiscardableCards(target, "h")
      )
    },
    async content(event, trigger, player) {
      const { target } = event
      ...
      const { bool } = await player
        .chooseUseTarget(
          `你可以使用${get.translation(card)}然后摸一张牌`,
          card,
          false,
        )
        .forResult()
    ...
    ai: {
      ...
      result: {
        player(player, target) {
          if (
            !player.hasCard(
              (card) => player.hasUseTarget(card, undefined, true),
              "h",
            )
          ) {
            return -1
          ...
          if (
            player.countCards("h", (card) =>
              player.hasUseTarget(card, undefined, true),
            ) >=
            player.countCards(
              "h",
              (card) => !player.hasUseTarget(card, undefined, true),
  ...
  clanchengqi: {
    getUsed: (player) =>
      player
        .getHistory("useCard", (evt) =>
          ["basic", "trick"].includes(get.type(evt.card, null, false)),
        )
    ...
    filter(event, player) {
      ...
      return get
        ...
        .some((card) =>
          event.filterCard(
            get.autoViewAs({ name: card[2], nature: card[3] }, "unsure"),
            player,
            event,
          ),
        )
        ...
        return {
          ...
          filterOk() {
            return (
              ...
              get.cardNameLength(lib.skill.clanchengqi_backup.viewAs.name)
            )
          },
          ai1(card) {
            ...
            if (
              ...
              if (
                ...
                get.cardNameLength(card) +
                  get.cardNameLength(ui.selected.cards[0])
              ) {
  ...
  clanjieli: {
      ...
      const num =
        player.getHistory("useCard").length > 0
          ? Math.max(
              ...player
                .getHistory("useCard")
                .map((history) => get.cardNameLength(history.card)),
            )
          : 0
      const str =
        num > 0
          ? `并观看牌堆顶${get.cnNumber(num)}张牌，然后你可以交换其中任意张牌`
          : ""
      event.result = await player
        .chooseTarget(
          get.prompt(event.skill),
        ...
        .set("ai", (target) => {
          const player = get.event().player
          const num = Math.max(
            ...target.getCards("h").map((card) => get.cardNameLength(card)),
          )
    ...
    async content(event, trigger, player) {
      ...
      const cards = target.getCards(
        "h",
        (card) => get.cardNameLength(card) === limit,
      )
      if (num > 0) {
        ...
        const result = await player
          ...
          .set("processAI", (list) => {
            ...
            const cards1 = list[0][1]
              .slice()
              .sort(
                (a, b) => get.value(a, "raw") * sgn - get.value(b, "raw") * sgn,
              )
            const cards2 = list[1][1]
              .slice()
              .sort(
                (a, b) => get.value(b, "raw") * sgn - get.value(a, "raw") * sgn,
              )
            return [
              cards1.slice().addArray(cards2.slice(0, num)),
              cards2.slice().addArray(cards1.slice(0, num)),
            ]
          })
          .forResult()
        if (result.bool) {
        ...
        const content = [
          `###诫厉###<div class="text center">${get.translation(target)}牌名字数最多的手牌</div>`,
          cards,
        ]
  ...
  clantanque: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          get.prompt(event.skill),
          `对一名体力值或手牌数为${delNum}的角色造成1点伤害`,
          (card, player, target) => {
            return (
              target.getHp() === get.event().delNum ||
              target.countCards("h") === get.event().delNum
            )
          },
        )
        .set("delNum", delNum)
  ...
  clanshengmo: {
    ...
    async content(event, trigger, player) {
      ...
      const names = lib.inpile.filter(
        ...
        cards = evt.clanshengmo_cards.sort(
          (a, b) => get.number(a, false) - get.number(b, false),
        ),
        canChoose = cards.filter(
          (card) =>
            !player
              .getStorage("clanshengmo_num")
              .includes(get.number(card, false)),
      ...
      if (links?.length) {
        ...
        player.markAuto(
          "clanshengmo_num",
          links.map((card) => get.number(card, false)).toUniqued(),
        )
        const numbers = cards.map((card) => get.number(card, false)).unique()
        const [min, max] = [Math.min(...numbers), Math.max(...numbers)],
  ...
  clanlilun: {
    ...
    filter(event, player) {
      return player.hasCard(
        (card) => get.info("clanlilun").filterCard(card, player),
        "he",
      )
    },
    filterCard(card, player) {
    ...
    check(card) {
      ...
      if (
        ui.selected.cards.length &&
        value(card, player) < value(ui.selected.cards[0], player)
      ) {
    ...
    ai: {
      order(item, player) {
        ...
        cards = cards.filter(
          (card) => cards.filter((i) => i.name === card.name).length > 1,
        )
        if (!cards.length) {
          return 1
        }
    ...
    async content(event, trigger, player) {
      const { bool } = await player
        ...
        .set("ai", (card) => {
          ...
          const num = player
            .getCards("h")
            .reduce((sum, card) => sum + get.number(card), 0)
    ...
    intro: {
      ...
      mark(dialog, storage = {}) {
        ...
        const types = ["basic", "trick", "equip"]
          .concat(Object.keys(storage).map((list) => list.split("+")[1]))
          .toUniqued()
        ...
        addNewRow(
          ...["花色"]
            .concat(types.map((i) => get.translation(i)))
            .map((type) => {
              return { item: type, ratio: type === "花色" ? 1 : 2 }
            }),
        )
        for (const suit of suits) {
          const list = [{ item: get.translation(suit), ratio: 1 }]
      ...
      count: {
        ...
        init(player) {
          ...
          const cards_old = player.getCards("h", (card) => {
            ...
            return (
              (suit === "none" ||
                `${suit}+${get.type2(card, false)}` in storage) &&
              card.hasGaintag("clanbaichu_new")
            )
          })
          if (cards_old.length) {
          ...
          const cards_trick = player.getCards(
            "h",
            (card) =>
              Object.values(storage).includes(get.name(card, false)) &&
              !card.hasGaintag("clanbaichu_trick"),
          )
        ...
        filter(event, player) {
          if (
            ["gain", "loseAsync"].includes(event.name) &&
            !event.getg?.(player)?.length
          ) {
            return false
          }
        ...
        intro: {
          content(storage, player) {
            ...
            for (var i = 0; i < storage.length; i++) {
              ...
              infos.add(
                `对${get.translation(list[0])}使用${strx[0]}后，视为对其使用${strx[1]}`,
              )
            }
        ...
        async content(event, trigger, player) {
          ...
          if (trigger.card.name === "sha") {
            matchedList = player
              .getStorage("clanqiuxin_effect")
              .filter(
                (item) => trigger.targets.includes(item[0]) && item[1] === 0,
              )
          }
          if (get.type(trigger.card) === "trick") {
            matchedList = player
              .getStorage("clanqiuxin_effect")
              .filter(
                (item) => trigger.targets.includes(item[0]) && item[1] === 1,
              )
          }
    ...
    async content(event, trigger, player) {
      ...
      const result = await trigger.player
        .chooseCard(
          ...
          "he",
          (card, player) =>
            _status.event.cards.includes(card) && player.canRecast(card),
          "allowChooseAll",
        )
        .set("ai", (card) => {
  ...
  clanhuanghan: {
    ...
    check(event, player) {
      const num = get.cardNameLength(event.card) - player.getDamagedHp()
      if (num >= 0) {
        return true
      }
      if (num < -1) {
    ...
    ai: {
      ...
      effect: {
        target(card, player, target) {
          if (
            !lib.translate[card.name] ||
            !get.tag(card, "damage") ||
            player.hasSkillTag("jueqing", false, target)
          ) {
            return
          }
          const num = get.cardNameLength(card) - target.getDamagedHp()
          if (num > 0) {
  ...
  clanfuxun: {
    ...
    check(card) {
      ...
      var targets = game.filterPlayer(
        (target) =>
          target !== player &&
          lib.skill.clanfuxun.ai.result.target(player, target) !== 0,
      )
      targets.sort(
        (a, b) =>
  ...
  clanjiexuan: {
    ...
    check(card) {
      ...
      var fix =
        ...
        (get.position(card) !== "h" || get.suit(card) === "spade")
          ? 2
          : 1
  ...
  clanmingjie: {
      ...
      if (!lib.skill[skill]) {
        lib.skill[skill] = {
          ...
          intro: {
            markcount: () => 0,
            content: (storage) =>
              `已被${get.translation(storage[1])}指定为【铭戒】目标`,
          },
    ...
    filterTarget(card, player, target) {
      return !Object.keys(target.storage).some((skill) => {
        return (
          skill.startsWith(`clanmingjiex_${player.playerid}_`) &&
          target.storage[skill][0] === 1 + (_status.currentPhase === target)
        )
      })
    },
    skillAnimation: true,
    ...
    subSkill: {
        ...
        filter(event, player) {
          ...
          if (event.targets && !info.multitarget) {
            return game.filterPlayer().some((current) => {
              if (
                !Object.keys(current.storage).some((skill) =>
                  skill.startsWith(`clanmingjiex_${player.playerid}_`),
                )
              ) {
                return false
          ...
          event.result = await player
            .chooseTarget(
              ...
              (card, player, target) => {
                ...
                if (
                  trigger.targets.includes(target) ||
                  !Object.keys(target.storage).some((skill) =>
                    skill.startsWith(`clanmingjiex_${player.playerid}_`),
                  )
                ) {
                  return false
      ...
      clear: {
        ...
        filter(event, player) {
          return Object.keys(player.storage).some((i) =>
            i.startsWith("clanmingjiex_"),
          )
        },
        forced: true,
        popup: false,
        firstDo: true,
        async content(event, trigger, player) {
          const storages = Object.keys(player.storage).filter((i) =>
            i.startsWith("clanmingjiex_"),
          )
      ...
      targeted: {
        ...
        filter(event, player) {
          if (
            !Object.keys(event.player.storage).some((skill) => {
              return (
                skill.startsWith(`clanmingjiex_${player.playerid}_`) &&
                event.player.storage[skill][0] === 1
              )
            })
          ) {
            return false
        ...
        async content(event, trigger, player) {
          ...
          while (
            cards.some(
              (card) =>
                get.position(card, true) === "d" && player.hasUseTarget(card),
            )
          ) {
            const result = await player
      ...
      record: {
        ...
        filter(event, player, name) {
          ...
          return (
            get.type(event.respondTo[1]) !== "trick" ||
            ["caochuan", "wuxie"].includes(event.card.name)
          )
        },
        silent: true,
        forced: true,
  ...
  clanxiaoyong: {
    ...
    filter(event, player) {
      ...
      if (
        player.hasHistory(
          "useCard",
          (evt) => evt !== event && get.cardNameLength(evt.card) === len,
          event,
        )
      ) {
        return false
      ...
      aiOrder(player, card, num) {
        ...
        if (numx === get.cardNameLength(card)) {
          if (
            !player.hasHistory(
              "useCard",
              (evt) => numx === get.cardNameLength(evt.card),
            )
          ) {
            return num + 9
          }
        ...
        result = await target
          ...
          .set("ai", (card) => {
            if (_status.event.canGive) {
              return (
                5 +
                Math.max(0, 3 - _status.event.player.hp) / 1.5 -
                get.value(card)
              )
            }
            return 0
        ...
        target(player, target) {
          if (
            player.hasCard((card) => {
              return (
                get.value(card) < 5 &&
                !["shan", "tao", "jiu", "wuxie", "caochuan"].includes(
                  get.name(card),
                )
              )
            }, "he")
  ...
  clanlianzhu: {
    ...
    intro: {
      content(storage) {
        ...
        if (!storage) {
          str +=
            "Ａ可以重铸一张牌，然后你可以重铸一张牌。若这两张牌颜色相同，则你的手牌上限+1。"
        } else {
          str +=
            "Ａ可以令你选择一名在你或Ａ攻击范围内的另一名其他角色Ｂ，然后Ａ和你可依次选择是否对Ｂ使用一张【杀】。若这两张【杀】颜色不同，则你的手牌上限-1"
        }
        return str
      },
    },
          ...
          if (cards?.length) {
            ...
            result =
              targets.length === 1
                ...
                : await target
                    .chooseTarget(
                      `联诛：选择${player === target ? "你" : `${get.translation(player)}与你`}使用【杀】的目标`,
                      true,
                      (card, player, target) => {
            ...
            const bool2 =
              usedCards.length === 2 &&
              get.color(usedCards[0], false) === get.color(usedCards[1], false)
            if (!bool1 && !bool2) {
              lib.skill.chenliuwushi.change(target, -1)
            }
          }
  ...
  clanliuju: {
    ...
    async content(event, trigger, player) {
      ...
      game.getGlobalHistory("cardMove", (evt) => {
        if (
          evt.getParent(2).name === "chooseToCompare" &&
      ...
      while (true) {
        const cardsx = cards.filter(
          (i) => get.position(i, true) === "d" && loser.hasUseTarget(i),
        )
        if (!cardsx.length) {
          break
        }
      ...
      if (
        shouldCheckDistance &&
        (get.distance(player, target) !== distanceFromPlayer ||
          get.distance(target, player) !== distanceFromTarget)
      ) {
        player.restoreSkill("clanxumin")
  ...
  clanlianhe: {
    ...
    subSkill: {
      effect: {
        ...
        filter(event, player) {
          return (
            player.getStorage("clanlianhe_effect").includes(event.player) &&
            event.clanlianhe_check
          )
        },
        marktext: "连",
        intro: {
          content: (storage = [], player) =>
            `已选择目标：${get.translation(storage.toUniqued())}`,
        },
        async content(event, trigger, player) {
  ...
  clanhuanjia: {
    ...
    async content(event, trigger, player) {
      ...
      const cardsx = cards.filter(
        (i) => get.position(i, true) === "d" && winner.hasUseTarget(i),
      )
      if (cardsx.length) {
        result = await winner
  ...
  clansankuang: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          "三恇：选择一名其他角色",
          `令其交给你至少X张牌${cards.length ? `，然后其获得${get.translation(cards)}` : ""}（X为以下条件中其满足的项数：场上有牌、已受伤、体力值小于手牌数）`,
          true,
          lib.filter.notMe,
        )
        .set("ai", (target) => {
          ...
          return (
            -Math.sqrt(Math.abs(att)) - lib.skill.clansankuang.getNum(target)
          )
        })
        .set(
          "goon",
  ...
  clanbeishi: {
    ...
    intro: {
      content: [
        "每当$失去最后的手牌，你回复1点体力",
        '<span style="font-family:yuanli">“唉，我本袁氏故吏，又能做些什么呢……”</span>',
      ].join("<br><br>"),
    },
    subSkill: {
      mark: {
        ...
        filter(event, player) {
          ...
          return (
            history.map((evt) => evt.event).indexOf(event.getParent()) === 0
          )
        },
        async content(event, trigger, player) {
  ...
  clanshenjun: {
    ...
    subSkill: {
      viewAs: {
        ...
        filter(event, player) {
          return (
            player.countCards("h", (card) => card.hasGaintag("clanshenjun")) > 0
          )
        },
        forced: true,
        charlotte: true,
        async content(event, trigger, player) {
          ...
          const markedCards = player.getCards("h", (card) =>
            card.hasGaintag("clanshenjun"),
          )
          const list = []
          const names = []
          ...
          result = await player
            .chooseButton([
              `是否将${get.cnNumber(markedCards.length)}张牌当下列一张牌使用？`,
              [list, "vcard"],
            ])
            .set("ai", (button) => {
              return get.event().player.getUseValue({
  ...
  clanbalong: {
    ...
    filter(event, player) {
      ...
      return (
        list[0][0] === "trick" && (list.length === 1 || list[0][1] > list[1][1])
      )
    },
    async content(event, trigger, player) {
  ...
  clanyunshen: {
    ...
    async content(event, trigger, player) {
      ...
      const players =
        result.control === "选项二" ? [player, target] : [target, player]
  ...
  clandianzhan: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        player.hasCard(
          (card) =>
            get.suit(card) === get.suit(event.card) && player.canRecast(card),
          "h",
        )
      )
    },
    async content(event, trigger, player) {
      ...
      const cards = player.getCards(
        "h",
        (card) =>
          get.suit(card) === get.suit(trigger.card) && player.canRecast(card),
      )
      if (cards.length > 0) {
        await player.recast(cards)
        recasted = true
      ...
      count: {
        ...
        filter(event, player) {
          ...
          return (
            ...
            !player.getStorage("clandianzhan").includes(suit)
          )
        },
        forced: true,
        silent: true,
    ...
    filter(event, player) {
      return (
        ...
        player
          .getHistory("useCard", (evt) => {
            return (
              get.type(evt.card, null, false) === "trick" &&
      ...
      if (!skills.length) {
        ...
        result = await player
          ...
          .set(
            "removeSkillCheck",
            player.hp < 2 &&
              !player.countCards("hs", (card) => get.tag(card, "save")),
          )
          .forResult()
      }
      if (result?.bool && result?.links?.length) {
  ...
  clanyirong: {
    ...
    check(card) {
      ...
      if (
        player.countCards(
          "h",
          (card) => lib.skill.clanyirong.checkx(card) > 0,
        ) +
          1 <
