...
const skills = {
  ...
  huanfu: {
    ...
    ai: {
      effect: {
        target_use(card, _player, target, current) {
          if (
            card.name === "sha" &&
            target.hp > 0 &&
            current < 0 &&
    ...
    content() {
      ...
      player
        .chooseTarget(
          get.prompt("zeyue"),
          "令一名可选角色的一个非锁定技失效",
          (_card, _player, target) => _status.event.sources.includes(target),
        )
        .set("sources", sources)
        .set("ai", (target) => {
    ...
    content() {
      ...
      player
        .chooseTarget(
          ...
          (_card, _player, target) => target.isDamaged(),
        )
        .set("ai", (target) => {
  ...
  jujian: {
    ...
    filter(_event, player) {
      return game.hasPlayer((current) => {
        return (
          player.hasZhuSkill("jujian", current) &&
          current.group === "wei" &&
          current !== player
        )
      })
    ...
    filterTarget(_, player, target) {
      return (
        player.hasZhuSkill("jujian", target) &&
        target.group === "wei" &&
        target !== player
      )
    },
    ...
    subSkill: {
      forbid: {
        ...
        ai: {
          effect: {
            player(card, player, target, _current) {
              if (
                get.type(card) === "trick" &&
    ...
    content() {
      ...
      if (
        player.hasHistory("useCard", (evt) => {
          return (
            evt.getParent(2).name === "jianzheng" &&
  ...
  zili: {
    ...
    async content(event, _trigger, player) {
      ...
      await player.chooseDrawRecover(2, true, (_event, player) => {
        ...
        .chooseTarget(
          ...
          (_card, player, target) =>
            ...
            (!player.storage.xianfu2 ||
              !player.storage.xianfu2.includes(target)),
        )
        .set("ai", (target) => {
  ...
  jianmie: {
    ...
    chooseControl(player, targets, eventId) {
      ...
      return player
        ...
        .set(
          "prompt2",
          `弃置选择颜色的手牌，然后若你/${str}弃置的牌更多，则你/${str}视为对${str}/你使用【决斗】`,
        )
        .set("ai", () => {
