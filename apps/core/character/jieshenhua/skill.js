...
chanyuan: {
	...
	zhengrong: {
		...
		async content(event, trigger, player) {
			...
			player.addToExpansion(card, target, "give").gaintag.add("zhengrong");
		},
		marktext: "荣",
		onremove(player, skill) {
	...
	hongju: {
		...
		derivation: "qingce",
		filter(event, player) {
			return player.getExpansions("zhengrong").length >= 3 && game.dead.length > 0;
		},
		content() {
			"step 0";
			player.awakenSkill(event.name);
			player.draw(player.getExpansions("zhengrong").length);
			"step 1";
			if (player.countCards("h") == 0) {
				...
				next.set("filterButton", function (button) {
					var ss = _status.event.player.getExpansions("zhengrong");
			...
			if (result.bool) {
				var gains = [];
				var pushs = [];
				var expansions = player.getExpansions("zhengrong");
				...
				player.addToExpansion(pushs, player, "give").gaintag.add("zhengrong");
	...
	qingce: {
		...
		chooseButton: {
			dialog(event, player) {
				return ui.create.dialog("请选择要移去的「荣」", player.getExpansions("zhengrong"), "hidden");
			},
			backup(links, player) {
				return {
					card: links[0],
