...
const skills = {
  jx_buqu: {
    ...
    mod: {
      maxHandcardBase(player, _num) {
        if (
          get.mode() !== "guozhan" &&
          player.getExpansions("jx_buqu").length
        ) {
      ...
      effect: {
        target(card, _player, target) {
          if (get.tag(card, "damage") || get.tag(card, "loseHp")) {
            const num = target.getExpansions("jx_buqu").length || target.getHp()
            return (num + 1) / 5
          }
        },
      },
        ...
        return {
          ...
          ai1(card) {
            ...
            if (enemyNum) {
              ...
              if (lib.skill.jx_guhuo_backup.aiUse < 0.5 && !player.isDying()) {
                return 0
              }
            }
  ...
  guhuo_guess: {
    ...
    filter(event, _player) {
      return (
        event.skill &&
        (event.skill.indexOf("guhuo_") === 0 ||
          event.skill.indexOf("jx_guhuo_") === 0)
      )
    },
      ...
      viewas: {
        ...
        viewAsFilter(player) {
          return player.hasCard(
            (card) => lib.skill.jx_shuangxiong_viewas.filterCard(card, player),
            "hs",
          )
        },
        filterCard(card, player) {
        ...
        prompt() {
          const colors = _status.event.player.getStorage(
            "jx_shuangxiong_viewas",
          )
          let str = "将一张颜色"
    ...
    subSkill: {
      ...
      count: {
        ...
        async content(_event, trigger, player) {
          ...
          player.storage.jx_luanji.addArray(
      ...
      damage: {
        ...
        filter(event, player) {
          return (
            player.storage.jx_luanji2 &&
            event.card === player.storage.jx_luanji2
          )
        },
        async content(_event, _trigger, player) {
          delete player.storage.jx_luanji2
    ...
    subSkill: {
      effect: {
        ...
        mod: {
          ...
          targetInRange(card, _player, _bool) {
            if (get.name(card) === "sha") {
              return true
            }
          },
          selectTarget(card, _player, range) {
            if (get.name(card) === "sha") {
              range[1]++
            }
          },
      ...
      diseffect: {
        ...
        ai: {
          effect: {
            player_use(_card, _player, _target) {
              return [0, 0, 0, 2]
            },
          },
        },
          ...
          if (minList.length === 1) {
            ...
            targets = (
              await player
                .chooseTarget(
                  true,
                  `好施：选择一名手牌最少的角色获得${get.translation(max)}的一半手牌（向下取整）`,
                )
                .set("filterTarget", (_, _player, target) =>
  ...
  jx_beige: {
    ...
    async content(_event, trigger, player) {
      ...
      const next = player.chooseToDiscard(
        "he",
        get.prompt2("jx_beige", trigger.player),
      )
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("ai", () => {
          ...
          const skills = player.storage.jx_huashen.character.flatMap(
            (i) => get.character(i).skills,
          )
          skills.randomSort()
          skills.sort((a, b) => get.skillRank(b, cond) - get.skillRank(a, cond))
          if (
            skills[0] === player.storage.jx_huashen.current2 ||
      ...
      while (true) {
        ...
        if (choice === "制衡其他化身") {
          ...
          next.set("ai", (button) => {
            ...
            return player.storage.jx_huashen.map[button.link].includes(
              skills[0],
            )
              ? 2.5
              : 1 + Math.random()
  ...
  jx_qiaobian: {
    ...
    async cost(event, _trigger, player) {
      ...
      event.result = await player
        .chooseTarget(
          get.prompt2(event.skill),
          (_, _player, target) => target !== get.event().lastTarget,
        )
        .set(
          "ai",
          (target) =>
        ...
        init(player, skill) {
          ...
          if (target) {
            player.markSkillCharacter(
              ...
              target,
              "巧变",
              `本轮指定${get.translation(target)}为目标`,
            )
          }
        },
        onremove: true,
  ...
  juzhan: {
    ...
    marktext: "☯",
    intro: {
      content(storage, _player, _skill) {
        if (storage) {
    ...
    subSkill: {
      effect: {
        ...
        onremove: true,
        mod: {
          playerEnabled(_card, player, target) {
  ...
  kuizhu: {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("ai", () => {
          ...
          if (
            game.countPlayer(
              (current) =>
                get.attitude(player, current) < 0 && current.hp === num,
            ) > 0 &&
            num <= 3
          ) {
            return 1
      ...
      if (result.index === 1) {
        ...
        const next = player.chooseTarget("请选择〖溃诛〗摸牌的目标", [
          1,
          event.num,
        ])
        next.ai = (target) => {
  ...
  lijun1: {
    ...
    filter(event, player) {
      ...
      if (
        !game.hasPlayer(
          (target) => player !== target && target.hasZhuSkill("lijun", player),
        )
      ) {
        return false
      }
    ...
    async cost(event, trigger, player) {
      ...
      const next = player.chooseTarget(
        ...
        (_card, player, target) =>
          player !== target && target.hasZhuSkill("lijun", player),
      )
      next.ai = (target) => get.attitude(_status.event.player, target)
      event.result = await next.forResult()
    },
    async content(event, trigger, player) {
      ...
      const list = trigger.cards.filter(
  ...
  jianxiang: {
    ...
    async cost(event, _trigger, player) {
      const next = player.chooseTarget(
        get.prompt(event.skill),
        "令场上手牌数最少的一名角色摸一张牌",
        (_card, _player, target) => target.isMinHandcard(),
      )
      next.ai = (target) => {
        const player = get.player()
        return get.attitude(player, target)
  ...
  shenshi: {
    ...
    prompt:
      "出牌阶段限一次，你可以将一张牌交给一名手牌数最多的角色，然后对其造成1点伤害，若该角色因此死亡，则你可以令一名角色将手牌摸至四张",
    async content(event, trigger, player) {
      const target = event.targets[0]
  ...
  shicai: {
    ...
    mod: {
      aiOrder(player, card, num) {
        if (
          num <= 0 ||
          player.shicai_aiOrder ||
          get.itemtype(card) !== "card" ||
          player.hasSkillTag("abnormalDraw")
        ) {
          return num
        }
        ...
        const val = player.getUseValue(card, true, true)
      ...
      if (get.type(event.card) === "equip") {
        ...
        return (
          player.countCards(
            "h",
            (card) =>
              get.subtype(card) === subtype && player.getUseValue(card) >= eff1,
          ) > 0
        )
      }
      return true
  ...
  qingce: {
    ...
    filter(_event, player) {
      return (
        player.getExpansions("zhengrong").length > 0 &&
        player.countCards("h") > 0
      )
    },
    chooseButton: {
  ...
  wanglie: {
    ...
    locked: false,
    mod: {
      targetInRange(_card, player, _target) {
  ...
  zuilun: {
    ...
    async content(_event, _trigger, player) {
      ...
      if (num < 3) {
        ...
        chooseToMove.set("filterMove", (_from, to, moved) => {
  ...
  qianjie: {
    ...
    subSkill: {
      ...
      2: {
        mod: {
          targetEnabled(card, _player, _target) {
  ...
  jueyan2: {
    mod: {
      targetInRange(_card, _player, _target, _now) {
        return true
      },
    },
    mark: true,
  ...
  xiongluan: {
    ...
    subSkill: {
      effect: {
        ...
        intro: {
          content: "本回合对$使用牌无距离和次数限制且其不能使用和打出手牌",
        },
      },
      ban: {
        charlotte: true,
  ...
  zhengu: {
    ...
    async cost(event, _trigger, player) {
      event.result = await player
        .chooseTarget(get.prompt2(event.skill), (_card, player, target) => {
  ...
  zhengu2: {
    ...
    sourceSkill: "zhengu",
    filter(event, player) {
      return event.player.storage.zhengu_mark?.includes(player)
    },
    async content(_event, trigger, player) {
    ...
    subSkill: {
      ...
      2: {
        ...
        async cost(event, _trigger, player) {
          ...
          next.set(
            "filterTarget",
            (_card, player, target) => target !== player && !target.isLinked(),
          )
  ...
  g_jieying: {
    mod: {
      maxHandcard(player, num) {
        if (
          game.countPlayer((current) => current.hasSkill("jieying")) > 0 &&
          player.isLinked()
        ) {
          return num + 2
        }
  ...
  duorui: {
    ...
    async content(event, trigger, player) {
      ...
      if (player.hasEnabledSlot()) {
        ...
        next.set("ai", (_event, _player, list) => {
    ...
    subSkill: {
      ...
      2: {
        ...
        async cost(event, _trigger, player) {
          ...
          const filterTarget = (_card, player, target) =>
            target !== player && !target.hasMark("ljieying_mark")
      ...
      3: {
        ...
        filter(event, player) {
          return (
            player !== event.player &&
            event.player.hasMark("ljieying_mark") &&
            event.player.isIn()
          )
        },
        forced: true,
        ...
        mod: {
          cardUsable(card, player, num) {
            if (player.hasMark("ljieying_mark") && card.name === "sha") {
              return (
                num +
                game.countPlayer((current) => current.hasSkill("ljieying"))
              )
            }
          },
          maxHandcard(player, num) {
          ...
          aiOrder(player, _card, num) {
            if (
              player.hasMark("ljieying_mark") &&
              game.hasPlayer((current) => {
                return (
                  current.hasSkill("ljieying") &&
                  current !== player &&
                  get.attitude(player, current) <= 0
                )
              })
