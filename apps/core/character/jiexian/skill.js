...
const skills = {
    ...
    filter(_event, player) {
      return (
        ...
        game.hasPlayer((current) =>
          get.info("jx_rende").filterTarget(null, player, current),
        )
      )
    },
  ...
  jx_paoxiao: {
    ...
    filter(event, player) {
      return (
        event.card.name === "sha" &&
        (!event.audioed || !player.hasSkill("jx_paoxiao2"))
      )
    },
    async content(_event, trigger, player) {
  ...
  jx_zhiheng: {
    ...
    subSkill: {
      draw: {
        ...
        filter(event, player) {
          if (
            event.getParent(2).skill !== "jx_zhiheng" &&
  ...
  jx_fanjian: {
    ...
    async content(event, _trigger, player) {
      ...
      // step 0
      target.storage.jx_fanjian = cards[0]
      await player.give(cards[0], target)

      // step 1
      if (!target.countCards("h")) {
        result = { control: "jx_fanjian_hp" }
      } else {
        result = await target
          .chooseControl("jx_fanjian_card", "jx_fanjian_hp")
          .set("ai", (_event, player) => {
            var cards = player.getCards("he", {
              suit: get.suit(player.storage.jx_fanjian),
            })
            if (cards.length === 1) {
              return 0
            }
      ...
      // step 2
      if (result.control === "jx_fanjian_card") {
      ...
      // step 3
      const suit = get.suit(target.storage.jx_fanjian)
      await target.discard(
        target.getCards(
          "he",
          (i) =>
            get.suit(i) === suit &&
            lib.filter.cardDiscardable(i, target, "jx_fanjian"),
        ),
      )
      delete target.storage.jx_fanjian
    },
    ai: {
      order: 9,
      result: {
  ...
  jx_guose: {
    ...
    filterTarget(_card, player, target) {
      if (
        ...
        lib.filter.cardDiscardable(ui.selected.cards[0], player, "jx_guose") &&
  ...
  jx_qianxun: {
    ...
    async content(_event, _trigger, player) {
      ...
      next.gaintag.add("jx_qianxun2")
      await next
      player.addSkill("jx_qianxun2")
    },
    ai: {
      effect: {
        target(card, player, target) {
  ...
  jx_qianxun2: {
    ...
    forced: true,
    audio: false,
    sourceSkill: "jx_qianxun",
    async content(_event, _trigger, player) {
      const cards = player.getExpansions("jx_qianxun2")
      if (cards.length) {
        await player.gain(cards, "draw")
      }
      player.removeSkill("jx_qianxun2")
    },
    intro: {
      mark(dialog, _storage, player) {
        var cards = player.getExpansions("jx_qianxun2")
  ...
  jx_lianying: {
    ...
    async content(_event, trigger, player) {
      ...
      player.logSkill("jx_lianying", result.targets)
  ...
  jx_tuxi: {
    ...
    async content(_event, trigger, player) {
      ...
      result = await player
        ...
        .setHiddenSkill("jx_tuxi")
        .forResult()

      // step 1
      if (result.bool) {
        result.targets.sortBySeat()
        player.logSkill("jx_tuxi", result.targets)
  ...
  jx_luoyi: {
    ...
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        player.addTempSkill("jx_luoyi_buff", { player: "phaseBeforeStart" })
        trigger.changeToZero()
      }
    },
    subSkill: { buff: { inherit: "jx_luoyi2", sourceSkill: "jx_luoyi" } },
  },
  jx_luoyi2: {
    trigger: { source: "damageBegin1" },
    filter(event) {
      ...
      while (event.num > 0) {
        result = await player
          .chooseCardTarget({
            filterCard(card) {
              return (
                get.itemtype(card) === "card" && !card.hasGaintag("jx_yiji_tag")
              )
            },
            filterTarget: lib.filter.notMe,
        ...
        if (result.bool) {
          ...
          player.addGaintag(res, "jx_yiji_tag")
  ...
  jx_qingguo: {
    mod: {
      ...
      aiUseful() {
        return lib.skill.jx_qingguo.mod.aiValue.apply(this, arguments)
      },
    },
    locked: false,
    audio: 2,
  ...
  jiyuan: {
    ...
    filter(event, player, _triggername, target) {
      if (!target.isIn()) {
        return false
      }
      if (event.name === "dying") {
    ...
    logTarget(_event, _player, _triggername, target) {
      return target
    },
    check(_event, player, _triggername, target) {
  ...
  jiangchi: {
    ...
    direct: true,
    logAudio: (index) =>
      typeof index === "number" ? `jiangchi${index}.mp3` : 2,
    content() {
      "step 0"
      var list = ["摸牌", "弃牌", "取消"]
      ...
      ;("step 1")
      player.logSkill("jiangchi", null, null, null, [
        result.control === "弃牌" ? 1 : 2,
      ])
