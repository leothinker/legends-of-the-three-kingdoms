...
const skills = {
      ...
      player
        ...
        .set("ai", (target) => {
          ...
          return target.hasCard((card) => {
            return (
              (lib.filter.canBeDiscarded(card, player, target) && get.value(card, target) > 3) ||
        ...
        content() {
          ...
          game.checkGlobalHistory("cardMove", (evt) => {
            if (
              ...
              cards.addArray(
                evt.cards.filter((i) => get.position(i, true) == "d" && !idList.includes(i.cardid)),
              )
            }
          })
      ...
      trigger: {
        ...
        content() {
          ...
          if (get.info(trigger.skill).silent) {
            ...
            next.set(
              "yes",
              !info.check || info.check(trigger, player, trigger.triggername, trigger.indexedData),
            )
            ...
            if (typeof info.prompt2 == "function") {
              next.set(
                "prompt2",
                info.prompt2(trigger, player, trigger.triggername, trigger.indexedData),
              )
  ...
  jsrgxiongbao: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseCard(get.prompt(event.skill), 2, "本次议事你展示两张手牌")
        .forResult()
    },
    async content(event, trigger, player) {
