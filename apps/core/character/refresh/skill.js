...
const skills = {
  ...
  jx_rende: {
    ...
    filter(event, player) {
      return (
        player.countCards("h") &&
        game.hasPlayer((current) => get.info("jx_rende").filterTarget(null, player, current))
      )
    },
    ...
    async content(event, trigger, player) {
      ...
      if (!target.countCards("h")) {
        ...
        result = await target
          ...
          .set("ai", function (event, player) {
            var cards = player.getCards("he", { suit: get.suit(player.storage.jx_fanjian) })
            if (cards.length == 1) {
              return 0
            }
            if (cards.length >= 2) {
      ...
      await target.discard(
        target.getCards("he", function (i) {
          return get.suit(i) == suit && lib.filter.cardDiscardable(i, target, "jx_fanjian")
        }),
      )
    ...
    async content(event, trigger, player) {
      ...
      while (event.num > 0) {
        result = await player
          .chooseCardTarget({
            filterCard(card) {
              return get.itemtype(card) == "card" && !card.hasGaintag("jx_yiji_tag")
            },
            filterTarget: lib.filter.notMe,
  ...
  liyu: {
    ...
    async content(event, trigger, player) {
      const gainResult = await player
        .gainPlayerCard(get.prompt("liyu", trigger.player), trigger.player, "hej", "visibleMove")
