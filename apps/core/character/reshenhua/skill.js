...
const skills = {
  ...
  jx_guhuo: {
    ...
    hiddenCard(player, name) {
      return (
        lib.inpile.includes(name) && player.countCards("hs") > 0 && !player.hasSkill("guhuo_phase")
      )
    },
    filter(event, player) {
  ...
  guhuo_guess: {
    ...
    filter(event, player) {
      return (
        event.skill && (event.skill.indexOf("guhuo_") == 0 || event.skill.indexOf("jx_guhuo_") == 0)
      )
    },
    async content(event, trigger, player) {
          ...
          if (minList.length == 1) {
            ...
            targets = (
              await player
                ...
                .set(
                  "ai",
                  (target) => get.attitude(get.player(), target) * (target.getDamagedHp() + 1),
                )
                .forResult()
            ).targets
          }
  ...
  jx_weimu: {
    ...
    subSkill: {
      effect: {
        ...
        filterCard(card) {
          return (
            get.color(card) == "black" && get.type(card) != "trick" && get.type(card) != "delay"
          )
        },
        position: "hes",
        check(card) {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("ai", () => {
          ...
          let skills = player.storage.jx_huashen.character
      ...
      while (true) {
        ...
        if (choice === "制衡其他化身") {
          ...
          next.set("ai", (button) => {
            ...
            skills.randomSort()
            skills.sort((a, b) => get.skillRank(b, cond) - get.skillRank(a, cond))
            return player.storage.jx_huashen.map[button.link].includes(skills[0])
              ? 2.5
              : 1 + Math.random()
          })
        ...
        async content(event, trigger, player) {
          ...
          if (trigger.name == "phaseUse") {
            ...
            const result = await trigger.player
              ...
              .set("ai", (target) =>
                get.effect(target, { name: "shunshou_copy2" }, get.player(), get.player()),
              )
              .forResult()
  ...
  shenshi: {
    ...
    filter(event, player) {
      ...
      if (event.name == "chooseToUse") {
        return (
          ...
          game.hasPlayer((current) => get.info("shenshi").filterTarget(null, player, current))
        )
      }
      return (
        event.source?.isIn() &&
    ...
    filterTarget(card, player, target) {
      return (
        target != player &&
        !game.hasPlayer(
          (current) => current != player && current.countCards("h") > target.countCards("h"),
        )
      )
    },
    check(card) {
      ...
      if (bool) {
        ...
        player.addTip(
          ...
          get.translation(effect) +
            player.getStorage(effect).reduce((str, suit) => str + get.translation(suit), ""),
        )
      }
    },
    ai: {
    ...
    async content(event, trigger, player) {
      ...
      if (cards.length > 1) {
        const result = await player
          ...
          .set(
            "reverse",
            _status.currentPhase?.next && get.attitude(player, _status.currentPhase.next) > 0,
          )
  ...
  mingren: {
    ...
    filter(event, player) {
      return (
        (event.name != "phase" || game.phaseNumber == 0) && !player.getExpansions("mingren").length
      )
    },
    async content(event, trigger, player) {
  ...
  congjian: {
    ...
    filter(event, player) {
      return (
        get.type(event.card) == "trick" && event.targets.length > 1 && player.countCards("he") > 0
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
    ...
    async content(event, trigger, player) {
      ...
      function filterButton(button) {
        ...
        if (
          ...
          !lib.filter.canBeDiscarded(button.link, get.owner(button.link), player)
        ) {
          return false
        }
  ...
  ljieying: {
    ...
    subSkill: {
          ...
          effect: {
            player(card, player, target) {
              if (get.name(card) === "lebu" && get.attitude(player, target) < 0) {
                return (
                  1 +
                  Math.min((target.countCards("h") + 1.5) * 0.8, target.getHandcardLimit() * 0.7)
                )
              }
            },
          },
      ...
      3: {
        ...
        filter(event, player) {
          return (
            player != event.player && event.player.hasMark("ljieying_mark") && event.player.isIn()
          )
        },
        forced: true,
        logTarget: "player",
