...
const dynamicTranslates = {
  ...
  sbjiang(player) {
    ...
    if (player.countMark("sbjiang")) {
      str +=
        "X次。你可以将所有手牌当【决斗】使用（X为场上其他吴势力角色数+1）。"
    } else {
  ...
  sbzhenliang(player) {
    ...
    let yang =
        "出牌阶段限一次，你可以弃置X张与“任”颜色相同的牌并对攻击范围内的一名角色造成1点伤害（X为你与其体力值值差且X至少为1）",
      yin =
        "你的回合外，一名角色使用或打出牌结算完成后，若此牌与“任”类别相同，则你可以令至多两名角色各摸两张牌"
    if (bool) {
  ...
  sbwansha(player) {
    ...
    var str =
      "①你的回合内，不处于濒死状态的其他角色不能使用【桃】。②每轮限两次，一名角色进入濒死状态时，你可以观看其手牌并选择其"
    str += storage ? "区域内的" : "中"
    str +=
      "零至三张牌（其他角色不可见），然后其选择一项：1.你将这些牌分配给任意名不为其的角色；2.其弃置除这些牌以外的牌。"
    return str
  },
  sbweimu(player) {
