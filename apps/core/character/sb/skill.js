...
const skills = {
  ...
  sbdengxian: {
    ...
    filter(event, player) {
      ...
      return get.inpileVCardList((info) => {
        if (
          info[0] !== "basic" ||
          player.getStorage("sbdengxian_round").includes(info[2])
        ) {
          return false
        }
              ...
              .then(async (event, trigger, player) => {
                ...
                if (bool1 && bool2) {
                  ...
                  if (cards.some((card) => player.canEquip(card, true))) {
                    const card = cards
                      .filter((card) => player.canEquip(card, true))
                      .randomGet()
    ...
    hiddenCard(player, name) {
      if (
        ...
        player.getStorage("sbdengxian_round").includes(name)
      ) {
        return false
      }
    ...
    ai: {
      ...
      skillTagFilter(player, tag, arg) {
        ...
        if (
          tag === "respondSha" &&
          player.getStorage("sbdengxian_round").includes("sha")
        ) {
          return false
        }
        if (
          tag === "respondShan" &&
          player.getStorage("sbdengxian_round").includes("shan")
        ) {
          return false
        }
      },
  ...
  sbzhejian: {
    ...
    filterCard(card, player) {
      return (
        get.type(card) === "basic" &&
        lib.filter.cardDiscardable(card, player, "sbzhejian")
      )
    },
    selectCard: -1,
    filterTarget: true,
    ...
    subSkill: {
      ...
      effect: {
        ...
        filter(event, player) {
          if (event.name === "dying") {
            return (
              player.getStorage("sbzhejian_effect").includes(event.player) &&
              event.reason.getParent()?.name === "sbzhejian_effect"
            )
          }
          if (
            !player.getStorage("sbzhejian_effect").includes(event.target) ||
            player.hasSkill("sbzhejian_used")
          ) {
            return false
          }
          const target = game.findPlayer(
            (current) => current.getSeatNum() === 1,
          )
  ...
  sbzhenwei: {
    ...
    async content(event, trigger, player) {
      ...
      const others = player
        ...
        .filter((i) => ![player, target].includes(i))
      const map = await game
        .chooseAnyOL(
          [player, target, ...others],
          get.info(event.name).chooseToDiscard,
          [player, target],
        )
        .forResult()
      const cards1 = [],
    ...
    map: {
      damage: [
        "对其造成1点伤害",
        (player, target) => get.damageEffect(target, player, player),
  ...
  sbheyuan: {
    ...
    filter(event, player) {
      ...
      return (
        game.hasPlayer(
          (target) =>
            !player.getStorage("sbheyuan").includes(target) &&
            target.isDamaged(),
      ...
      event.result = await player
        .chooseCardTarget({
          ...
          ai2(target) {
            ...
            if (
              !num ||
              (num < 3 && player.hasCard((card) => 6 - get.value(card), "he"))
            ) {
              return eff
            }
            return 0
  ...
  sbqiaobian: {
    getNum(player) {
      return ["h", "e", "j"].filter((position) => {
        ...
        if (position === "e") {
          ...
          if (
            player.isDamaged() &&
            cards.some((card) => card.name === "baiyin") &&
    ...
    choice: {
      ...
      phaseUse(player, bool = false, numx = 0) {
        if (player.canMoveCard(true)) {
          if (bool) {
            ...
            if (
              max === 1 &&
              player.countCards("h", (card) => player.hasValueTarget(card)) > 2
            ) {
              return 0
            }
      ...
      phaseDiscard(player, bool = false, numx = 0) {
        if (
          player.countCards("h") >= game.players.length - 1 ||
    ...
    async cost(event, trigger, player) {
      const num1 = ["phaseDraw", "phaseUse", "phaseDiscard"].filter(
        (phase) => get.info(event.skill).choice[phase](player, true) > 0,
      ).length
      ...
      event.result = await player
        ...
        .set("filterButton", (button) => {
          ...
          return !ui.selected.buttons
            .reduce((list, cardx) => list.add(get.position(cardx.link)), [])
            .includes(get.position(card))
        })
        .set("ai", (button) => {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseButton(
          [
            `###巧变###<div class='text center'>请选择要跳过的${get.cnNumber(num)}个阶段</div>`,
            [list.map((phase) => [phase, get.translation(phase)]), "tdnodes"],
          ],
          true,
          num,
        )
        ...
        .set("numx", num)
        .forResult()
      for (const phase of list.filter((phase) =>
        result.links?.includes(phase),
      )) {
        player.skip(phase)
      ...
      phaseDraw: {
        ...
        filter(event, player) {
          return game.hasPlayer(
            (target) =>
              target !== player && target.hasGainableCards(player, "h"),
          )
        },
        async cost(event, trigger, player) {
          event.result = await player
            ...
            .set("ai", (target) =>
              get.effect(
                target,
                { name: "shunshou_copy2", position: "h" },
                get.player(),
                get.player(),
              ),
            )
          ...
          if (result?.targets?.length) {
            ...
            if (num === player.countCards("he")) {
              const result = await player
                ...
                .set(
                  "choice",
                  get.effect(target, { name: "draw" }, player, player) > 0,
                )
                .forResult()
              if (result?.bool) {
  ...
  sbliaoshi: {
    ...
    hiddenCard(player, name) {
      return (
        ...
        player.hasCards("he", (card) => card.hasGaintag("sbqiaobian")) &&
    ...
    filter(event, player) {
      return (
        get.info("sbliaoshi").getList(event, player).length &&
        player.hasCards("he", (card) => card.hasGaintag("sbqiaobian"))
      )
    },
    chooseButton: {
    ...
    async cost(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("filterButton", (button) => {
          ...
          return button.link
            .split("|")
            .every((i) => !player.skipList.includes(i))
        })
        .set("ai", (button) => {
          const player = get.player(),
            item = button.link,
            ...
            [
              ...
              -player
                ...
                .reduce(
                  (sum, card) =>
                    sum +
                    get.effect(player, { name: card.name }, player, player),
                  0,
                ),
            ],
            [
              "phaseDraw",
              -get.effect(player, { name: "wuzhong" }, player, player),
            ],
            [
              "phaseUse",
              -player
                .getCards("hs", (card) => player.hasValueTarget(card))
                .reduce((sum, card) => sum + player.getUseValue(card), 0),
            ],
      ...
      event.result = {
        bool: result?.bool,
        cost_data: list.filter((item) => (result?.links ?? []).includes(item)),
      }
    },
    async content(event, trigger, player) {
      const cost_data = event.cost_data.flatMap((item) => item.split("|"))
    ...
    filter(event, player) {
      ...
      return (
        player
          ...
          .reduce((sum, evt) => sum + evt.num, 0) >= player.getHp()
      )
    },
    frequent: true,
    content() {
  ...
  sbjingce: {
    ...
    locked: true,
    logAudio: (index) =>
      typeof index === "number" ? `sbjingce${index}.mp3` : 2,
    async cost(event, trigger, player) {
            ...
            while (true) {
              ...
              if (arr[0] === "none") {
                ...
                player.logSkill(
                  ...
                  null,
                  null,
                  player.storage.sbjingceCount >= 6
                    ? [get.rand(7, 8)]
                    : [get.rand(3, 4)],
                )
            ...
            if (storage.has(card)) {
              if (storage.get(card)[0] === trigger.player) {
                ...
                player.logSkill(
                  ...
                  null,
                  null,
                  player.storage.sbjingceCount >= 7 ? [9] : [get.rand(5, 6)],
                )
                player.storage.sbjingceCount = 0
              }
              storage.delete(card)
  ...
  sbtuxi: {
    ...
    filter(event, player) {
      if (
        ...
        event.getParent("sbtuxi").player === player
      ) {
        return false
      }
    ...
    async content(event, trigger, player) {
      ...
      if (
        !game.hasPlayer(
          (current) =>
            player !== current && current.countGainableCards(player, "h"),
        )
      ) {
        return
      }
      ...
      const { bool, targets, links } = await player
        .chooseButtonTarget({
          ...
          ai2(target) {
            ...
            if (
              (es.some((card) => get.equipValue(card, target) <= 4) &&
                att > 0) ||
      ...
      if (["equip", "all"].includes(control) && target.countCards("e")) {
        const { cards } = await player
          .choosePlayerCard(
            ...
            true,
            "e",
            `选择${get.translation(target)}的至多两张装备牌令其获得之`,
            [1, 2],
          )
          .forResult()
        if (cards?.length) {
          ...
          .set("ai", (card) => {
            ...
            const list = get.inpileVCardList((info) => {
              ...
              return (
                ...
                (get.info(cardx).notarget &&
                  lib.filter.cardEnabled(cardx, player))
              )
            })
            if (!list.length) {
              return 0
    ...
    async content(event, trigger, player) {
      ...
      if (storage) {
        ...
        if (suits.some((suit) => !record.includes(suit))) {
          ...
          lib.skill[skillName].subSkill.record.init(
            player,
            `${skillName}_record`,
          )
        }
        const list = get.inpileVCardList((info) => {
          ...
          return (
            ...
            (get.info(card).notarget && lib.filter.cardEnabled(card, player))
          )
        })
        if (!list.length) {
          return
        ...
        init(player, skill) {
          ...
          player.addTip(
            ...
            get.translation(skill) +
              info.reduce((str, suit) => str + get.translation(suit), ""),
          )
        },
        onremove(player, skill) {
  ...
  sbyiji: {
    ...
    async content(event, trigger, player) {
      ...
      let num =
        name === "damage" || !["identity", "doudizhu"].includes(mode) ? 2 : 1
      ...
      if (
        ...
        !player.hasCard((card) => !yiji || card.hasGaintag("sbyiji"), "h")
      ) {
        return
      }
      ...
      while (
        ...
        const result = await player
          .chooseCardTarget({
            ...
            position: "eh".slice(
              -1 +
                (!["identity", "doudizhu"].includes(mode) && name === "dying"),
            ), //三若为，怎么若都为构思
          })
          .forResult()
  ...
  sbhuanshi: {
    ...
    async content(event, trigger, player) {
      ...
      if (result?.cards?.length) {
        cards = result.cards
        cardsx = player.getCards("s", (card) =>
          card.hasGaintag("sbhuanshi_tag"),
        )
        if (cardsx.length) {
          if (cards) {
      ...
      game.deleteFakeCards(cardsx)
      const bool =
        result?.cards?.length && !result.cards[0].hasGaintag("sbhuanshi_tag")
    ...
    ai: {
      effect: {
        player(card, player, target) {
          if (
            player === _status.currentPhase ||
            player.countMark("sbmingzhe_used") >= 2
          ) {
            return
          }
        ...
        target(card, player, target) {
          if (
            ...
            target.countMark("sbmingzhe_used") >= 2
          ) {
            return
          }
  ...
  sbwansha: {
    ...
    filter(event, player) {
      ...
      return (
        event.player.countCards(position) &&
        player.countMark("sbwansha_used") < 2
      )
    },
    check(event, player) {
    ...
    async content(event, trigger, player) {
      ...
      let result
      result = await player
        .choosePlayerCard(target, position, [0, num], true, prompt)
        .set("visible", true)
        .forResult()
      if (!result?.cards?.length) {
        return
      ...
      if (result?.index === 0 && cards.length) {
        ...
        while (
          ...
          if (result?.links?.length) {
            ...
            result = await player
              .chooseTarget(
                `选择获得${get.translation(gives)}的角色`,
                true,
                (card, player, target) => {
              ...
              .set("ai", (target) => {
                return (
                  get.attitude(get.event().player, target) *
                  get.sgn(get.sgn(get.event().goon) + 0.5)
                )
              })
              .set(
                "goon",
          ...
          function (card, player, target) {
            ...
            if (dist > 1) {
              if (
                game.hasPlayer(
                  (current) =>
                    current !== player && get.distance(player, current) < dist,
                )
              ) {
                return false
              }
    ...
    subSkill: {
      add: {
        ...
        async cost(event, trigger, player) {
          ...
          for (let i = 0; i < 2; i++) {
            if (
              !player.hasSkill(list[i], null, null, false) ||
              player.storage[list[i]]
            ) {
              choiceList[i] =
                `<span style="opacity:0.5;">${choiceList[i]}</span>`
            } else {
  ...
  sbweimu: {
    ...
    logAudio: (index) =>
      typeof index === "number" ? `sbweimu${index}.mp3` : 4,
    filter(event, player) {
      ...
      return Array.from(ui.discardPile.childNodes).some((card) =>
        get.info("sbweimu").filterCardx(card),
      ) //num <= 1 &&
    },
    filterCardx(card) {
      return (
        get.subtype(card) === "equip2" ||
        (get.type(card) === "trick" && get.color(card) === "black")
      )
    },
    forced: true,
    popup: false,
  ...
  sbyicong: {
      ...
      while (true) {
        ...
        result2 = await player
          ...
          .set("ai", () => {
            ...
            if (
              player.getHp() > 2 ||
              player.countCards("hsx", ["shan", "caochuan"]) >= 3
            ) {
              return 0
            }
            return 1
    ...
    async content(event, trigger, player) {
      ...
      while (
        cards.length < num &&
        cards.length +
          player.countCards("s", (card) => card.hasGaintag("sbyicong")) <
          4
      ) {
        const name = choice === 0 ? "sha" : "shan"
        const card = get.cardPile2(
          (card) => card.name === name && !cards.includes(card),
        )
        if (card) {
          cards.push(card)
        } else {
  ...
  sbqiaomeng: {
    ...
    async cost(event, trigger, player) {
      const list = ["cancel2"]
      const choiceList = [
        `弃置${get.translation(trigger.player)}区域里的一张牌，你摸一张牌`,
        `获得3点蓄力值`,
      ]
      if (player.countCharge(true)) {
        list.unshift("蓄力")
      ...
      if (
        trigger.player.isIn() &&
        trigger.player.countDiscardableCards(player, "hej")
      ) {
        list.unshift("弃牌")
      } else {
        choiceList[0] = `<span style="opacity:0.5">${choiceList[0]}</span>`
  ...
  sbxianzhen: {
      ...
      result: {
        target(player, target) {
          if (
            !player.countCards("hs", (card) => {
              return (
                get.name(card) === "sha" && player.canUse(card, target, false)
              )
            })
          ) {
            return -0.1
    ...
    subSkill: {
      attack: {
          ...
          if (result.bool) {
            ...
            if (!player.storage.sbxianzhen_damaged) {
              player.storage.sbxianzhen_damaged =
                (player.storage.sbxianzhen_damaged || 0) + 1
        ...
        intro: {
          content:
            "本阶段对$使用牌无距离限制，且使用杀指定其为目标后可以与其拼点",
        },
        mod: {
      ...
      decrease: {
        ...
        filter(event, player) {
          return event.getParent(
            (evt) => evt.name === "useCard" && evt.card === event.card,
          ).jiu
        },
  ...
  sbganglie: {
    ...
    async cost(event, trigger, player) {
      ...
      const list = [],
        choiceList = [
          `弃置你或${get.translation(target)}的一张牌`,
          `对${get.translation(target)}造成一点伤害`,
        ]
      if (
    ...
    async content(event, trigger, player) {
      ...
      const fun1 = async (target) => {
        ...
        if (target.countDiscardableCards(player, "e")) {
          dialog.push(
            `<div class="text center">${get.translation(target)}的装备牌</div>`,
          )
        ...
        const result = await player
          ...
          .set("ai", (button) => {
            ...
            return (
              get.value(button.link, get.owner(source)) *
              Math.sign(-get.attitude(player, source))
            )
          })
          .set("source", target)
          .forResult()
  ...
  sbqingjian: {
    ...
    filter(event, player) {
      ...
      return (
        event.type === "discard" &&
        event.getl?.(event.player)?.cards2?.length === 1
      )
    },
    logTarget: "player",
    async content(event, trigger, player) {
  ...
  sbquhu: {
    ...
    filter(event, player) {
      return (
        game.countPlayer((current) =>
          lib.skill.sbquhu.filterTarget(null, player, current),
        ) > 1
      )
    },
    filterTarget(card, player, target) {
  ...
  sbxingshang: {
    ...
    filter(event, player) {
      return get.info("sbxingshang").getList.some((effect) => {
        return (
          player.countMark("sbxingshang") >= effect.cost &&
      ...
      check(button) {
        ...
        return Math.max(
          ...game
            ...
            .map((target) => {
              game.broadcastAll(
                (effect) => (lib.skill.sbxingshang_aiSkill.ai = effect.ai),
                effect,
              )
              ...
              return Math.max(
                ...game
                  ...
                  .map((target) => {
                    game.broadcastAll(
                      (effect) =>
                        (lib.skill.sbxingshang_aiSkill.ai = effect.ai || {}),
                      effect,
                    )
                    return get.effect(
                      target,
    ...
    subSkill: {
      ...
      gain: {
        ...
        filter(event, player) {
          if (
            player.countMark("sbxingshang") >= get.info("sbxingshang").getLimit
          ) {
            return false
          }
          return (
            event.name === "die" ||
            !player.getHistory("custom", (evt) => evt.sbxingshang).length
          )
        },
        forced: true,
        locked: false,
    ...
    getList: [
      {
        ...
        filter: (player) =>
          game.hasPlayer(
            (target) =>
              target !== player &&
              !target.getStorage("sbfangzhu_ban").includes("basic"),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target !== player &&
      ...
      },
      {
        ...
        filter: (player) =>
          game.hasPlayer(
            (target) =>
              target !== player &&
              !target.getStorage("sbfangzhu_ban").includes("trick"),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target !== player &&
      ...
      },
      {
        ...
        filter: (player) =>
          ...
          game.hasPlayer(
            (target) =>
              target !== player &&
              !target.getStorage("sbfangzhu_ban").includes("equip"),
          ),
        filterTarget: {
          filterTarget: (card, player, target) =>
            target !== player &&
      ...
      ban: {
        ...
        init(player, skill) {
          ...
          if (storage.length) {
            player.addTip(
              skill,
              `放逐 限${storage.length === 1 ? get.translation(storage[0])[0] : "手牌"}`,
            )
          }
        },
        onremove(player, skill) {
    ...
    subSkill: {
      delete: {
        ...
        filter(event, player) {
          ...
          return game.hasPlayer((target) =>
            lib.skill.sbsongwei.subSkill.delete.filterTarget(
              null,
              player,
              target,
            ),
    ...
    async content(event, trigger, player) {
      var result = await player
        .chooseTarget(
          ...
          `选择一名非主公的其他角色，本阶段对其使用【杀】无距离和次数限制，使用【杀】指定其为目标后摸${get.mode() === "identity" ? "两" : "一"}张牌，对其使用三张【杀】后不能对其使用【杀】`,
          (card, player, target) => {
        ...
        chooseButton: {
          dialog(event, player) {
            ...
            for (var j of lib.inpile_nature) {
              if (
                event.filterCard(
                  get.autoViewAs({ name: "sha", nature: j }, "unsure"),
                  player,
                  event,
                )
              ) {
      ...
      effect: {
        ...
        mod: {
          targetInRange(card, player, target) {
            if (
              card.name === "sha" &&
              typeof player.storage.sbwusheng_effect[target.playerid] ===
                "number"
            ) {
              return true
            }
        ...
        filter(event, player) {
          ...
          return (
            typeof player.storage.sbwusheng_effect[event.target.playerid] ===
            "number"
          )
        },
        direct: true,
  ...
  sbyijue: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        !player.getStorage("sbyijue").includes(event.player)
      )
    },
    forced: true,
    logTarget: "player",
  ...
  sbqicai: {
    ...
    filterCardx(card, player) {
      ...
      return (
        ...
        game.hasPlayer(
          (target) =>
            target !== player && target.hasEmptySlot(get.subtype(card)),
        )
      )
    },
    usable: 1,
      ...
      backup(links, player) {
        return {
          ...
          filterTarget(card, player, target) {
            return (
              target !== player &&
              target.canEquip(lib.skill.sbqicai_backup.card)
            )
          },
          check: () => 1,
          discard: false,
        ...
        content() {
          ...
          if (
            ...
            target
              .chooseButton(
                [
                  `奇才：将其中${get.cnNumber(num)}张牌交给${get.translation(player)}`,
                  cards,
                ],
                num,
                true,
              ...
              .set(
                "ai",
                (button) => get.value(button.link) * get.sgn(_status.event.att),
              )
  ...
  sbhuoji: {
    ...
    subSkill: {
      fire: {
        ...
        filterTarget: lib.filter.notMe,
        prompt:
          "选择一名其他角色，对其与其势力相同的所有其他角色各造成1点火属性伤害",
        usable: 1,
        line: "fire",
        content() {
          "step 0"
      ...
      achieve: {
        ...
        filter(event, player) {
          return (
            player
              .getAllHistory(
                "sourceDamage",
                (evt) => evt.hasNature("fire") && evt.player !== player,
              )
        ...
        async content(event, trigger, player) {
          ...
          player.changeSkills(
            ["sbguanxing", "sbkongcheng"],
            ["sbhuoji", "sbkanpo", "twkanpo"],
          )
        },
      },
      fail: {
    ...
    subSkill: {
      kanpo: {
        ...
        filter(event, player) {
          return (
            event.player !== player &&
            player.storage.sbkanpo[1].includes(event.card.name)
          )
        },
        prompt2(event, player) {
  ...
  sbguanxing: {
    ...
    filter(event, player) {
      ...
      if (event.name === "phaseZhunbei") {
        return (
          bool ||
          7 - lib.skill.sbguanxing.getNum * player.countMark("sbguanxingx") > 0
        )
      }
  ...
  sbkongcheng: {
    ...
    filter(event, player, name) {
      ...
      const num = player.countCards("s", (card) =>
        card.hasGaintag("sbguanxing"),
      )
        ...
        player
          .judge((result) => {
            if (
              ...
              get
                .player()
                .countCards("s", (card) => card.hasGaintag("sbguanxing"))
            ) {
              return 2
            }
            return -1
  ...
  sbmingren: {
    ...
    filter(event, player) {
      if (event.name === "lose") {
        ...
        return (
          ...
          event.getlx !== false &&
          evt.xs?.some((card) =>
            evt.gaintag_map[card.cardid]?.includes("sbmingren"),
          )
        )
      }
      return event.name !== "phase" || game.phaseNumber === 0
    ...
    async content(event, trigger, player) {
      ...
      if (
        player.hasCards("h") &&
        game.hasPlayer((target) => !target.hasExpansions(event.name))
      ) {
        const result = await player
          .chooseCardTarget({
        ...
        if (cards?.length && targets?.length) {
          player.line(targets)
          await targets[0].addToExpansion({
            cards,
            source: player,
            animate: "giveAuto",
            gaintag: [event.name],
  ...
  sbweiliu: {
    ...
    filter(event, player) {
      ...
      const choices = ["gain", "damage"].removeArray(
        player.getStorage("sbweiliu_used"),
      )
    ...
    async cost(event, trigger, player) {
      ...
      const num =
        player.getAllHistory("useSkill", (evt) => evt.skill === event.skill)
          .length + 1
      const choiceList = [
        `弃置${get.cnNumber(num)}张${get.translation(color)}牌对一名角色造成一点伤害`,
        `从牌堆获得${get.cnNumber(num)}张${get.translation(color)}牌`,
      ]
      const choices = ["gain", "damage"].removeArray(
        player.getStorage("sbweiliu_used"),
      )
      ...
      if (result.control !== "cancel2") {
        event.result = {
          ...
          cost_data: {
            control: ["damage", "gain"][
              ["选项一", "选项二"].indexOf(result.control)
            ],
            num,
            color,
          },
    ...
    async content(event, trigger, player) {
      ...
      if (control === "damage") {
        ...
        const result = await player
          .chooseCardTarget({
            ...
            filterCard(card, player) {
              return (
                get.color(card) === get.event().color &&
                lib.filter.cardDiscardable(card, player, "sbweiliu")
              )
            },
        ...
        while (cards.length < num) {
          const card = get.cardPile2(
            (card) => get.color(card) === color && !cards.includes(card),
          )
          if (card) {
            cards.push(card)
          } else {
      ...
      player.addTip(
        `${name}_effect`,
        `卫旒 ${control === "damage" ? "摸" : "弃"}${num}${get.translation(color).slice(0, 1)}`,
      )
    },
    ai: {
      combo: "sbmingren",
    ...
    subSkill: {
      ...
      effect: {
        ...
        filter(event, player) {
          return (
            event.player === player.getStorage("sbweiliu_effect")[3] &&
            event.player?.isIn()
          )
        },
        logTarget: "player",
        async content(event, trigger, player) {
          ...
          target.checkHistory(name === "damage" ? "gain" : "lose", (evt) => {
            if (name === "damage") {
              numx += evt.cards.filter(
                (card) => get.color(card) === color,
              ).length
            } else if (evt.type === "discard") {
              numx += evt.cards2.filter(
                (card) => get.color(card) === color,
              ).length
            }
          })
          if (numx >= num) {
            ...
            if (target.hasExpansions("sbmingren")) {
              await target.loseToDiscardpile({
                cards: target.getExpansions("sbmingren"),
              })
            }
          }
        },
  ...
  sbzhenliang: {
    ...
    filter(event, player) {
      ...
      if (event.name === "chooseToUse") {
        if (
          player.storage.sbzhenliang ||
          player.hasSkill("sbzhenliang_used", null, null, false)
        ) {
          return false
        }
    ...
    prompt:
      "弃置与攻击范围内的一名角色体力值之差张与“任”颜色相同的牌（至少一张），对其造成1点伤害",
  ...
  sbtianxiang: {
    ...
    filter(event, player) {
      return (
        player.countCards("h", (card) =>
          lib.skill.sbtianxiang.filterCard(card, player),
        ) &&
        game.hasPlayer((target) =>
          lib.skill.sbtianxiang.filterTarget(null, player, target),
        )
      )
    },
    filterCard(card, player) {
    ...
    subSkill: {
      damage: {
        ...
        async cost(event, trigger, player) {
          ...
          event.result = await player
            .chooseTarget(
              `烈刃：是否对除${get.translation(target)}外的一名其他角色造成1点伤害？`,
              (card, player, target) => {
    ...
    subSkill: {
      add: {
        ...
        content() {
          ...
          player
            .chooseTarget(get.prompt("sbjiang_add"), (card, player, target) => {
              var player = _status.event.player
            ...
            .set("ai", (target) => {
              ...
              if (
                eff + get.effect(player, { name: "losehp" }, player) / 8 >
                0
              ) {
                return eff
              }
        ...
        ai: {
          ...
          skillTagFilter(player, tag, arg) {
            if (tag === "nokeep") {
              ...
              const limit = player.hasMark("sbjiang")
                ? game.countPlayer((current) => {
                    return current.group === "wu" && current !== player
                  }) + 1
                : 1
        ...
        target(card, player, target) {
          ...
          if (
            ...
            ((target.hasZhuSkill("sbzhiba") &&
              ...
              player.countCards("hs", (card) =>
                player.canSaveCard(card, target),
              ) +
    ...
    subSkill: {
      heart: {
        ...
        async cost(event, trigger, player) {
          ...
          event.result = await player
            .chooseTarget(
              `流离：是否令一名不为${get.translation(sourcex)}的其他角色获得“流离”标记？`,
              (card, player, target) => {
    ...
    selectTargetAi: (event, player) => {
      ...
      let targets = [],
        ...
        sbbiyue = player.hasSkill("sbbiyue")
          ? Math.max(
              0,
              3 - game.countPlayer2((current) => current.hasHistory("damage")),
            )
          : 0,
        alter = [null, 1, 1],
        temp
  ...
  sbbiyue: {
    ...
    async content(event, trigger, player) {
      await player.draw(
        Math.min(
          4,
          game.countPlayer2((current) => current.hasHistory("damage")) + 1,
        ),
      )
    },
  },
  ...
  sbmingce: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        ...
        .set(
          "prompt",
          `${get.translation(player)}对你发动了【明策】，请选择一项`,
        )
        .set("ai", () => {
  ...
  sbluanji: {
    ...
    filter(event, player) {
      ...
      return (
        ...
        event.player !== player &&
        player.getHistory("gain", (evt) => evt.getParent(2).name === "sbluanji")
          .length < 3
      )
    },
    filterCard: true,
    ...
    check(card) {
      ...
      for (var i = 0; i < targets.length; i++) {
        ...
        if (
          get.attitude(player, targets[i]) === 0 ||
          targets[i].group === "qun"
        ) {
          eff += 0.5
        }
  ...
  sbxueyi: {
    ...
    filter(event, player) {
      return (
        player.hasZhuSkill("sbxueyi") &&
        event.target !== player &&
        event.target.group === "qun"
      )
    },
    zhuSkill: true,
    forced: true,
      ...
      maxHandcard(player, num) {
        if (player.hasZhuSkill("sbxueyi")) {
          return (
            ...
            2 *
              game.countPlayer(
                (current) => player !== current && current.group === "qun",
              )
          )
        }
      },
  ...
  sblianhuan: {
    ...
    subSkill: {
      ...
      discard: {
        ...
        charlotte: true,
        filter(event, player) {
          return event.isFirstTarget && event.card.storage?.sblianhuan
        },
        content() {
          "step 0"
      ...
      if (history.length) {
        for (var i = history.length - 1; i >= 0; i--) {
          ...
          if (typeof skill === "string") {
            if (
              evt.getParent(2).name === skill ||
              evt.getParent(3).name === skill
            ) {
        ...
        getIndex(event, player) {
          ...
          return game
            .filterPlayer((current) => {
              if (
                !event.getg(current).length ||
                !current.hasMark("sbxuanhuo_mark")
              ) {
                return false
              }
  ...
  sbenyuan: {
    ...
    getIndex(event, player) {
      return game
        .filterPlayer((target) => target.hasMark("sbxuanhuo_mark"))
        .sortBySeat()
    },
    logTarget(event, player, triggername, target) {
      return target
    ...
    logAudio: (index) =>
      typeof index === "number" ? `sbenyuan${index}.mp3` : 2,
    direct: true,
    locked: true,
    async content(event, trigger, player) {
      const target = event.targets[0]
      ...
      if (num >= 3) {
        ...
        if (num) {
          await player.chooseToGive(
            target,
            `恩怨：交给${get.translation(target)}${get.cnNumber(num)}张牌`,
            true,
            num,
            "he",
          )
  ...
  sbtiaoxin: {
    ...
    usable: 1,
    logAudio: (index) =>
      typeof index === "number" ? `sbtiaoxin${index}.mp3` : 2,
    filter(event, player) {
        ...
        return {
          ...
          async content(event, trigger, player) {
            ...
            if (control !== "sha") {
              ...
              next.set(
                "openskilldialog",
                `挑衅：是否将一张手牌当做决斗对${get.translation(player)}使用？`,
              )
      ...
      if (!event.tiaoxinEffect) {
        const targets1 = game.filterPlayer((current) => {
            ...
            return (
              ...
              current.countCards("h") <= player.countCards("hs", "sha")
            )
          }),
        ...
        const bool =
          targets1.length &&
          targets2.length &&
          targets1.containsSome(...targets2)
        ...
        player(player, target) {
          if (!target) {
            return game.countPlayer((current) => {
              ...
              return (
                get.info("sbtiaoxin").ai.result.player(player, current) > -1
              )
            })
          }
  ...
  sbbeifa: {
    ...
    chargeSkill: 9,
    logAudio: (index) =>
      typeof index === "number" ? `sbbeifa${index}.mp3` : 2,
    init(player, skill) {
    ...
    subSkill: {
      ...
      benghuai: {
        ...
        filter(event, player) {
          ...
          return (
            player.getStorage("sbbeifa")?.length ===
            9 - player.countMark("sbbeifa_benghuai")
          )
        },
        async content(event, trigger, player) {
  ...
  sbrende: {
    ...
    chooseButton: {
      dialog(event, player) {
        ...
        if (
          event.type === "phase" &&
          player.hasCard(() => true, "he") &&
          game.hasPlayer(
            (current) => !storage.includes(current) && current !== player,
          )
        ) {
    ...
    ai: {
      ...
      skillTagFilter(player) {
        return (
          player.countMark("sbrende") > 1 &&
          !player.getStorage("sbrende_used").includes("card")
        )
      },
      order(item, player) {
        if (
          _status.event.type === "phase" &&
          lib.skill.sbzhangwu.ai.result.player(player) > 0
        ) {
          return 7.1
        }
        return 4
    ...
    subSkill: {
      ...
      give: {
        ...
        ai1(card) {
          ...
          if (
            ui.selected.cards.length >=
            Math.max(2, player.countCards("he") - player.hp)
          ) {
            return 0
          }
  ...
  sbjijiang: {
    ...
    async content(event, trigger, player) {
      ...
      var choiceList = [
        ...
        result
      if (
        !targets[0].canUse({ name: "sha", isCard: true }, targets[1], false)
      ) {
        result = { index: 1 }
      } else {
  ...
  sblongdan: {
    ...
    chooseButton: {
      dialog(event, player) {
        ...
        for (var name of lib.inpile) {
          ...
          if (
            player.hasCard(lib.skill.sblongdan.getFilter(name, player), "hs")
          ) {
            if (
              event.filterCard(
    ...
    ai: {
      ...
      respondShan: true,
      skillTagFilter(player, tag) {
        return lib.skill.sblongdan.hiddenCard(
          player,
      ...
      effect: {
        ...
        filter(event, player) {
          return (
            ...
            ["sblongdan", "jdlongdan"].some((skill) =>
              player.hasSkill(skill, null, null, false),
            )
          )
        },
        content() {
          ...
          player.addTempSkill(
            "sblongdan_mark",
            player.hasSkill("jdlongdan", null, null, false)
              ? { player: "phaseAfter" }
              : { player: "phaseJieshuBegin" },
          )
          game.delayx()
  ...
  sbpaoxiao: {
    ...
    subSkill: {
      effect: {
        ...
        popup: false,
        filter(event, player) {
          return event.card.storage?.sbpaoxiao && event.target.isIn()
        },
        content() {
      ...
      recoil: {
        ...
        forced: true,
        filter(event, player) {
          return event.card?.storage?.sbpaoxiao && event.player.isIn()
        },
        content() {
          "step 0"
          player.loseHp()
          ...
          var hs = player.getCards("h", (card) =>
            lib.filter.cardDiscardable(card, player, "sbpaoxiao_recoil"),
          )
          if (hs.length > 0) {
  ...
  sbxieji: {
    ...
    async content(event, trigger, player) {
      ...
      //选择对方的协击条件
      await player
        .chooseCooperationFor(target, "sbxieji")
        .set("ai", (button) => {
          var base = 0
          switch (button.link) {
            case "cooperation_damage":
    ...
    subSkill: {
      effect: {
        ...
        content() {
          ...
          player
            .chooseTarget(
              "协击：请选择【杀】的目标",
              `你和${get.translation(trigger.player)}协力成功，可以视为对至多三名其他角色使用一张【杀】，且此【杀】造成伤害时，你摸等同于伤害值的牌`,
              [1, 3],
              true,
      ...
      reward: {
        ...
        filter(event, player) {
          return (
            event.card?.storage?.sbxieji && event.getParent().type === "card"
          )
        },
        content() {
  ...
  sbfenwei: {
    ...
    content() {
      ...
      for (var i = 0; i < cards.length; i++) {
        targets[i]
          .addToExpansion(cards[i], player, "give")
          .gaintag.add("sbfenwei_effect")
      }
      ;("step 1")
      ...
      result: {
        target(player, target) {
          if (
            game.hasPlayer((current) => {
              return (
                (!get.rawAttitude || get.rawAttitude(player, current) > 0) &&
                current !== player &&
    ...
    content() {
      ...
      player.chooseTarget(get.prompt2("sbluoshen")).set("ai", (target) => {
        ...
        for (var targetx of players) {
          eff +=
            get.attitude(player, targetx) * Math.sqrt(targetx.countCards("h"))
        }
        return 1 - eff
      })
      ;("step 1")
      ...
      if (num) {
        const result = await player
          ...
          .set("ai", () => {
            ...
            if (
              get.distance(current, player, "absolute") > 3 &&
              player.hp <= 2
            ) {
              return true
            }
      ...
      effect: {
        target(card, player, target) {
          ...
          if (get.tag(card, "damage") && player !== target) {
            ...
            return [
              1,
              0.55 + 0.05 * Math.max(0, 1 - target.countMark("sbjianxiong")),
            ]
          }
        },
      },
    ...
    subSkill: {
      mark: {
        ...
        async content(event, trigger, player) {
          ...
          list.push("cancel2")
          const result = await player
            .chooseControl(list, () => get.cnNumber(2, true))
  ...
  sbguidao: {
    ...
    filter(event, player) {
      if (
        player.hasSkill("sbguidao_banned") ||
        player.countMark("sbguidao") >= 8
      ) {
        return false
      }
      if (event.name === "damage") {
  ...
  sbhuangtian: {
    ...
    filter(event, player) {
      ...
      return (
        !game.hasPlayer((current) =>
          current.countCards("hej", "taipingyaoshu"),
        ) &&
  ...
  sbqiaoshi: {
    ...
    async cost(event, trigger, player) {
      ...
      event.result = await source
        .chooseBool(
          `樵拾：是否令${get.translation(player)}回复${num}点体力，然后你摸两张牌？`,
        )
        .set("ai", () => {
        ...
        .set(
          "bool",
          get.recoverEffect(player, source, source) +
            2 * get.effect(source, { name: "draw" }, source) >
            5,
        )
        .forResult()
    },
  ...
  sbjiewei: {
    ...
    content() {
      ...
      player
        .gainPlayerCard(target, "visible", true, "h")
        .set("ai", (button) => get.value(button.link, _status.event.target))
    },
    ai: {
      combo: "sbjushou",
      order: 8,
  ...
  sbfanjian: {
    ...
    chooseButton: {
      dialog() {
        return ui.create.dialog(
          `###反间###${get.translation("sbfanjian_info")}`,
        )
      },
        ...
        return {
          ...
          content() {
            ...
            target
              ...
              .set(
                "prompt",
                `${get.translation(player)}对你发动了【反间】并选择了一张牌，请选择一项`,
              )
              .set("ai", () => {
                var player = _status.event.player,
  ...
  sbtongye: {
    ...
    filter(event, player) {
      return ["h", "e", "j"].every((pos) => {
        return game.hasPlayer((current) => {
          return (
            current !== player &&
            current.countCards(pos) === player.countCards(pos)
          )
        })
      })
    },
    async content(event, trigger, player) {
      const card = get.cardPile(
        (card) => !player.getStorage(`${event.name}_used`).includes(card.name),
      )
      if (card) {
        await player.gain(card, "gain2")
  ...
  sbjieyin: {
    ...
    async cost(event, trigger, player) {
      ...
      const list = [],
        choiceList = [
          `令${get.translation(target)}获得一点护甲`,
          `将势力变更为吴，然后获得${get.translation(target)}${num}张牌`,
        ]
      if (target.hujia < 5) {
        list.push("选项一")
      } else {
    ...
    subSkill: {
      fail: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            player.hasCard(
              (card) => get.suit(card) === "heart" && player.canRecast(card),
              "h",
            )
          ) {
            await player.recast(
              player.getCards("h", (card) => get.suit(card) === "heart"),
            )
          }
      ...
      achieve: {
        audio: "sbjieyin",
        logAudio: (event) =>
          event.player.hujia >= 5 ? "sbjieyin3.mp3" : "sbjieyin4.mp3",
        forced: true,
        trigger: {
      ...
      init: {
        ...
        filter(event, player) {
          return (
            game.hasPlayer(
              (current) =>
                current !== player && !current.hasMark("sbjieyin_mark"),
            ) &&
  ...
  sbliangzhu: {
    ...
    filter(event, player) {
      ...
      const targets = [
        player,
        ...game.filterPlayer((current) => current.hasMark("sbjieyin_mark")),
      ]
      if (targets.length <= 1) {
        return false
      }
    ...
    async cost(event, trigger, player) {
      if (trigger.name === "phaseUse" && !player.isDamaged()) {
        ...
        while (true) {
          const card = get.cardPile(
            (card) => get.type(card) === "equip" && !cards.includes(card),
          )
          if (card) {
            cards.push(card)
          } else {
  ...
  sbxiaoji: {
    ...
    getIndex(event, player) {
      ...
      const bool = player.hasAllHistory("useSkill", (evt) => {
        ...
        for (const skill of skills) {
          ...
          return (
            skill.endsWith("_achieve") &&
            player.awakenedSkills.includes(sourceSkill) &&
            info?.dutySkill
          )
        }
      })
    ...
    async content(event, trigger, player) {
      ...
      if (
        game.hasPlayer((current) =>
          current.countDiscardableCards(player, "ej"),
        ) &&
        bool
      ) {
        const result = await player
      ...
      if (
        player.storage.sbxiaoji_hujia &&
        player.countRoundHistory(
          "useSkill",
          (evt) => evt.skill === "sbxiaoji",
        ) === 1
      ) {
  ...
  sbkeji: {
    ...
    filter(event, player) {
      return (
        (player.getStat("skill").sbkeji || 0) < (player.storage.sbkeji ? 1 : 2)
      )
    },
    check(card) {
      var player = _status.event.player
    ...
    subSkill: {
      ...
      block: {
        mod: {
          cardEnabled(card, player) {
            ...
            if (!evt?.respondTo || !cards.includes(evt.respondTo[1])) {
              return
            }
      ...
      count: {
        ...
        content() {
          ...
          player.storage.sbliegong.sort(
            (a, b) => lib.suit.indexOf(b) - lib.suit.indexOf(a),
          )
          player.addTip(
            "sbliegong",
            get.translation("sbliegong") +
              player
      ...
      add: {
        ...
        filter(event, player) {
          return (
            event.card.name === "sha" &&
            player.isPhaseUsing() &&
            player.countMark("splveying_used") < 2
          )
        },
        async content(event, trigger, player) {
    ...
    async content(event, trigger, player) {
      const suits = player
        ...
        targets = game.filterPlayer((current) => {
          return (
            current !== player &&
            !player.getStorage(event.name).includes(current)
          )
        })
      ...
      const result = await player
        .chooseCard(
          ...
          true,
          [1, numx],
          `瞑昡：请选择至多${get.cnNumber(numx)}张花色各不相同的手牌`,
          (card, player) => {
  ...
  spxianchou: {
    ...
    filter(event, player) {
      return (
        ...
        event.source !== player &&
        game.hasPlayer(
          (current) => current !== player && current !== event.source,
        )
      )
    },
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          return (
            get.effect(source, card, target, get.player()) *
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        .chooseToDiscard(
          "he",
          `是否弃置一张牌，视为对${get.translation(trigger.source)}使用一张【杀】？`,
        )
        .set("ai", (card) => {
  ...
  new_spxianchou: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await target
        ...
        .set(
          "choice",
          get.effect(trigger.source, card, target, target) +
