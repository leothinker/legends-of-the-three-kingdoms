...
const skills = {
  ...
  buqu: {
    ...
    async content(_event, trigger, player) {
      ...
      await player.showCards(
        `${get.translation(player)}的不屈牌`,
        player.getExpansions("buqu"),
      )
      if (lib.skill.buqu.process(player)) {
      ...
      effect: {
        target_use(card, player, target, _current) {
          if (
            ...
            const pos = player.hasSkillTag("viewHandcard", null, target, true)
              ? "hes"
              : "e"
            ...
            if (pos === "e") {
              return [
                1,
                Math.min(4, 1 + 0.75 * Math.max(1, target.countCards("hs"))),
              ]
            }
            return [1, 1]
          }
  ...
  huangtian2: {
    ...
    prompt() {
      ...
      const list = game.filterPlayer(
        (target) =>
          target !== player && target.hasZhuSkill("huangtian", player),
      )
    ...
    filter(_event, player) {
      ...
      if (
        player.countCards("h", "shan") + player.countCards("h", "shandian") ===
        0
      ) {
        return 0
      }
        ...
        return {
          ...
          ai1(card) {
            ...
            if (enemyNum) {
              ...
              if (lib.skill.guhuo_backup.aiUse < 0.5 && !player.isDying()) {
                return 0
              }
            }
            return 6 - get.value(card)
      ...
      guess: {
        ...
        async content(event, trigger, player) {
          ...
          if (event.fake) {
            ...
            for (const target of event.betrayer) {
              await target.loseHp()
            }
  ...
  quhu: {
    ...
    filter(_event, player) {
      ...
      return game.hasPlayer(
        (current) => current.hp > player.hp && player.canCompare(current),
      )
    },
    filterTarget(_card, player, target) {
    ...
    async content(event, _trigger, player) {
      ...
      if (
        !game.hasPlayer((player) => player !== target && target.inRange(player))
      ) {
        return
      }
      const result = await player
        ...
        .set("ai", (target) =>
          get.damageEffect(target, _status.event.source, player),
        )
        .set("source", target)
        .forResult()
  ...
  jieming: {
    ...
    async cost(event, _trigger, player) {
      event.result = await player
        .chooseTarget(get.prompt2(event.skill), (_card, _player, _target) => {
  ...
  lianhuan: {
    ...
    hiddenCard(player, name) {
      return (
        name === "tiesuo" &&
        player.hasCard((card) => get.suit(card) === "club", "sh")
      )
    },
    enable: "chooseToUse",
    filter(event, player) {
      ...
      return (
        event.type === "phase" ||
        event.filterCard(
          get.autoViewAs({ name: "tiesuo" }, "unsure"),
          player,
          event,
        )
      )
    ...
    filterTarget(_fuck, player, target) {
      ...
      const cardx = get.autoViewAs({ name: "tiesuo" }, [card])
      return (
        backup.filterCard(cardx, player, event) &&
        backup.filterTarget(cardx, player, target)
      )
    },
    selectTarget() {
      const card = ui.selected.cards[0],
    ...
    lose: false,
    delay: false,
    viewAs(_cards, _player) {
      return {
        name: "tiesuo",
      }
    },
    prepare: () => true,
    async precontent(event, _trigger, _player) {
    ...
    ai: {
      order(_item, player) {
        if (
          game.hasPlayer(
            (current) =>
              get.effect(current, { name: "tiesuo" }, player, player) > 0,
          ) ||
          player.hasCard(
            (card) => get.suit(card) === "club" && player.canRecast(card),
            "h",
          )
        ) {
          return 8
  ...
  niepan: {
    ...
    ai: {
      order: 1,
      skillTagFilter(player, _arg, target) {
  ...
  shuangxiong: {
    ...
    subSkill: {
      judge: {
        ...
        preHidden: true,
        prompt2: () =>
          "进行一次判定，本回合可以将一张与此牌颜色不同的手牌当作【决斗】使用",
        async content(_event, trigger, player) {
          trigger.changeToZero()
          await player
            .judge()
            .set("callback", lib.skill.shuangxiong_judge.callback)
        },
      ...
      viewas: {
        ...
        viewAsFilter(player) {
          return player.hasCard(
            (card) => lib.skill.shuangxiong_viewas.filterCard(card, player),
            "hs",
          )
        },
        filterCard(card, player) {
      ...
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          return (
            (4.25 - draw) *
              10 *
              Math.sqrt(Math.max(0.01, get.threaten(target))) +
          ...
          (
            await player
              .chooseControl(
                str1,
                str2,
                (_event, _player) => _status.event.choice,
              )
  ...
  benghuai: {
    ...
    async content(_event, _trigger, player) {
      const { control } = await player
        .chooseControl("baonue_hp", "baonue_maxHp", (_event, player) => {
  ...
  baonue2: {
    ...
    async content(_event, trigger, player) {
      ...
      const next = trigger.source.judge((card) => {
        if (get.suit(card) === "spade") {
          return 4
        }
        return 0
  ...
  fangquan2: {
    ...
    async content(event, _trigger, player) {
      const [target] = event.targets
      player.logSkill(
        player.name === "re_liushan" ? "refangquan" : "fangquan",
        event.targets,
        "fire",
      )
  ...
  jiang: {
    ...
    ai: {
      effect: {
        target_use(card, _player, _target) {
          if (card.name === "sha" && get.color(card) === "red") {
            return [1, 0.6]
          }
        },
        player_use(card, _player, _target) {
          if (card.name === "sha" && get.color(card) === "red") {
            return [1, 1]
          }
        },
      ...
      global: {
        ...
        async content(event, _trigger, player) {
          ...
          if (result.bool === false) {
            ...
            const next = target
              .chooseBool(`是否获得${get.translation(list)}？`)
              .set("ai", () => get.value(list) > 0)
    ...
    async cost(event, trigger, player) {
      ...
      if (cards.length === 1) {
        ...
        event.result = await player
          ...
          .set(
            "choice",
            lib.skill.guzheng.checkx(trigger, player, cards, cards2),
          )
        ...
        event.result = await player
          ...
          .set("filterButton", (button) => {
            ...
            return (
              type === "string" || _status.event.allowed.includes(button.link)
            )
          })
          .set("allowed", cards2)
          .set(
  ...
  qinyin: {
    ...
    logAudio2: {
      mb_zhouyu: (index) =>
        typeof index === "number" ? `qinyin_mb_zhouyu${index}.mp3` : 2,
    },
    filter(event, player) {
      var cards = []
    ...
    async content(event, _trigger, player) {
      ...
      for (let time = event.count; time > 0; time--) {
        ...
        const _prompt = `${get.prompt("qinyin")}（剩余${get.cnNumber(time)}次）`
        ...
        player.logSkill("qinyin", null, null, null, [
          result.control === "回复体力" ? 2 : 1,
        ])
  ...
  yeyan: {
    ...
    selectCard: [0, 4],
    filterTarget(_card, _player, _target) {
    ...
    getBigFire(player) {
      if (
        player
          ...
          .reduce((list, card) => list.add(get.suit(card, player)), []).length <
        4
      ) {
        return false
      }
        ...
        return (
          player.hp +
            player.countCards(
              "h",
              (card) =>
                !cards.includes(card) && player.canSaveCard(card, player),
            ) -
            3 >
          0
        )
  ...
  qixing2: {
    ...
    filter(_event, player) {
      return (
        player.getExpansions("qixing").length > 0 && player.countCards("h") > 0
      )
    },
    async content(_event, _trigger, player) {
      const expansions = player.getExpansions("qixing")
  ...
  kuangfeng3: {
    ...
    filter(event, player) {
      return (
        ...
        event.player.getStorage("kuangfeng2").includes(player)
      )
    },
    charlotte: true,
    forced: true,
      ...
      const {
        ...
        .set(
          ...
          player.getExpansions("qixing").length >=
            game.countPlayer((current) => get.attitude(player, current) > 4) *
              2,
        )
        .forResult()
      event.result = {
  ...
  dawu3: {
    ...
    filter(event, player) {
      return (
        ...
        event.player.getStorage("dawu2").includes(player)
      )
    },
    forced: true,
    charlotte: true,
  ...
  renjie: {
    ...
    ai: {
      ...
      effect: {
        target(card, player, target) {
          if (
            (!target.hasSkill("sbaiyin") && !target.hasSkill("jilue")) ||
            !target.hasFriend()
          ) {
            return
          }
  ...
  jilue_zhiheng: {
    ...
    async content(event, _trigger, player) {
      ...
      const _extraNum = discardingCards.every((card) =>
    ...
    ai: {
      ...
      nokeep: true,
      skillTagFilter(player, tag, _arg) {
        if (tag === "nokeep") {
          return (
            player.isPhaseUsing() &&
