...
const skills = {
    ...
    chooseButton: {
      ...
      backup(links, _player) {
        return {
          ...
          popname: true,
          ignoreMod: true,
          async precontent(event, _trigger, player) {
            if (
              !event.result?.bool ||
              !event.result.card ||
  ...
  rejijun: {
    ...
    async callback(event, _trigger, player) {
      const { card } = event
  ...
  refangtong: {
    ...
    async cost(event, _trigger, player) {
      event.result = await player
        ...
        .set("ai", (card) => {
          ...
          if (
            player.getExpansions("rejijun").reduce((num, card) => {
    ...
    thunderEffect(card, player) {
      let cards = player.getExpansions("rejijun"),
        num = 0
      cards.push(card)
      if (cards.reduce((num, card) => num + get.number(card, false), 0) <= 36) {
        return false
      }
      // @ts-expect-error
  ...
  rexingshuai: {
        ...
        if (current.group === "wei") {
          const result = await current
            ...
            .set(
              "ai",
              () =>
                get.attitude(_status.event.player, _status.event.target) > 2,
            )
            .set("target", player)
            .forResult()
          if (result?.bool) {
    ...
    subSkill: {
      restore: {
        ...
        filter(event, _player) {
          return (
            event.source?.isIn() && event.source.hasSkill("remingjian_buff")
          )
        },
        async content(_event, _trigger, player) {
  ...
  rejiqiao: {
    ...
    async content(event, _trigger, player) {
      ...
      const num =
        cards.length + cards.filter((card) => get.type(card) === "equip").length
  ...
  relinglong: {
    ...
    init(player, skill) {
      player.addExtraEquip(
        ...
        "bagua",
        true,
        (player) => player.hasEmptySlot(2) && lib.card.bagua,
      )
    },
    onremove(player, skill) {
    ...
    subSkill: {
      directhit: {
        ...
        ai: {
          directHit_ai: true,
          skillTagFilter(player, _tag, arg) {
            if (
              !arg ||
              !arg.card ||
              !arg.target ||
  ...
  dcyicong: {
    ...
    async content(_) {},
    mod: {
      globalFrom(_from, _to, current) {
        return current - 1
      },
      globalTo(_from, to, current) {
  ...
  olhuoji: {
    ...
    async content(_event, trigger, _player) {
      trigger.set("chooseToShow", async (event, _player, target) => {
    ...
    async huogongContent(event, _trigger, player) {
      const { target } = event
  ...
  dcjiushi: {
    ...
    subSkill: {
      use: {
        ...
        ai: {
          save: true,
          skillTagFilter(player, _tag, _arg) {
            ...
            player(player) {
              if (_status.event.parent.name === "phaseUse") {
                ...
                if (e2) {
                  ...
                  if (e2.name === "renwang") {
                    if (
                      !player.countCards("h", { name: "sha", color: "red" })
                    ) {
                      return 0
                    }
                  }
  ...
  remoshi: {
    ...
    async content(_event, trigger, _player) {
      ...
      const next = trigger.player.addToExpansion(
    ...
    subSkill: {
      retrieve: {
        ...
        filter(_event, _player, _name, target) {
        ...
        forced: true,
        logTarget: (_event, _player, _name, target) => target,
        async content(event, _trigger, player) {
  ...
  refuman: {
    ...
    subSkill: {
      ...
      draw: {
        ...
        forced: true,
        filter: (_event, _player, _name, target) => target,
        logTarget: (_event, _player, _name, target) => target,
        async content(event, trigger, player) {
  ...
  rekuangbi: {
    ...
    async cost(event, _trigger, player) {
      event.result = await player
        .chooseTarget(get.prompt2("rekuangbi"), (_card, player, target) => {
        ...
        .set(
          "goon",
          player.countCards("hs", (card) => player.hasValueTarget(card)) >= 2,
        )
        .forResult()
    },
    async content(event, _trigger, player) {
        ...
        while (event.given.length < 4) {
          ...
          if (resultx?.bool) {
            ...
            const resulty = await player
              .chooseTarget(
                `将${get.translation(resultx.links)}交给一名其他角色`,
                true,
                (_card, player, target) => target !== player,
              )
              .set("ai", (target) => {
    ...
    async content(event, _trigger, player) {
      ...
      if (
        ...
        result = await targets[0]
          ...
          .set(
            "prompt",
            `对${get.translation(targets[1])}使用一张杀，或摸一张牌`,
          )
          .forResult()
      }
    ...
    async content(_event, trigger, player) {
      ...
      if (!trigger.source?.isIn()) {
        ...
        result = await trigger.source
          .chooseToDiscard("智愚：请弃置一张手牌", true)
          .forResult()
      }
      let cards = player.getCards("h")
      const bool =
        cards.map((card) => get.color(card, player)).unique().length === 1
      if (bool) {
  ...
  repingkou: {
    ...
    async content(event, _trigger, player) {
      ...
      const targets2 = targets.filter(
        (target) => target.countDiscardableCards(player, "e") > 0,
      )
      if (targets2.length > 0) {
        const result = await player
          .chooseTarget(
            "是否弃置一名目标角色的一张装备牌？",
            (_card, _player, target) => _status.event.targets.includes(target),
          )
        ...
        const result = await target
          ...
          .set("ai", () => {
            ...
            if (get.attitude(player, evt.player) > 0) {
              return !evt.player.countCards("h", "sha")
            }
            if (
    ...
    subSkill: {
      sha: {
        ...
        prompt2(event, player) {
          var list = player.getStorage("rejixu_sha").filter((target) => {
            if (event.targets.includes(target) || !target.isIn()) {
              return false
            }
        ...
        logTarget(event, player) {
          return player.getStorage("rejixu_sha").filter((target) => {
            if (event.targets.includes(target) || !target.isIn()) {
              return false
            }
            return lib.filter.targetEnabled2(event.card, player, target)
  ...
  oljieming: {
    ...
    async cost(event, _trigger, player) {
      event.result = await player
        .chooseTarget(get.prompt2(event.skill), (_card, _player, target) => {
    ...
    ai: {
      ...
      maixie_hp: true,
      effect: {
        target(card, player, target, _current) {
  ...
  reqinwang: {
    ...
    selectTarget: -1,
    filterTarget(_card, player, current) {
      return (
        current !== player &&
        current.group === "shu" &&
        player.hasZhuSkill("reqinwang", current)
    ...
    async content(event, _trigger, player) {
      ...
      if (
        ...
        const result = await target
          ...
          .set("ai", (_card) => (_status.event.goon ? 1 : 0))
          .forResult()
        if (result?.bool) {
  ...
  reandong: {
    ...
    async content(_event, trigger, player) {
      ...
      const choiceList = [
        `防止${str}即将受到的伤害，且本回合内红桃牌不计入${bool ? get.translation(target) : "自己"}的手牌上限。`,
      ]
      ...
      if (result?.index === 0) {
        ...
        if (!target.countCards("h")) {
          ...
          const cards = target.getCards(
            "h",
            (card) => get.suit(card, target) === "heart",
          )
          if (cards.length > 0) {
            await player.gain(cards, target, "give", "bySelf")
          }
  ...
  reyingshi: {
    ...
    async content(event, _trigger, player) {
      ...
      if (result?.bool) {
        ...
        next.set("filterTarget", function (_card, _player, target) {
          if (
  ...
  dcanxu: {
    ...
    async content(event, _trigger, player) {
      ...
      const result = await gainner
        .gainPlayerCard(giver, true, "h", "visibleMove")
        .forResult()
      if (result?.cards?.length) {
        const card = result.cards[0]
        if (
          gainner.getCards("h").includes(card) &&
          get.suit(card, gainner) !== "spade"
        ) {
          await player.draw()
        }
      }
      ...
      judge: {
        ...
        filter(_event, player) {
          return (
            ...
            player.hasCard(
              (card) =>
                lib.filter.cardDiscardable(card, player, "reqiaobian_judge"),
              "he",
            )
          )
        },
      ...
      draw: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            player.hasCard(
              (card) =>
                lib.filter.cardDiscardable(card, player, "reqiaobian_draw"),
              "he",
            )
          ) {
            choices.push("弃置牌")
      ...
      use: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            player.hasCard(
              (card) =>
                lib.filter.cardDiscardable(card, player, "reqiaobian_use"),
              "he",
            )
          ) {
            choices.push("弃置牌")
      ...
      discard: {
        ...
        async content(event, trigger, player) {
          ...
          if (
            player.hasCard(
              (card) =>
                lib.filter.cardDiscardable(card, player, "reqiaobian_discard"),
              "he",
            )
          ) {
            choices.push("弃置牌")
  ...
  rejianyan: {
    ...
    chooseButton: {
      ...
      check() {
        if (
          !_status.event.player.getStorage("rejianyan_used").includes("color")
        ) {
          return "red"
        }
        return "trick"
      },
      backup(result, _player) {
        return {
          audio: "rejianyan",
          filterCard: () => false,
          selectCard: -1,
          ...
          async content(event, _trigger, player) {
            ...
            if (info === "red" || info === "black") {
              ...
              card = get.cardPile2((card) => get.color(card) === info, "top")
            } else {
              player.markAuto("rejianyan_used", "type")
              card = get.cardPile2((card) => get.type(card) === info, "top")
            }
            if (card) {
              event.card = card
            ...
            result = await player
              .chooseTarget(
                ...
                (_card, _player, target) =>
                  target.group === "key" || target.hasSex("male"),
              )
              .set("ai", (target) => {
                var player = _status.event.player,
                  att = get.attitude(player, target)
  ...
  decadexianzhen2: {
    ...
    mod: {
      targetInRange(_card, player, target) {
        if (target === player.storage.decadexianzhen2) {
          return true
        }
      },
      cardUsableTarget(_card, player, target) {
        if (target === player.storage.decadexianzhen2) {
          return true
        }
      },
  ...
  xinbuyi: {
    ...
    async content(_event, trigger, player) {
      ...
      if (player === trigger.player) {
        ...
        result = await player
          .choosePlayerCard("h", trigger.player, true)
          .forResult()
      }
  ...
  reqianxi: {
      ...
      if (
        ...
        result = await player
          .chooseTarget(
            ...
            (_card, player, target) =>
              target !== player && get.distance(player, target) <= 1,
          )
          .set(
            "ai",
            (target) =>
    ...
    subSkill: {
      effect: {
        ...
        popup: false,
        filter(_event, player) {
          return player.storage.reqianxi_effect?.[1].isIn()
        },
        async content(_event, _trigger, player) {
  ...
  olduanliang: {
    ...
    mod: {
      targetInRange(card, player, _target) {
      ...
      while (player.isIn() && target.isIn()) {
        ...
        if (
          ...
          list1
            ...
            .toUniqued()
            .every((suit) =>
              list2.some((card) => get.suit(card, target) === suit),
            )
        ) {
          const result = await player
            .chooseBool(
              "是否继续发动【樵拾】？",
              `和${get.translation(target)}各摸一张牌`,
            )
            .forResult()
          if (!result?.bool) {
            break
    ...
    async content(_event, trigger, player) {
      ...
      const num = Math.min(
        ...
        player.getHistory(
          "useSkill",
          (evt) =>
            evt.skill === "reyanyu" && evt.event.getParent(2) === trigger,
        ).length,
      )
      result = await player
        .chooseTarget(
          get.prompt("reyanyu"),
          `令一名男性角色摸${get.cnNumber(num)}张牌`,
          (_card, player, target) => target.hasSex("male") && target !== player,
        )
        .set("ai", (target) => get.attitude(_status.event.player, target))
        .forResult()
  ...
  xinzongxuan: {
    ...
    async content(_event, trigger, player) {
      ...
      if (result.bool) {
        ...
        const list = result.moved[0].filter(
          (i) => get.type2(i, false) === "trick",
        )
  ...
  xinhuangtian2: {
    ...
    delay: false,
    line: true,
    prepare(_cards, _player, targets) {
    ...
    prompt() {
      ...
      var list = game.filterPlayer(
        (target) =>
          target !== player && target.hasZhuSkill("xinhuangtian", player),
      )
    ...
    filter(_event, player) {
      ...
      return player.hasCard(
        (card) => lib.skill.xinhuangtian2.filterCard(card, player),
        "h",
      )
    },
    filterCard(card, player) {
        ...
        async content(event, _trigger, player) {
          ...
          while (true) {
            ...
            result = await target
              .chooseCard(
                "h",
                `好施：是否将一张手牌交给${get.translation(player)}？`,
              )
              .set("ai", (card) => {
                var player = _status.event.player,
                  target = _status.event.getTrigger().player
  ...
  reluanwu: {
    audio: "luanwu",
    inherit: "luanwu",
    async contentAfter(_event, _trigger, player) {
  ...
  rebingyi: {
    ...
    async cost(event, _trigger, player) {
      const selfDraw =
          get.info(event.skill).filterx(player) &&
          get.info(event.skill).filtery(player),
    ...
    async content(event, _trigger, player) {
      ...
      if (data.asyncDraw && data.targets?.length) {
    ...
    filter(event, player) {
      ...
      if (event.name === "loseAsync") {
        ...
        return game.hasPlayer((current) => {
          ...
          return event
            .getg?.(current)
            .some((card) => event.getl?.(player)?.cards2?.includes(card))
        })
      }
      if (player === event.player) {
        return false
    ...
    chooseButton: {
      ...
      prompt(links, player) {
        ...
        var num = get.cnNumber(
            Math.max(1, player.getExpansions("xinquanji").length - 1),
          ),
    ...
    subSkill: {
      ...
      backup0: {
        ...
        async content(event, _trigger, player) {
          ...
          // step 1
          await target
            .draw(Math.max(1, player.getExpansions("xinquanji").length))
            .forResult()
        },
        ai: {
          result: {
      ...
      backup1: {
        ...
        selectTarget() {
          return [
            ...
            Math.max(
              1,
              _status.event.player.getExpansions("xinquanji").length - 1,
            ),
          ]
        },
        async content(event, _trigger, player) {
    ...
    filter(event, player) {
      ...
      return (
        target.getHistory(
          "useCard",
          (evt) =>
            evt.targets && evt.targets.filter((i) => i !== target).length > 0,
        ).length === 0
      )
    },
    frequent: true,
  ...
  rexianzhou: {
    ...
    async content(event, _trigger, player) {
      ...
      const list = game.filterPlayer((current) => target.inRange(current))
      if (list.length) {
        ...
        const result = await target
          .chooseTarget(
            ...
            (_card, _player, target) => _status.event.list.includes(target),
          )
          .set("list", list)
          .set("ai", (target) => {
            var player = _status.event.player
  ...
  relongyin: {
    ...
    onremove: (_player) => {
      if (
        !game.hasPlayer(
          (current) => current.hasSkill("relongyin", null, null, false),
          true,
        )
      ) {
  ...
  decadezhenjun: {
    ...
    filter(_event, player) {
      return game.hasPlayer(
        (current) => current.countDiscardableCards(player, "he") > 0,
      )
    },
    direct: true,
    async content(_event, _trigger, player) {
      const chooseResult = await player
        ...
        .set("ai", (target) => {
          ...
          return (
            -get.attitude(player, target) *
            (target.countDiscardableCards(player, "e") + 1)
          )
        })
        .forResult()
      ...
      const num = Math.min(
        Math.max(target.countCards("h") - target.hp, 1),
        target.countDiscardableCards(player, "he"),
      )
      player.logSkill("decadezhenjun", target)
      const discardResult = await player
        .discardPlayerCard(num, target, true, "allowChooseAll")
        .forResult()
  ...
  redingpin: {
    ...
    filter(event, player) {
      ...
      return (
        player.countCards("he", (card) => !list.includes(get.type2(card))) > 0
      )
    },
    filterCard(card) {
  ...
  xinjiangchi: {
    ...
    async content(_event, _trigger, player) {
      ...
      if (
        player.countCards(
          "he",
          (card) => lib.filter.cardDiscardable(card, player, "xinjiangchi") > 0,
        ) > 0
      ) {
        ...
        .set("ai", () => {
          ...
          if (
            !player.countCards(
              "hs",
              (card) =>
                get.name(card) === "sha" && player.hasValueTarget(card, false),
            )
          ) {
            return 1
          }
  ...
  rezhongyong: {
    ...
    async content(event, trigger, player) {
      ...
      const allCards = usedCards.slice()
      game.countPlayer2((current) => {
      ...
      const targetResult = await player
        .chooseTarget(
          ...
          `令一名其他角色获得${get.translation(allCards)}`,
          (_card, player, target) =>
            !_status.event.source.includes(target) && target !== player,
        )
        .set("ai", (target) => get.attitude(_status.event.player, target))
  ...
  changbiao: {
    audio: 2,
    mod: {
      targetInRange(card, _player, _target) {
        if (card.changbiao) {
          return true
        }
      },
  ...
  gzpaiyi: {
    ...
    async contentx(event, _trigger, player) {
      const { target } = event
    ...
    async content(_event, _trigger, player) {
      ...
      result = await player
        .chooseCard(
          ...
          get.prompt("residi"),
          "将一张非基本牌置于武将牌上作为“司”",
          (card, _player) => get.type(card) !== "basic",
        )
        .set("ai", (card) => {
          if (get.position(card) === "e") {
  ...
  residi_push: {
    ...
    filter(event, player) {
      return (
        event.player !== player && player.getExpansions("residi").length > 0
      )
    },
    async content(_event, trigger, player) {
      let result
      ...
      result = await player
        ...
        .set("ai", (button) => {
          ...
          var _card = button.link
          var color = get.color(button.link, false)
          var eff = target.countCards(
            "h",
            (card) =>
              get.color(card, target) === color && target.hasValueTarget(card),
          )
          if (
            !target.countCards(
              "h",
  ...
  residi3: {
    ...
    async content(_event, trigger, player) {
      ...
      if (sha) {
        await player
          .useCard({ name: "sha", isCard: true }, trigger.player)
          .forResult()
      }
      if (trick) {
    ...
    async content(event, _trigger, player) {
      ...
      if (
        ...
        const { targets } = await player
          .chooseTarget(
            `请选择至多${get.cnNumber(num)}名有牌的其他角色，获得这些角色的各一张牌。`,
            [1, num],
            (_card, player, target) => {
              return (
                target !== player && target.countGainableCards(player, "he")
              )
            },
          )
          .set("ai", (target) => {
    ...
    async content(_event, trigger, player) {
      ...
      const result = await player
        ...
        .set(
          "small",
          player.hp > 1 &&
            get.effect(player, { name: "sha" }, target, player) > 0 &&
            Math.random() < 0.9,
        )
        .forResult()
      if (result.bool) {
    ...
    async content(event, _trigger, player) {
      ...
      result = await player
        .chooseCardTarget({
          ...
          filterTarget: lib.filter.notMe,
          goon: game.hasPlayer(
            (current) => current !== player && ai2(player, current) > 0,
          ),
          ai1(card) {
            if (!_status.event.goon) {
              return 0
      ...
      if (
        ...
        result = await player
          .chooseTarget(
            (_card, player, target) =>
              target !== player && target !== _status.event.target,
  ...
  decadezishou_zhiheng: {
    ...
    filter(_event, player) {
      return (
        ...
        !player.getHistory(
          "useCard",
          (evt) => evt.targets.filter((target) => target !== player).length > 0,
        ).length
      )
    },
    async content(_event, _trigger, player) {
  ...
  yongjin: {
    ...
    animationColor: "wood",
    enable: "phaseUse",
    filter(_event, _player, cards) {
      return game.hasPlayer((current) => {
        var es = current.getCards(
          "e",
          (card) => !cards || !cards.includes(card),
        )
        for (var i = 0; i < es.length; i++) {
          if (
            game.hasPlayer(
    ...
    async content(event, _trigger, player) {
      ...
      while (event.count > 0) {
        ...
        const chooseTargetResult = await player
          .chooseTarget(2, (_card, _player, target) => {
            if (ui.selected.targets.length) {
              var from = ui.selected.targets[0]
              if (target.isMin()) {
                return false
              }
              var es = from.getCards(
                "e",
                (card) => !_status.event.cards.includes(card),
              )
              for (var i = 0; i < es.length; i++) {
          ...
          .set("ai", (target) => {
            ...
            var es = ui.selected.targets[0].getCards(
              "e",
              (card) => !_status.event.cards.includes(card),
            )
            var i
    ...
    ai: {
      ...
      result: {
        player(player) {
          var num = 0
          var friends = game.filterPlayer(
            (current) => get.attitude(player, current) >= 4,
          )
          var vacancies = {
            equip1: 0,
            equip2: 0,
  ...
  decadexuanfeng: {
    ...
    async cost(event, _trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) => {
          ...
          if (ui.selected.targets.length) {
            const pre = ui.selected.targets[0],
    ...
    async content(event, _trigger, player) {
      const targets = event.targets
      for (const target of targets) {
        const num = targets.length > 1 ? 1 : 2
        if (get.mode() !== "identity" || player.identity !== "nei") {
          player.addExpose(0.2)
        }
        for (let i = 0; i < num; i++) {
          ...
          if (i > 0) {
            next.set(
              "prompt",
              `旋风：是否继续弃置${get.translation(target)}一张牌？`,
            )
          } else {
    ...
    filter(_event, player) {
      return game.hasPlayer(
        (current) =>
          ...
          current.getHistory(
            "lose",
            (evt) => evt.cards2 && evt.cards2.length > 0,
          ).length > 0,
      )
    },
    async content(_event, _trigger, player) {
  ...
  rezishou: {
    ...
    check(_event, player) {
      return (
        ...
        !player.countCards(
          "h",
          (card) => get.tag(card, "damage") && player.hasUseTarget(card),
        )
      )
    },
    filter(event, _player) {
  ...
  rezishou2: {
    ...
    async content(event, trigger, player) {
      ...
      var filterTarget = (_card, player, target) =>
        target !== player &&
        target.countCards("e", (card) => player.canEquip(card))
      if (game.hasPlayer((current) => filterTarget(null, player, current))) {
        ...
        if (result.bool) {
          ...
          const result2 = await player
            ...
            .set("filterButton", (button) =>
              _status.event.player.canEquip(button.link),
            )
            .forResult()
          // step 2
    ...
    async content(event, trigger, player) {
      ...
      if (discard) {
        ...
        result2 = await player
          ...
          .set("ai", (button) =>
            get.value(button.link, _status.event.getTrigger().target),
          )
          .forResult()
      }
      if (draw) {
      ...
      // step 3
      if (event.equip && result2?.links?.length) {
  ...
  rejianchu: {
    ...
    ai: {
      unequip_ai: true,
      directHit_ai: true,
      skillTagFilter(_player, tag, arg) {
        if (tag === "directHit_ai") {
          return (
            arg.card.name === "sha" &&
  ...
  regongji: {
    ...
    position: "he",
    filter(_event, player) {
      return player.hasCard((card) => lib.skill.regongji.filterCard(card), "eh")
    },
    filterCard(card, _player) {
      return get.type(card) !== "basic"
    },
    filterTarget(_card, player, target) {
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          ...
          var noe2 =
            es.length === 1 && es[0].name !== "tengjia" && get.value(es[0]) <= 0
  ...
  olyajiao: {
    ...
    async content(_event, trigger, player) {
      ...
      await player
        ...
        .set("callback", async (event, _trigger, player) => {
          const { cards } = event
          const [card] = cards
          const evt = event.getParent()
  ...
  olpaoxiao: {
    ...
    audioname2: {
      ...
      ol_guanzhang: "paoxiao_ol_guanzhang",
    },
  ...
  rexuanfeng: {
    ...
    ai: {
      effect: {
        target(card, _player, _target, _current) {
          if (get.type(card) === "equip" && !get.cardtag(card, "gifts")) {
            return [1, 3]
          }
        },
        ...
        async content(event, trigger, player) {
          ...
          if (cards.length) {
            result = await player
              ...
              .set("ai", (button) =>
                get.value(button.link, _status.event.player, "raw"),
              )
              .forResult()
          }
          // step 2
  ...
  olniepan: {
    ...
    ai: {
      order: 1,
      skillTagFilter(player, _tag, target) {
  ...
  rewurong: {
    ...
    async content(event, _trigger, player) {
      ...
      if (player.isOnline()) {
        ...
        player.chooseCard(true).set("glow_result", true).ai = (card) => {
  ...
  rejinjiu2: {
    ...
    ai: {
      filterDamage: true,
      skillTagFilter(_player, _tag, arg) {
  ...
  olzhiba2: {
    ...
    direct: true,
    clearTime: true,
    async contentBefore(event, _trigger, player) {
      const { targets } = event
      const list = []
      if (
        player.hasZhuSkill("olzhiba") &&
      ...
      if (list.length === 1) {
        ...
        chooseRes = await player
          .chooseTarget(
            true,
            "请选择获得所有拼点牌的角色",
            (_card, _pl, target) => _status.event.list.includes(target),
          )
          .set("list", list)
          .forResult()
        if (!chooseRes?.bool) {
  ...
  reguhuo: {
    ...
    chooseButton: {
      ...
      backup(links, _player) {
        return {
          ...
          filterCard(card, player, _target) {
            var result = true
            var suit = card.suit,
              number = card.number
            card.suit = "none"
    ...
    subSkill: {
      ...
      guess: {
        ...
        async contentx(event, _trigger, _player) {
  ...
  rechanyuan: {
    ...
    mark: true,
    intro: {
      content(_storage, player, _skill) {
        var str =
  ...
  xinleiji: {
    ...
    ai: {
      useShan: true,
      effect: {
        target_use(card, player, target, _current) {
          let name
          if (typeof card === "object") {
            if (card.viewAs) {
              name = card.viewAs
  ...
  reqingguo: {
    ...
    ai: {
      ...
      effect: {
        target(card, _player, _target, current) {
  ...
  new_yijue: {
      ...
      if (!lib.skill[skill]) {
        lib.skill[skill] = {
          ...
          intro: {
            markcount: () => 0,
            content: (storage) =>
              `本回合不能使用或打出手牌、非锁定技失效且受到${get.translation(storage[1])}红桃【杀】的伤害+1`,
          },
          group: "new_yijue_ban",
        }
    ...
    subSkill: {
      effect: {
        ...
        async content(_event, trigger, player) {
          trigger.num +=
            trigger.player.storage[`new_yijue_${player.playerid}`][0]
        },
      },
      ban: {
        charlotte: true,
  ...
  rejizhi: {
    ...
    async content(event, _trigger, player) {
      ...
      const result2 = await player
        .chooseBool(
          `是否弃置${get.translation(event.card)}并令本回合手牌上限+1？`,
        )
        .set(
          "ai",
          (_evt, player) =>
  ...
  liyu: {
    ...
    async content(event, trigger, player) {
      // step 0
      const result = await trigger.player
        .chooseTarget((_card, _player, target) => {
          var evt = _status.event.getParent()
          return (
            evt.player.canUse({ name: "juedou" }, target) &&
            target !== _status.event.player
  ...
  relianying: {
    ...
    ai: {
      threaten: 0.8,
      effect: {
        player_use(_card, player, _target) {
          if (player.countCards("h") === 1) {
            return [1, 0.8]
          }
        },
        ...
        while (true) {
          ...
          if (result?.bool) {
            ...
            game.addVideo("storage", tar, [
              "reyiji2",
              get.cardsInfo(tar.storage.reyiji2),
              "cards",
            ])
    ...
    async content(event, _trigger, player) {
      ...
      result = await player
        .chooseTarget(
          ...
          (_card, _player, target) => target.hasSex("male"),
        )
        .set("ai", (target) => {
          var att = get.attitude(_status.event.player, target)
  ...
  retuntian: {
    ...
    async content(_event, _trigger, player) {
      const judgeEvent = player.judge((_card) => 1)
  ...
  refangzhu: {
    ...
    async content(event, _trigger, player) {
      ...
      const next = player.chooseTarget(
        ...
        (_card, player, target) => player !== target,
      )
      next.ai = (target) => {
        if (target.hasSkillTag("noturn")) {
          return 0
