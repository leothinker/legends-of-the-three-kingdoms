...
const skills = {
      ...
      event.result = await player
        ...
        .set("ai", (card) => {
          ...
          if (
            !game.hasPlayer(
              (target) =>
                target != player && get.damageEffect(target, player, player, "thunder") > 0,
            )
          ) {
            return 0
          }
      ...
      event.result = await trigger.player
        .chooseBool({
          ...
          prompt2:
            ...
            (player === trigger.player
              ? ""
              : "，将所有手牌交给" + get.translation(player) + "，然后其可以交给你等量张牌"),
          ai() {
        ...
        ai: {
          ...
          skillTagFilter(player, tag, arg) {
            if (
              ...
              !arg.card ||
              !arg.target ||
              (arg.card.name != "sha" && get.type(arg.card, null, false) != "trick")
            ) {
              return false
            }
            for (var i = 2; i < 6; i++) {
          ...
          return game
            .filterPlayer((current) => {
              ...
              if (
                evt &&
                evt.cards2 &&
                evt.cards2.some((i) => get.subtypes(i).some((slot) => keys.includes(slot)))
              ) {
                return true
              }
            })
            ...
            .set("ai", function (button) {
              ...
              if (four) {
                return (
                  ...
                  (given.map((i) => get.suit(i)).includes(get.suit(button.link)) ? 0 : 10)
                )
              }
    ...
    subSkill: {
      gain: {
        ...
        filter(event, player) {
          ...
          var filter = (card) =>
            card.name == "juedou" || (card.name == "sha" && get.color(card, false) == "red")
  ...
  olbeige: {
    ...
    filter(event, player) {
      return (
        event.card && event.card.name == "sha" && event.player.isIn() && player.countCards("he") > 0
      )
    },
    check(event, player) {
    ...
    async content(event, trigger, player) {
      ...
      if (result.control != "cancel2") {
        if (result.control == "选项一") {
          ...
          if (result.bool) {
            await player
              .useCard({ name: "sha" }, result.cards, "rezhuhai", trigger.player, false)
              .forResult()
          }
        } else {
          await player
            .useCard({ name: "guohe", isCard: true }, trigger.player, "rezhuhai")
            .forResult()
        }
      }
    },
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          ...
          var eff_target =
            get.value(target2.getCards("e"), target) - get.value(target.getCards("e"), target)
    ...
    async content(event, trigger, player) {
      ...
      if (
        ...
        result = await player
          ...
          .set("ai", function (target) {
            return (
              -get.attitude(_status.event.player, target) * Math.sqrt(1 + target.countCards("he"))
            )
          })
          .forResult()
      } else {
    ...
    async content(event, trigger, player) {
      ...
      while (player.isIn() && target.isIn()) {
        ...
        if (
          ...
          const result = await player
            .chooseBool("是否继续发动【樵拾】？", `和${get.translation(target)}各摸一张牌`)
            .forResult()
          if (!result?.bool) {
            break
          }
    ...
    check(trigger, player) {
      ...
      if (
        !game.hasPlayer(function (current) {
          return (
            current != player && get.attitude(player, current) > 0 && !current.hasSkillTag("nogain")
          )
        })
      ) {
        return false
                ...
                if (
                  ...
                  (cardx.name == "juedou" ||
                    (get.tag(card, "respondSha") &&
                      target.countCards("h", "sha") < player.countCards("h", "sha")))
                ) {
                  return 2
                }
                if (
  ...
  reshenxing: {
    ...
    prompt() {
      return (
        "弃置" +
        get.cnNumber(Math.min(2, _status.event.player.countMark("reshenxing_used"))) +
        "张牌并摸一张牌"
      )
    },
    check(card) {
  ...
  rebingyi: {
    ...
    async cost(event, trigger, player) {
      const selfDraw =
          get.info(event.skill).filterx(player) && get.info(event.skill).filtery(player),
  ...
  xinquanji: {
    ...
    async content(event, trigger, player) {
      ...
      const result =
        hs.length == 1
          ? { bool: true, cards: hs }
          : await player.chooseCard("h", true, "选择一张手牌作为“权”").forResult()
      if (result?.bool && result?.cards?.length) {
  ...
  decadezhenjun: {
    ...
    async content(event, trigger, player) {
      ...
      if (discardResult.cards && discardResult.cards.length) {
        ...
        if (cardNum > 0) {
          const prompt =
            "弃置一张牌，或令" + get.translation(target) + "摸" + get.cnNumber(cardNum) + "张牌"
          const result = await player
    ...
    async content(event, trigger, player) {
      ...
      if (result.index == 0) {
        if (event.vcards.length == 1) {
          ...
          result = await target
            .chooseButton(
              ["请选择要对" + get.translation(event.target2) + "使用的牌", [event.vcards, "vcard"]],
              true,
            )
            .set("ai", function (button) {
              var player = _status.event.player
      ...
      effect: {
        ...
        async content(event, trigger, player) {
          ...
          const card = get.discardPile(
            (card) => get.type(card) == "equip" && player.canEquip(card) && !get.tag(card, "gifts"),
          )
          if (card) {
  ...
  rechunlao2: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseCardButton(get.translation("rechunlao"), player.getExpansions("rechunlao"), true)
        .forResult()
      // step 1
      if (result.bool) {
        player.logSkill("rechunlao")
    ...
    async contentBefore(event, trigger, player) {
      ...
      if (list.length === 1) {
        ...
        chooseRes = await player
          .chooseTarget(true, "请选择获得所有拼点牌的角色", (card, pl, target) =>
            _status.event.list.includes(target),
          )
          .set("list", list)
          .forResult()
        if (!chooseRes?.bool) {
  ...
  rehuashen: {
    ...
    async cost(event, trigger, player) {
      ...
      const prompt =
        "###" +
        get.prompt(event.skill) +
        '###<div class="text center">替换当前化身牌或制衡至多两张其他化身牌</div>'
      const result = await player
    ...
    async content(event, trigger, player) {
      ...
      if (choice === "替换当前化身") {
        ...
        if (choosed.length > 0 || groups.length > 1) {
          dialog.style.bottom =
            (parseInt(dialog.style.top || "0", 10) + get.is.phoneLayout() ? 230 : 220) + "px"
      ...
      while (true) {
        ...
        if (choice === "制衡其他化身") {
          ...
          next.set("ai", (button) => {
            const { player, cond } = get.event()
            let skills = player.storage.rehuashen.character
              .map((i) => get.character(i).skills)
              .flat()
            skills.randomSort()
            skills.sort((a, b) => get.skillRank(b, cond) - get.skillRank(a, cond))
  ...
  reguhuo: {
    ...
    hiddenCard(player, name) {
      return (
        lib.inpile.includes(name) && player.countCards("h") > 0 && !player.hasSkill("reguhuo_used")
      )
    },
    filter(event, player) {
        ...
        .set("ai", (card) => {
          ...
          if (attitude == 0 || result == 0) {
            ...
            if (
              ...
              var checkx =
                lib.skill.xinleiji.judgeCheck(card, true) - lib.skill.xinleiji.judgeCheck(judging)
              if (checkx > 0) {
                return checkx
              }
            }
  ...
  new_liyu: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        !game.hasPlayer(
          (current) =>
            current != player && current != trigger.player && player.canUse("juedou", current),
        )
      ) {
        return
      }
  ...
  reqicai: {
    ...
    mod: {
      ...
      canBeDiscarded(card, player, target) {
        if (
          get.position(card) == "e" &&
          get.subtypes(card).some((subtype) => ["equip2", "equip5"].includes(subtype)) &&
          player != target
        ) {
          return false
        }
  ...
  rerende: {
    ...
    filter(event, player) {
      return (
        player.countCards("h") &&
        game.hasPlayer((current) => get.info("rerende").filterTarget(null, player, current))
      )
    },
    filterTarget(card, player, target) {
      if (player == target) {
      ...
      if (num < 2 && num + cards.length > 1 && list.length) {
        const result = await player
          ...
          .set("ai", (button) => {
            return get
              .player()
              .getUseValue({ name: button.link[2], nature: button.link[3], isCard: true })
          })
          .forResult()
        if (!result?.links?.length) {
          return
        ...
        await player.chooseUseTarget(
          get.autoViewAs({ name: result.links[0][2], nature: result.links[0][3], isCard: true }),
          true,
        )
      }
    },
  ...
  relianying: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseTarget(get.prompt("relianying"), "令至多" + get.cnNumber(num) + "名角色各摸一张牌", [
          1,
          num,
        ])
        .set("ai", function (target) {
  ...
  reqiaomeng: {
    ...
    filter(event, player) {
      ...
      return (
        event.card && event.card.name == "sha" && event.player.countDiscardableCards(player, "hej")
      )
    },
    async content(event, trigger, player) {
      // step 0
