...
const skills = {
  ...
  rende: {
    ...
    check(card) {
      ...
      const num = player.storage.rende
      if (
  ...
  jijiang: {
    ...
    filter(event, player) {
      ...
      return (
        !event.jijiang &&
        (event.type !== "phase" || !player.hasSkill("jijiang2"))
      )
    },
  ...
  kongcheng: {
    mod: {
      targetEnabled(card, _player, target, _now) {
  ...
  qianxun: {
    mod: {
      targetEnabled(card, _player, _target, _now) {
  ...
  xiaoguo: {
    ...
    async content(_event, trigger, player) {
      ...
      const { bool } = await target
        .chooseToDiscard("he", "弃置一张装备牌，或受到1点伤害", {
          type: "equip",
        })
        .set("ai", (card) => {
