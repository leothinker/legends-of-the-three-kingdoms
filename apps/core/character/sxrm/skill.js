...
const skills = {
          ...
          if (event.name === "damage") {
            return game.hasPlayer(
              (target) =>
                ...
                target.getStorage("sxrmxianger_mark")?.some((i) => i.target === player),
            )
          }
          return true
        },
          ...
          while (true) {
            ...
            if (bool) {
              ...
              const targets = player
                ...
                .filter((current) => current.countGainableCards(event.current, "he"))
              if (!targets.length) {
                return
              }
              const result =
        ...
        event.result = await player
          ...
          .set("ai", (target) => {
            ...
            if (!ui.selected.targets.length) {
              if (
                player == target &&
                ((player.isDamaged() && player.hasValueTarget({ name: "tao" })) ||
    ...
    ai: {
      ...
      order(item, player) {
        const num = game
          ...
          .reduce(
            (sum, current) =>
              sum + current.countConnectedCards() * -get.sgnAttitude(player, current),
            0,
          )
        if (num > 0) {
          return 10
  ...
  sxrmkongzhi: {
    ...
    filter(event, player, name) {
      if (name == "useCard") {
        return (
          player.isDamaged() && get.type(event.card) == "trick" && event.targets?.includes(player)
        )
      }
      return (
        event.card?.name == "sha" &&
  ...
  sxrmbizha: {
    ...
    async content(event, trigger, player) {
      ...
      const result =
        targets.length > 1
          ? await player
              .chooseTarget("鄙诈：与一名角色拼点", true, (card, player, target) =>
              ...
              if (link == 1) {
                ...
                if (
                  ...
                  game.hasPlayer(
                    (current) =>
                      current != player &&
                      current.countCards("h", (cardx) => !cards?.includes(cardx)) > cards.length &&
        ...
        target(card, player, target) {
          if (
            ...
            let gain = game.countPlayer((current) => {
              ...
              if (
                current.hasCard(
                  (cardx) => lib.filter.canBeGained(cardx, target, current, "sxrmkuxin"),
                  "h",
                )
              ) {
                return 0.9
        ...
        filter(event, player) {
          ...
          return (
            player.getExpansions("sxrmshefu_effect").some((card) => {
              return (
                get.color(card) == get.color(event.card) && get.type2(card) == get.type2(event.card)
              )
            }) &&
