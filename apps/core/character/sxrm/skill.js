...
const skills = {
        ...
        .chooseBool({
          ...
          ai() {
            ...
            return player.hasCards(
              "h",
              (card) => get.tag(card, "damage") && player.getUseValue(card),
            )
          },
        })
        .forResult()
  ...
  sxrmxianger: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        !player.getStorage("sxrmxianger_mark")?.some((i) => i.target === target)
      ) {
        const tmp = { target, damage: 0 }
      ...
      limit: {
        ...
        filter(event, player) {
          if (event.name === "damage") {
            return game.hasPlayer(
              (target) =>
                target.hasSkill("sxrmxianger") &&
                target
                  .getStorage("sxrmxianger_mark")
                  ?.some((i) => i.target === player),
            )
          }
          return true
        },
        onremove(player, skill) {
          ...
          for (const target of targets) {
            const item = target.storage.sxrmxianger_mark.find(
              (i) => i.target === player,
            )
        ...
        async content(event, trigger, player) {
          if (trigger.name === "damage") {
            for (const target of game.filterPlayer(
              ...
              target.storage.sxrmxianger_mark.find(
                (i) => i.target === player,
              ).damage += trigger.num
              target.markSkill("sxrmxianger_mark")
            }
    ...
    ai: {
      ...
      result: {
        target(player, target) {
          if (
            player.hasSkill("sxrmxianger_limit") ||
            get.attitude(player, target) > 0 ||
            player === target ||
            !player.storage.sxrmjiehuo_mark
          ) {
            return 0
          }
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseTarget({
          ...
          filterTarget(card, player, target) {
            return target.hasCards("e", (card) =>
              game.hasPlayer(
                (current) => current !== target && current.canEquip(card),
              ),
            )
          },
          ai(target) {
            ...
            if (val > 0) {
              ...
              if (
                game.hasPlayer(
                  (current) =>
                    get.attitude(player, current) > 0 &&
                    target.hasCards("e", (card) => current.canEquip(card)) &&
                    current !== player,
                )
              ) {
                val = -val + 3
      ...
      let result = await player
        .choosePlayerCard({
          ...
          ai(button) {
            ...
            const player = get.player()
            const val = get.value(button.link, from)
    ...
    subSkill: {
      ban: {
        ...
        mod: {
          cardEnabled(card, player) {
            if (
              player.getStorage("sxrmjiaozong_ban")?.includes(get.color(card))
            ) {
              return false
            }
          },
  ...
  sxrmbiyi: {
    ...
    locked: false,
    init(player, skill) {
      const lostSkill = player.storage.sxrmfusui_skill
      if (lostSkill) {
    ...
    filter(event, player) {
      ...
      const currentSkill = !player.storage.sxrmbiyi
        ? player.storage.sxrmfusui_skill
        : "xiaoji"
      if (!currentSkill) {
        return false
      }
    ...
    async content(event, trigger, player) {
      ...
      const skill = !player.storage.sxrmbiyi
        ? player.storage.sxrmfusui_skill
        : "xiaoji"
      if (skill) {
  ...
  sxrmxiezhong: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        count1 === count2 ||
        !game.hasPlayer((target) => !targets.includes(target))
      ) {
        return
      }
  ...
  sxrmwanli: {
    ...
    filter(event, player) {
      return (
        game.roundNumber === 1 &&
        !player.getStorage("sxrmwanli")?.length &&
        game.countPlayer() > 1
      )
    },
    async cost(event, trigger, player) {
    ...
    subSkill: {
      eff: {
        ...
        async content(event, trigger, player) {
          ...
          for (const i in items) {
            ...
            for (const j in player.storage.sxrmwanli_eff) {
        ...
        intro: {
          content(storage) {
            ...
            for (const item of storage) {
              const { target, cards: num, round, huanqian } = item
      ...
      draw: {
        ...
        filter(event, player) {
          ...
          const num = player.getStorage("sxrmwanli_eff").reduce((sum, item) => {
      ...
      if (result2.winner === player) {
        ...
        for (const i in player.storage.sxrmwanli_eff) {
          if (player.storage.sxrmwanli_eff[i].target === target) {
            const num = player.storage.sxrmwanli_eff[i].round
            player.storage.sxrmwanli_eff[i].round = Math.max(
              game.roundNumber,
              num - 1,
            )
          }
  ...
  sxrmlanjiao: {
    ...
    filterTarget(card, player, target) {
      return (
        target !== player &&
        !player.getStorage("sxrmlanjiao_used")?.targets?.includes(target) &&
    ...
    async content(event, trigger, player) {
      ...
      while (1) {
        ...
        await player.showCards(
          cards,
          `${get.translation(player)}对${get.translation(target)}发动了【揽娇】`,
        )
        const rechoose = await target
          .chooseBool({
      ...
      const suits = player.storage.sxrmlanjiao_used.suits
    ...
    subSkill: {
      used: {
        ...
        intro: {
          content(storage, player) {
            if (!storage) return ""
            const { targets, suits } = storage
            const str1 = targets.map((i) => get.translation(i)).join("、"),
  ...
  sxrmbingqu: {
    ...
    getVcards(player) {
      ...
      const vcard = list
        ...
        .map((namex) => {
          const name = Array.isArray(namex) ? namex.name : namex
    ...
    async content(event, trigger, player) {
      const target = event.targets[0]
      const vcard1 = lib.skill.sxrmbingqu.getVcards(target)
      const vcard2 = lib.skill.sxrmbingqu.getVcards(player)
      const targetCard = {}
      for (const i of [
      ...
      for (const playerx of [player, target]) {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseTarget({
          ...
          filterTarget: (card, player, target) => {
            return (
              target !== player && !player.storage.sxrmfanxin?.includes(target)
            )
          },
          ai: (target) => {
            const player = get.player()
    ...
    subSkill: {
      draw: {
        ...
        filter(event, player) {
          ...
          return (
            player.storage.sxrmfanxin?.includes(target) &&
            target.hasSkill("baonu") &&
            target.countMark("baonu")
          )
        },
        async cost(event, trigger, player) {
          const target = trigger.player,
            num = target.countMark("baonu")
          const list = Array.from({ length: num }, (_, i) => String(i + 1))
          list.push("cancel2")
          console.log(list)
          const result = await player
            .chooseControl({
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseNumbers({
          ...
          processAI: () => {
            const list = Array.from({ length: get.event("range0") }).map(
              (val, idx) => idx + 1,
            )
            if (list.length >= 2) {
              list.remove(1)
    ...
    subSkill: {
      ...
      clear: {
        ...
        filter(event, player) {
          if (!player.storage.sxrmfubei_damage?.length) return false
          const pile = Array.from(ui.cardPile.childNodes)
          return player.storage.sxrmfubei_damage.some(
            (i) => pile.indexOf(i) === -1,
          )
        },
        async content(event, trigger, player) {
          const cards = player.getStorage("sxrmfubei_damage")
  ...
  sxrmzhaduo: {
    ...
    async content(event, trigger, player) {
      ...
      const targetsx = targets.filter((i) => {
        return (
          i.isIn() && player.canUse({ name: "sha", isCard: true }, i, false)
        )
      })
      if (!targetsx.length) {
        return
      ...
      if (
        !target2.isIn() ||
        !target2.canUse({ name: "juedou", isCard: true }, player)
      ) {
        return
      }
  ...
  sxrmyinfeng: {
    ...
    async content(event, trigger, player) {
      ...
      player.when({ player: "phaseBegin" }).then(() => {
        ...
        for (const i of player.storage.sxrmyinfeng) {
          if (
            !i.isIn() ||
            !i.hasSkill("sxrmyinfeng_debuff") ||
            !i.storage.sxrmyinfeng_debuff
          ) {
            continue
          }
          player.line(i)
  ...
  sxrmhanguo: {
    ...
    async content(event, trigger, player) {
      ...
      target.getStorage(skill).forEach((current) => {
        target.addTip(
          `${skill}_${current.playerid}`,
          `撼国 ${get.translation(current)}`,
        )
      })
    ...
    getSelected(player) {
      ...
      return historys.flatMap((evt) => evt.targets || [])
    },
    init(player, skill) {
    ...
    subSkill: {
      ...
      hujia: {
        ...
        filter(event, player) {
          ...
          if (
            !event.filterCard(
              get.autoViewAs({ name: "shan", isCard: true }),
              player,
              event,
            )
          ) {
          ...
          while (true) {
            ...
            if (bool) {
              ...
              const targets = player
                ...
                .filter((current) =>
                  current.countGainableCards(event.current, "he"),
                )
              if (!targets.length) {
                return
              }
              const result =
                targets.length > 1
                  ? await event.current
                      .chooseTarget(
                        `撼国：获得${get.translation(targets)}中的一名角色一张牌`,
                        true,
                        (card, player, target) => {
  ...
  sxrmweiwo: {
    ...
    subSkill: {
      rende: {
        ...
        init(player, skill) {
          player.getStorage(skill).forEach((current) => {
            player.addTip(
              `${skill}_${current.playerid}`,
              `${get.translation(skill)} ${get.translation(current)}`,
            )
          })
        },
        onremove(player, skill) {
        ...
        filterTarget(card, player, target) {
          ...
          return (
            player !== target &&
            player.getStorage("sxrmweiwo_rende").includes(target)
          )
        },
        check(card) {
          ...
          player.getHistory("lose", (evt) => {
            if (
              evt.getParent().skill === "sxrmweiwo_rende" &&
              evt.getParent(3) === evt2
            ) {
        ...
        async content(event, trigger, player) {
          ...
          player.getHistory("lose", (evt) => {
            if (
              evt.getParent(2).name === "sxrmweiwo_rende" &&
              evt.getParent(5) === evt2
            ) {
              num += evt.cards.length
            }
          })
      ...
      longyin: {
        ...
        filter(event, player) {
          if (
            player.getStorage("sxrmweiwo_longyin").length &&
            !player.getStorage("sxrmweiwo_longyin").includes(event.player)
          ) {
            return false
          }
          return (
    ...
    subSkill: {
      global: {
        ...
        filter(event, player) {
          if (
            !player.hasSex("male") ||
            !player.countCards("h", (card) => !get.is.connectedCard(card))
          ) {
            return false
          }
          return game.hasPlayer((current) => {
            return (
              current.hasSkill("sxrmyinmou") &&
              current.countCards("h", (card) => !get.is.connectedCard(card))
            )
          })
        },
        async cost(event, trigger, player) {
          event.result = await player
            .chooseTarget(
              get.prompt("sxrmyinmou"),
              "连接你与一名拥有〖姻谋〗的角色各一张未连接的手牌",
              (card, player, target) => {
                return (
                  target.hasSkill("sxrmyinmou") &&
                  target.countCards("h", (card) => !get.is.connectedCard(card))
                )
              },
            )
            .set("ai", (target) => {
        ...
        async content(event, trigger, player) {
          ...
          for (const current of [target, player].sortBySeat().toUniqued()) {
            const cards = current.getCards(
              "h",
              (card) => !get.is.connectedCard(card),
            )
  ...
  sxrmsuwu: {
    ...
    filter(event, player) {
      if (event.name === "useCard") {
        ...
        const evts = event.player.getHistory("useCard", (evt) =>
          ...
          .set("ai", (target) => {
            ...
            if (!ui.selected.targets.length) {
              if (
                player === target &&
                ((player.isDamaged() &&
                  player.hasValueTarget({ name: "tao" })) ||
  ...
  sxrmrenwang: {
    ...
    ai: {
      ...
      order(item, player) {
        const num = game
          .filterPlayer((current) => player !== current)
          .reduce(
            (sum, current) =>
              sum +
              current.countConnectedCards() * -get.sgnAttitude(player, current),
            0,
          )
        if (num > 0) {
          return 10
  ...
  sxrmhuoe: {
    ...
    filter(event, player) {
      ...
      return game.hasPlayer(
        (current) => current !== player && player.canUse(card, current),
      )
    },
    async cost(event, trigger, player) {
      const card = new lib.element.VCard({ name: "huogong", isCard: true })
    ...
    async content(event, trigger, player) {
      ...
      const cards = game
        .getGlobalHistory("everything", (evt) => {
          return (
            evt.name === "showCards" &&
            evt.getParent((evtx) => evtx === next, true)
          )
        })
  ...
  sxrmhuaibing: {
        ...
        .set("ai", (target) => {
          ...
          if (ui.selected.targets.length) {
            ...
            if (first.hp !== target.hp) {
              ...
              return (
                eff +
                get.effect(current, { name: "wuzhong" }, current, player) *
                  count
              )
            }
          }
    ...
    async content(event, trigger, player) {
      const gains = event.targets.filter(
      ...
      const num =
          event.targets.reduce((sum, current) => sum + current.getHp(), 0) / 2,
      ...
      for (const key of ["Draw", "Use", "Discard"]) {
        current.addTempSkill(`${event.name}_${key}`, {
          player: `phase${key}After`,
        })
  ...
  sxrmchanyu: {
    ...
    async content(event, trigger, player) {
      ...
      ;[player, target].forEach((current) => {
        const num = Math.min(
          ...current.getCards("h").map((card) => get.number(card, current)),
        )
        current.addGaintag(
      ...
      if (result?.winner) {
        ...
        const dialog = ui.create.dialog(
          "谄谀：是否交换其中一种颜色或类别的所有手牌？",
          "你的手牌",
          winner.getCards("h"),
          `${get.translation(loser)}的手牌`,
          loser.getCards("h"),
        )
          ...
          .set(
            ...
            (() => {
              ...
              const getV = (key) => {
                ...
                const cards1 = winner.getCards("h", (card) =>
                    getFilter(card, key),
                  ),
                  cards2 = loser.getCards("h", (card) => getFilter(card, key))
                const sum1 = cards1.reduce((sum, card) => {
                  sum += get.value(card, winner)
                  return sum
                }, 0)
                const sum2 = cards2.reduce((sum, card) => {
  ...
  sxrmcongfeng: {
    ...
    check(event, player) {
      const bool = player.getStorage("sxrmcongfeng", false),
        getV = (current, name) =>
          get.effect(current, { name: name }, player, player)
      if (bool) {
        ...
        if (result2?.winner !== target) {
          ...
          while (num < 2 && target?.isIn() && target.countCards("h")) {
            const result3 = await player
              .chooseButton([
                `鄙诈：是否使用其中一张点数小于${result2.num2}的牌？（${num}/2）`,
                target.getCards("h"),
              ])
              .set("filterButton", (button) => {
  ...
  sxrmnuozhan: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        ...
        .set("ai", (target) =>
          get.damageEffect(target, get.player(), get.player()),
        )
        .forResult()
    },
    async content(event, trigger, player) {
      ...
      while (target?.isIn()) {
        ...
        const result = await target
          ...
          .set("ai", (button) => {
            ...
            const eff = Math.max(
              get.effect(player, card, kuangtu, kuangtu),
              get.effect(kuangtu, card, player, kuangtu),
            )
            return eff
          })
          .set("kuangtu", player)
        ...
        const targets = [player, target].filter((current) =>
          current.canUse(card, current === player ? target : player, false),
        )
        if (!targets?.length) {
          return
        }
    ...
    async content(event, trigger, player) {
      ...
      if (list.length) {
          ...
          .chooseButtonTarget({
            createDialog: [
              ...
              [
                (dialog) => {
                  ...
                  dialog.buttons = dialog.buttons.filter(
                    (button) => typeof button.link === "number",
                  )
                },
                "handle",
              ],
            ...
            ai1(button) {
              ...
              if (link === 1) {
                if (
                  player.isTurnedOver() &&
                  cards.some((card) => get.suit(card, false) === "heart")
                ) {
                  return 2
                }
                if (
        ...
        if (link === 0) {
          ...
          player.line(target)
          gains = target.getCards(
            "h",
            (card) =>
              !list.flatMap((i) => i[0]).includes(card) &&
              lib.filter.canBeGained(card, target, player),
          )
        }
        ...
        target(player, target) {
          ...
          if (eff <= 0) {
            ...
            if (
              ...
              target.hp + target.hujia >= 5 &&
              eff >= -2 &&
              player.hasCards("he", (card) =>
                numbers.includes(get.number(card)),
              )
            ) {
              return 0.1
            }
          ...
          return (
            (-eff * get.threaten(target)) /
            Math.sqrt(target.hp + target.hujia + 1) /
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        .chooseButton(
          [
            "纵横：展示并获得其中一张",
            `###${get.translation(targets[0])}的手牌###`,
            targets[0].getCards("h"),
            `###${get.translation(targets[1])}的手牌###`,
            targets[1].getCards("h"),
          ],
          true,
        )
        .set("targets", targets)
        .set("ai", (button) => {
          ...
          let eff1 = get.value(button.link),
            eff2 = other
              ? get.effect(other, { name: "guohe_copy2" }, player, player)
              : 0
          if (other) {
            eff2 *= Math.min(
              3,
      ...
      if (other) {
        ...
        const result = await player
          ...
          .set("list", [
            ...
            [
              [`花色为${get.translation(suit)}`],
              [`点数为${get.translation(num)}`],
              [`类型为${get.translation(type)}`],
            ],
          ])
          .set("filterOk", (moved) => {
            const list = [null, "suit", "number", "type2"]
            for (let i = 1; i < 4; i++) {
              const key = list[i]
              if (
                moved[i].some((card) => get[key](card) !== get.event()[key]) ||
                moved[i].length > 1
              ) {
                return false
              }
          ...
          .set("filterMove", (from, to, moved) => {
            ...
            if (typeof to === "number") {
              if (to !== 0) {
                return (
                  moved[to].length < 1 &&
                  get[list[to]](from.link) === get.event()[list[to]]
                )
              }
              return true
            }
            const num1 = [0, 1, 2, 3].find((i) => moved[i].includes(from.link)),
              num2 = [0, 1, 2, 3].find((i) => moved[i].includes(to.link))
            if (
              num1 !== 0 &&
              get[list[num1]](to.link) !== get.event()[list[num1]]
            ) {
              return false
            }
            if (
              num2 !== 0 &&
              get[list[num2]](from.link) !== get.event()[list[num2]]
            ) {
              return false
            }
            return true
          ...
          .set("processAI", (list) => {
            ...
            for (let i = 0; i < keys.length; i++) {
              const key = keys[i]
              const card = cardx.find(
                (j) => !cards.includes(j) && get[key](j) === get.event()[key],
              )
              if (card) {
                cards.add(card)
                discards[i].add(card)
  ...
  sxrmduibian: {
    ...
    async content(event, trigger, player) {
      ...
      trigger.cancel()
      let bool =
        get.damageEffect(player, target, target) +
          get.effect(target, { name: "guohe_copy2" }, player, target) >
        0
      bool = Math.random() > 0.4 ? bool : false
      const result = await target
        .chooseBool(
          `对辩：是否令${get.translation(player)}弃置你一张牌，然后揭示拼点结果？`,
        )
        .set("choice", bool)
        .forResult()
      if (result.bool) {
        await player.discardPlayerCard(target, "he", true)
        const result2 = await game
          .createEvent("chooseToCompare", false)
          .set("player", player)
          .set("parentEvent", next)
          .setContent("chooseToCompareEffect")
        ...
        .step(async (event, trigger, player) => {
          const targets = game.filterPlayer((current) => {
            return current.getHistory("lose", (evt) => {
              ...
              if (
                !evt.getParent((evt) => evt === trigger, true, true) ||
                !cards.some((card) => get.suit(card) === "spade")
              ) {
                return false
              }
              return evt.visible
      ...
      for (const targetx of targets) {
        ...
        targetx
          ...
          .step(async (event, trigger, player) => {
            ...
            const cards = player.getExpansions("sxrmshefu_effect")
            if (cards.length) {
          ...
          return (
            player.getExpansions("sxrmshefu_effect").some((card) => {
              return (
                ...
                get.type2(card) === get.type2(event.card)
              )
            }) &&
            event.player.getHistory(
              ...
              (evt) =>
                ...
                evt.hs &&
                evt.hs.length === event.cards.length,
            ).length
          )
        },
        async cost(event, trigger, player) {
          ...
          const result = await player
            ...
            .set("filterButton", (button) => {
              ...
              return (
                get.color(button.link) === get.color(used) &&
                get.type2(button.link) === get.type2(used)
              )
            })
            .set("ai", (button) => {
      ...
      for (const target of event.targets) {
        ...
        if (result.bool) {
          const targetx = result.targets[0],
            card = target
              .getCards("h")
              .find((card) => card.hasGaintag("sxrmmitu"))
          let bool =
            get.attitude(target, player) >= 0
              ? get.effect(targetx, { name: "sha" }, target, target) > 0
              : false
          if (
            card &&
            get.number(card) < 7 &&
          ...
          const result2 = await target
            .chooseBool(
              `是否与${get.translation(targetx)}进行拼点？`,
              "赢的角色视为对没赢的角色使用一张【杀】",
            )
            .set("choice", bool)
            .forResult()
      ...
      ai: {
        ...
        mod: {
          aiValue: (player, card, num) => {
            const evt = _status.event.getParent("sxrmmitu", true)
            if (!evt?.player || get.attitude(player, evt.player) <= 0) {
              return
            }
            if (
              num > 0 &&
              get.itemtype(card) === "card" &&
              card.hasGaintag("sxrmmitu")
            ) {
              return -114514
            }
          },
  ...
  sxrmqianliu: {
    ...
    filter(event, player) {
      return (
        get.distance(player, event.target) <= 1 && event.card?.name === "sha"
      )
    },
    frequent: true,
    logTarget: "target",
  ...
  sxrmhuice: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        game.hasPlayer(
          (current) => current !== event.target && player.canCompare(current),
        )
      ) {
        const result = await player
          .chooseTarget(
        ...
        const result2 = await player
          .chooseToCompare(result.targets[0])
          .forResult()
        if (result1 && result2) {
          if (result1.winner) {
        ...
        player(player, target) {
          const targets = game.filterPlayer((current) => {
            return (
              ...
              get.damageEffect(current, current, player) > 0
            )
          })
  ...
  sxrmyihe: {
    ...
    filter(event, player) {
      ...
      const bool1 = get.sgn(event.source.hp - event.source.countCards("h")),
    ...
    async content(event, trigger, player) {
      const bool1 = get.sgn(trigger.source.hp - trigger.source.countCards("h")),
  ...
  sxrmzhengsi: {
    ...
    filterTarget(card, player, target) {
      if (
        ...
        !ui.selected.targets.includes(player)
      ) {
        if (target !== player) {
          return false
        }
    ...
    async content(event, trigger, player) {
      ...
      const cards = result.cards.slice(0, 1)
      await target.showCards(result.cards)
      ...
      const max = cards.map((i) => get.number(i)).maxBy((i) => i),
  ...
  sxrmchengming: {
    ...
    getIndex(event, player) {
      ...
      if (
        bool &&
        player.countCards("h") ===
          targets.map((i) => i.countCards("h")).maxBy((i) => i)
      ) {
        result.push("recover")
      }
    ...
    filter(event, player, name, data) {
      ...
      return data.some(
        (i) => i !== player && i.countGainableCards(player, "he"),
      )
    },
  ...
  sxrmjugu: {
    ...
    async cost(event, trigger, player) {
      ...
      do {
        const result = await player
          .chooseTarget(
            forced ? "聚谷：是否继续选择牌？" : get.prompt2(event.skill),
          )
    ...
    subSkill: {
      return: {
        ...
        locked: false,
        async content(event, trigger, player) {
          for (const i of trigger.jugu) {
            const card = get.cardPile2((card) => card === i[1])
            if (card) {
      ...
      log: {
        ...
        filter(event, player) {
          if (
            ...
            event.getParent("sxrmjugu_return", true)
          ) {
            return false
          }
          ...
          return evt?.jugu
            ?.map((i) => i[1])
            .some((i) => event.getg(player).includes(i))
        },
        async content(event, trigger, player) {
