...
const skills = {
    ...
    yuheng: {
        ...
        filter(event, player) {
            return player.hasCard(function (card) {
                return lib.filter.cardDiscardable(card, player, "yuheng");
            }, "he");
        },
        content() {
            "step 0";
        ...
        subSkill: {
            remove: {
                ...
                forced: true,
                filter(event, player) {
                    return player.additionalSkills.yuheng && player.additionalSkills.yuheng.length > 0;
                },
    ...
    quandao: {
        ...
        async content(event, trigger, player) {
            ...
            if (cards1.length !== cards2.length) {
                ...
                let i = 0;
                cards.forEach(card => {
                    if (i < Math.abs(num) && lib.filter.cardDiscardable(card, player, "quandao")) {
                        i++;
                    }
                });
                if (i > 0) {
    ...
    chigang: {
        ...
        content() {
            ...
            let phase = player.storage.chigang ? "phaseDraw" : "phaseUse";
    ...
    lanjiang: {
        ...
        logTarget(event, player) {
            return game.filterPlayer(current => current.countCards("h") >= player.countCards("h")).sortBySeat();
        },
        async content(event, trigger, player) {
    ...
    diaodu: {
        ...
        async cost(event, trigger, player) {
            const next = player.chooseTarget(get.prompt2("diaodu"), (_card, player, current) => current.isFriendOf(player) && current.countGainableCards(player, "e") > 0);
    ...
    hongyuan: {
        ...
        filter(event, player) {
            if (!player.countCards("he") || player.hasSkill("hongyuan_blocker", null, null, false)) {
                return false;
            }
    ...
    guanwei: {
        ...
        onremove: () => {
            if (!game.hasPlayer(i => i.hasSkill("guanwei", null, null, false), true)) {
        ...
        subSkill: {
            ai: {
                ...
                filter: () => {
                    return !game.hasPlayer(i => i.hasSkill("guanwei", null, null, false), true);
                },
                silent: true,
                forceDie: true,
                content: () => {
                        ...
                        player_use(card, player, target) {
                            ...
                            var hasPanjun = game.hasPlayer(function (current) {
                                return (
                                    ...
                                    (!current.storage.counttrigger || !current.storage.counttrigger.guanwei) &&
                                    get.attitude(current, player) >= 1 &&
                                    current.hasCard(function (card) {
                                        return (
                                            get.value(card) < 7 ||
                                            (current != game.me && !current.isUnderControl() && !current.isOnline() && get.value(card) < 9)
                                        );
                                    }, "he")
                                );
                            });
    ...
    bingyi: {
        ...
        filter(event, player) {
            return event.type == "discard" && event.getl(player).cards2.length > 0 && player.countCards("h") > 0 && !player.hasSkill("bingyi_blocker", null, null, false);
        },
        ...
        content() {
            "step 0";
            player.addTempSkill("bingyi_blocker", ["phaseZhunbeiAfter", "phaseJudgeAfter", "phaseDrawAfter", "phaseUseAfter", "phaseDiscardAfter", "phaseJieshuAfter"]);
