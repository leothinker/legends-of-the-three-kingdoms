...
const skills = {
  ...
  ylygtianxiang: {
    ...
    async cost(event, trigger, player) {
      event.result = await player
        .chooseToDiscard(
          ...
          `弃置一张红桃牌防止${trigger.source ? `${get.translation(trigger.source)}造成的` : ""}伤害`,
          { suit: "heart" },
          "he",
          "chooseonly",
        )
    ...
    subSkill: {
      lose: {
        ...
        filter(event, player) {
          if (
            !["chooseToCompare", "equip"].includes(event.name) &&
            !event.visible
          ) {
            return false
          }
          return (
            get.info("ylygtianxiang_lose").getCards(event, player).length > 0
          )
        },
        getCards(event, player) {
          ...
          return cards.filter(
            (card) =>
              get.suit(card, player) === "heart" && get.position(card) === "d",
          )
        },
        async cost(event, trigger, player) {
          ...
          event.result = await player
            ...
            .set("createDialog", [
              ...
              [
                (dialog) =>
                  dialog.buttons.forEach((i) => (i.style.opacity = 1)),
                "handle",
              ],
            ])
            .set("ai", (target) => {
  ...
  ylyghongyan: {
    ...
    forced: true,
    async content(event, trigger, player) {
      const keys = ["花色", "颜色", "红色", "黑色"].concat(
        lib.suit.flatMap((i) => [get.translation(i), get.translation(`${i}2`)]),
      )
      const map = new Map()
      game
  ...
  ylygguhuo: {
    ...
    getList(event, player) {
      return get.inpileVCardList((info) => {
        if (
          !["basic", "trick"]
            .removeArray(player.getStorage("ylygguhuo_used"))
            .includes(info[0])
        ) {
          return false
        }
    ...
    filter(event, player) {
      return (
        player.countCards("hs") &&
        get.info("ylygguhuo").getList(event, player).length
      )
    },
    chooseButton: {
      dialog(event, player) {
        ...
        return {
          ...
          ai1(card) {
            ...
            if (enemyNum) {
              ...
              if (lib.skill.ylygguhuo_backup.aiUse < 0.5 && !player.isDying()) {
                return 0
              }
            }
    ...
    ai: {
      ...
      skillTagFilter(player) {
        if (
          !player.countCards("hs") ||
          player.getStorage("ylygguhuo_used").includes("basic")
        ) {
          return false
        }
      },
    ...
    subSkill: {
      guess: {
        ...
        async content(event, trigger, player) {
          ...
          const next = player
            .lose(card, ui.ordering)
            .set("relatedEvent", trigger)
          await next
          trigger.throw = false
          ...
          const doubter = targets.filter((target) => target.hasSkill("huinu"))
          const acceptor = []
          game.broadcastAll(
            (card, player) => {
            ...
            if (targets.length) {
              ...
              if (giver.length) {
                ...
                await game.doAsyncInOrder(giver, async (target) =>
                  target.chooseToGive(player, "h", true),
                )
              }
            }
  ...
  ylygqiangxi: {
    ...
    filter(event, player) {
      return game.hasPlayer(
        (target) =>
          !event.ylygqiangxi_targets.includes(target) &&
          player.canCompare(target),
      )
    },
    filterTarget(card, player, target) {
      return (
        !get.event().ylygqiangxi_targets.includes(target) &&
        player.canCompare(target)
      )
    },
    async content(event, trigger, player) {
      ...
      if (result.bool) {
        ...
        const list = player.getHistory(
          "useSkill",
          (evt) => evt.skill === event.name && evt.event[event.name] === false,
        )
        if (list.length === 2) {
          await player.loseHp()
          ...
          prevs = prevs.filter(
            (target) => targets.includes(target) || target.isIn(),
          )
          nexts = nexts.filter(
            (target) => targets.includes(target) || target.isIn(),
          )
            ...
            if (prevs.length === nexts.length) {
              const getEff = (targets) =>
                targets
                  ...
                  .reduce(
                    (sum, target) =>
                      sum + get.damageEffect(target, player, player),
                    0,
                  )
      ...
      result: {
        target(player, target) {
          ...
          if (
            ...
            (player.getHp() < 2 &&
              player.getHp() +
                player.countCards("h", { name: ["tao", "jiu"] }) >
                2) ||
            (player.getHp() > 1 &&
              player.getHp() + player.countCards("h", { name: "tao" }) > 2)
          ) {
            return -1
          }
          return 0
  ...
  xiangxing_yingzi: {
    ...
    filter(event, player) {
      return (
        game.hasPlayer(
          (target) => target !== player && target.hasSkill("xiangxing"),
        ) && !event.numFixed
      )
    },
    mod: {
  ...
  ylygduanchang: {
    ...
    async content(event, trigger, player) {
      ...
      const result = await player
        ...
        .set("forceDie", true)
        .set("ai", () =>
          source.countCards("hej") > 3 ? "弃置所有牌" : "失去所有技能",
        )
        .forResult()
  ...
  ylygbeige: {
    ...
    async content(event, trigger, player) {
      ...
      if (
        ["heart", "diamond", "club", "spade"].every((suit) =>
          suits.includes(suit),
        )
      ) {
    ...
    intro: {
      markcount(storage) {
        ...
        return suits
          .map(([suit, recorded, missing]) =>
            storage?.includes(suit) ? recorded : missing,
          )
          .join("")
      },
      content(storage) {
        ...
        return suits
          .map(
            (suit) =>
              `${storage?.includes(suit) ? "已记录" : "未记录"}：${names[suit]}`,
          )
          .join("<br>")
      },
    },
  ...
  ylygjiang: {
    ...
    prompt2(event, player) {
      return event.player === player
        ? "你摸两张牌"
        : `你与${get.translation(event.player)}各摸一张牌`
    },
    async content(event, trigger, player) {
      if (trigger.player === player) {
        await player.draw(2)
  ...
  ylygzhiba: {
    ...
    filter(event, player) {
      ...
      return (
        player.hasZhuSkill("ylygzhiba") &&
        current.group === "wu" &&
        current.countCards("h") > 0 &&
        game.hasPlayer(
          (target) => target !== current && current.canCompare(target),
        )
      )
    },
    async content(event, trigger, player) {
      ...
      const choose = await player
        .chooseTarget({
          ...
          ai(target) {
            const current = get.event().current
            return (
              get.attitude(get.player(), current) *
              get.effect(target, { name: "juedou" }, current, get.player())
            )
          },
          current,
        })
