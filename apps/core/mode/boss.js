...
export default () => {
  return {
    ...
    start() {
      ...
      for (var i in lib.character) {
        ...
        if (info.isBoss) {
          ...
          if (
            lib.characterPack.mode_boss[i] &&
            get.config(`${i}_boss_config`) === false
          ) {
      ...
      setTimeout(() => {
        if (event.current) {
          var left =
            event.current.offsetLeft - (ui.window.offsetWidth - 180) / 2
        ...
        game.chooseCharacter((target) => {
          ...
          if (_status.bosschoice) {
            ...
            if (_status.bosschoice.link !== name) {
              lib.boss[_status.bosschoice.name].control(
                "cancel",
                _status.bosschoice,
              )
          ...
          if (lib.boss[target.name] && lib.boss[target.name].control) {
            ...
            _status.bosschoice.link =
              lib.boss[target.name].controlid || target.name
      ...
      if (lib.config.test_game) {
        event.current.classList.remove("highlight")
        if (event.current.nextSibling?.classList.contains("player")) {
    ...
    element: {
      player: {
        dieAfter() {
          ...
          if (
            game.bossinfo.checkResult &&
            game.bossinfo.checkResult(this) === false
          ) {
            return
          }
          if (
    ...
    card: {
      ...
      gubuzifeng: {
        ...
        content() {
          ...
          if (skills.length) {
            ...
            target.disableSkill(
              "gubuzifeng_disable",
              target.storage.gubuzifeng_disable,
            )
          }
        },
        ai: {
        ...
        ai: {
          ...
          result: {
            target(player, target) {
              ...
              if (
                target === player &&
                player.countCards("h", "sadouchengbing")
              ) {
                nh--
              }
              if (nh < num) {
      ...
      yihuajiemu: {
        ...
        content() {
          ...
          } else if (result.bool && result.cards?.length) {
    ...
    characterSubstitute: {
      boss_mingxingzhu: [
        [
          "boss_mingxingzhu_shadow",
          ["img:image/mode/boss/character/boss_mingxingzhu_shadow.jpg"],
        ],
      ],
    },
    game: {
            ...
            if (player.chongzheng) {
              ...
              if (player.maxHp > 0 && game.bossinfo.chongzheng) {
                if (player.hp < player.maxHp) {
                  ...
                  var position =
                    sort > 0 ? player.node.handcards1 : player.node.handcards2
                  card.fix()
                ...
                player.update()
                if (
                  player.storage.boss_chongzheng >= game.bossinfo.chongzheng
                ) {
      ...
      chooseCharacter(func) {
        ...
        next.setContent(() => {
          ...
          next.filterButton = (button) => {
            const { current } = get.event().getParent("game")
            if (current) {
              const { name } = current
              const banrule = _status.banlist[name]
              if (!banrule) {
                return true
              }
              const { link } = button
            ...
            while (event.enemy.length < 3) {
              var name = event.list
                .filter((name) => {
                  ...
                  const banrule = _status.banlist[current?.name]
                  if (!banrule) {
                    return true
                  }
                  if (Array.isArray(banrule)) {
                ...
                .randomRemove()
              if (
                lib.boss[lib.storage.current] &&
                lib.boss[lib.storage.current].randchoice
              ) {
                name = lib.boss[lib.storage.current].randchoice(
                  name,
                  event.enemy,
                )
              }
    ...
    boss: {
      ...
      boss_taowu: {
          ...
          if (type === "cancel") {
            ...
            var control = ui.create.control("神将", function () {
              ...
              if (this.classList.contains("glow")) {
                ...
                dialog.content.insertBefore(
                  this.backup1,
                  dialog.buttons[0].parentNode,
                )
        ...
        init() {
          ...
          const defaultMap = [
            [
              "关羽",
              { replace: "qinglong", card: ["guilongzhanyuedao", "spade", 5] },
            ],
            ["诸葛亮", { replace: "bagua", card: ["qimenbagua", "spade", 2] }],
            [
              "周瑜",
              { replace: "zhuque", card: ["chiyanzhenhunqin", "diamond", 1] },
            ],
            [
              "曹操",
              { replace: "jueying", card: ["juechenjinge", "spade", 5] },
            ],
            [
              "赵云",
              { replace: "qinggang", card: ["chixueqingfeng", "spade", 6] },
            ],
            [
              "吕布",
              { replace: "fangtian", card: ["xiuluolianyuji", "diamond", 12] },
            ],
            ["司马懿", { card: ["xuwangzhimian", "diamond", 4] }],
            [
              "刘备",
              { replace: "fangtian", card: ["longfenghemingjian", "spade", 2] },
            ],
            ["吕蒙", { card: ["guofengyupao", "diamond", 3] }],
            ["陆逊", { card: ["qicaishenlu", "diamond", 3] }],
            [
              "甘宁",
              { replace: "qilin", card: ["jinwuluorigong", "heart", 5] },
            ],
            [
              "张辽",
              { replace: "guanshi", card: ["xingtianpojunfu", "diamond", 5] },
            ],
          ...
          while (cards.length > 0) {
            ui.cardPile.insertBefore(
              ...
              ui.cardPile.childNodes[
                get.rand(0, ui.cardPile.childElementCount - 1)
              ],
            )
          }
      ...
      boss_nianshou: {
        ...
        init() {
          ...
          _status.additionalReward = () =>
            Math.round(_status.damageCount ** 2.4) * 2
          var time = 360
          var interval = setInterval(() => {
            ...
            if (time <= 0) {
              ...
              if (typeof _status.coin === "number") {
                if (game.me === game.boss) {
                  _status.coin += Math.round(_status.damageCount ** 2.4)
                } else {
                  _status.coin += Math.round(_status.damageCount ** 1.8)
                }
              }
              game.forceOver(true)
              clearInterval(interval)
    ...
    characterIntro: {
      ...
      boss_niutou:
        "牛头来源于佛家。牛头又叫阿傍，其形为牛头人身，手持钢叉，力能排山。据《铁城泥犁经》说：阿傍为人时，因不孝父母，死后在阴间为牛头人身，担任巡逻和搜捕逃跑罪人的衙役。",
      boss_mamian:
      ...
      boss_baiwuchang:
        "黑白无常，亦称无常。是中国传统文化中的一对神祇，也是最有名的鬼差。此二神手执脚镣手铐，专职缉拿鬼魂、协助赏善罚恶，也常为阎罗王、城隍、东岳大帝等冥界神明的部将。 白无常名为谢必安，属阳。时常满面笑容，身材高瘦，面色惨白，口吐长舌，其头上官帽写有“一见生财”四字，予感谢并对恭敬神明之人以好运，尊之曰“活无常”，“白爷”等。对男性吸其阴魂，对女性散其阴魄。 ",
      boss_heiwuchang:
        "黑白无常，亦称无常。是中国传统文化中的一对神祇，也是最有名的鬼差。此二神手执脚镣手铐，专职缉拿鬼魂、协助赏善罚恶，也常为阎罗王、城隍、东岳大帝等冥界神明的部将。 黑无常名为范无救（或称范无赦、范无咎），属阴。面容凶悍，身宽体胖，个小面黑，官帽上写有“天下太平”四字，意为对违抗法令身负罪过者一概无赦，尊之曰“矮爷”或“黑爷”。对女性吸其阳魂，对男性散其阳魄。 因其在城隍麾下地位仅次于文武判官，牛头马面，枷爷，锁爷，故又得名“七爷”，“八爷”。",
      boss_luocha:
        "罗刹是相传原为南亚次大陆土著名称。自雅利安人征服印度后﹐凡遇恶人恶事﹐皆称罗刹﹐遂成恶鬼名。",
      boss_yecha:
        "夜叉是佛经中一种形象丑恶的鬼，勇健暴恶，能食人，有的后受伟大的佛陀之教化而成为护法之神，列为天龙八部众之一。",
      boss_qinguangwang:
        "第一殿秦广王蒋歆，东方玉宝皇上天尊化冥府一殿泰素妙广真君秦广大王，神居玄冥宫，二月初一日诞辰，位列震宫，尊居卯位，执掌风雷地狱，权衡霹雳神威，专司人间生死，统管吉凶。行善者作于青篇，作恶者标于黑簿，考察无私。专司大海之底，沃燋石外正黄泉黑路，统管幽冥吉凶、善人寿终，接引超升；功过两半者，送交第十殿发放，仍投入世间，男转为女，女转为男。恶多善少者，押赴殿右高台，名曰孽镜台，令之一望，照见在世之心好坏，随即批解第二殿，发狱受苦。",
      boss_chujiangwang:
        "第二殿楚江王历温，南方玄真万福天尊化冥府二殿阴德定休真君楚江大王，神居普明宫，三月初一日诞辰，位列离宫，尊居午位，执掌火医地狱，威专烈焰之权，专司大海之底，正南沃燋石下活大地狱，又名剥衣亭寒冰地狱，另设十六小地狱，凡在阳间伤人肢体、奸盗杀生者，推入此狱，另发入到十六小狱受苦，满期转解第三殿，加刑发狱。",
      boss_songdiwang:
        "第三殿宋帝王余懃，西方太妙至极天尊化冥府三殿洞明昔静真君宋帝大王，神居纣绝宫，二月初八诞辰，位列兑宫，尊居酉位，执掌金刚地狱，威司考掠之权，专司大海之底，东南沃燋石下黑绳大地狱，另设十六小狱，凡阳世忤逆尊长，教唆兴讼者，推入此狱，受倒吊、挖眼、刮骨之刑，刑满转解第四殿。",
      boss_wuguanwang:
        "第四殿五官王吕岱，北方玄上玉宸天尊化冥府四殿玄德五灵真君五官大王，神居太和宫，二月十八日诞辰，位居坎宫，尊居子位，执掌滨冷地狱，权衡冰雪之威，专司大海之底，正东沃燋石下合大地狱，又名剥剹血池地狱，另设十六小地狱，凡世人抗粮赖租，交易欺诈者，推入此狱，另再判以小狱受苦，满日送解第五殿察核。",
      boss_yanluowang:
        "第五殿阎罗王包拯，东北方度仙上圣天尊化冥府五殿最胜耀灵真君阎罗天子，神居纠纶宫，正月初八日诞辰，位列垦宫，尊居丑位，执掌镬汤地狱，威张煮溃之权，专司大海之底，东北沃焦石下叫唤大地狱，并十六诛心小狱。前本居第一殿，因怜屈死，屡放还阳伸雪，降调此殿。凡解到此殿者，押赴望乡台，令之闻见世上本家，因罪遭殃各事，随即推入此狱，细查曾犯何恶，再发入诛心十六小狱，钩出其心，掷与蛇食，铡其身首（包公即善于用铡刀），受苦满日，另发别殿。",
      boss_bianchengwang:
        "第六殿卞城王毕元宾，东南方好生度命天尊化冥府六殿宝肃昭成真君卞城大王，神居明晨宫，三月初八日诞辰，位列巽官，尊居幽府，执掌铜柱地狱，威专足履之刑，专司大海之底，正北沃燋石下大叫唤大地狱，及枉死城，另设十六小地狱。忤逆不孝者，被两小鬼用锯分尸。凡世人怨天尤地，对北溺便涕泣者，发入此狱。查所犯事件，亦要受到铁锥打、火烧舌之刑罚。再发小狱受苦，满日转解第七殿，再查有无别恶。",
      boss_taishanwang:
        "第七殿泰山王董和，西南方太灵虚皇天尊化冥府七殿等观明理真君泰山大王，神居神华宫，三月二十七日诞辰，位列坤宫，尊居泉曲，执掌屠割地狱，威权刀割之刑，专司大海之底，西北沃燋石下热恼大地狱，又名碓磨肉酱地狱，另设十六小地狱。凡阳世取骸合药、离人至戚者，发入此狱。再发小狱。受苦满日，转解第八殿，收狱查治。凡盗窃、诬告、敲诈、谋财害命者，均将遭受下油锅之刑罚。",
      boss_dushiwang:
        "第八殿都市王黄中庸，西北方无量太华天尊化冥府八殿飞魔衍庆真君都市大王，神居碧真宫，四月初一日诞辰，位列乾宫，尊居阴府，执掌火车地狱，威司运转之权，专司大海之底，正西沃焦石下大热大恼大地狱，又名恼闷锅地狱，另设十六小地狱。凡在世不孝，使父母翁姑愁闷烦恼者，掷入此狱。再交各小狱加刑，受尽痛苦，解交第十殿，改头换面，永为畜类。",
      boss_pingdengwang:
        "第九殿平等王陆游，上方玉虚明皇天尊化冥府九殿无上正度真君平等大王，神居七非宫，四月初八日诞辰，敕合乾元，德隆坤域，执掌普掠地狱，威转炽盛之权，专司大海之底，西南沃焦石下酆都城铁网阿鼻大地狱，另设十六小狱。凡阳世杀人放火、斩绞正法者，解到本殿，用空心铜桩，链其手足相抱，煽火焚烧，烫烬心肝，随发阿鼻地狱受刑。直到被害者个个投生，方准提出，解交第十殿发生六道（天道、修罗道、人间道、畜生道、饿鬼道、地狱道）。",
      boss_zhuanlunwang:
        "第十殿转轮王薛礼，下方真皇洞神天尊化冥府十殿五华威灵真君轮转大王，神居肃英宫，四月十七日诞辰，位尊幽都，名尊十帝，执掌罗酆之府，专司大海之底，沃燋石外正东直对世界五浊，司掌各殿解到鬼魂，分别善恶，核定等级，发四大部洲投生。男女寿夭，富贵贫贱，逐名详细开载，每月汇知第一殿注册。凡有作孽极恶之鬼，着令更变卵胎湿化，朝生暮死，罪满之后，再复人生，投胎蛮夷之地。凡发往投生者，先令押交孟婆神，醧忘台下，灌饮迷汤，使忘前生之事。",
      boss_dizangwang:
      ...
      boss_mengpo:
        "孟婆神孟姜女，常驻在忘川河黄泉路奈何桥旁醧忘台，为了能忘记这些痛苦万分的记忆，熬制了能使人忘记记忆的孟婆汤。她在奈何桥畔熬制孟婆汤，让参与轮回的阴魂们忘记前世的一切。醧忘台高大如方丈，四周的廊房有一百零八间。有一条通道，通向东方，仅一尺四寸宽。所有奉令押解到的男女鬼魂，在各廊房中，都有杯子，招各鬼饮下此汤，多饮少饮不论。",
      ...
      boss_qinglong:
        "青龙，源于上古星宿崇拜。我国古代把天空里的恒星划分成为“三垣”和“四象”七大星区。“三垣”是“紫微垣”，象征皇宫；“太微垣”象征行政机构；“天市垣”象征繁华街市。这三垣环绕着北极星呈三角状排列。在“三垣”外围分布着“四象”：“东苍龙、西白虎、南朱雀、北玄武”。也就是说，东方的星象如一条龙。由于地球围绕太阳公转，天空的星相也随着季节转换。每到冬春之交的傍晚，苍龙显现。 龙是东方七宿（角、亢、氐、房、心、尾、箕），而这七宿的形状又极似龙形，从他们的字义上就可以看出来，角是龙的角，亢是颈项，氐是本，而是颈根，房是膀，是胁，心是心脏，尾是尾，箕是尾末。",
      boss_mushengoumang:
        "句芒是中国古代民间神话中的木神、春神、东方之神，主管树木的发芽生长，忠心耿耿地辅佐伏羲。太阳每天早上从扶桑上升起，神树扶桑归句芒管，太阳升起的那片地方也归句芒管。句芒在古代非常重要，每年春祭都有份。传说它的本来面目是鸟，它鸟身人面，乘两龙，后来竟一点影响也没有了。不过我们可以在祭祀仪式和年画中见到它：它变成了春天骑牛的牧童，头有双髻，手执柳鞭，亦称芒童。",
      boss_shujing:
        "树精可能指由句芒管辖的灵木，《玄中记》（郭璞撰，佚文辑录）“千岁树精为青羊，万岁树精为青牛，句芒遣使巡木，见之则呼其名。”；句芒神力衍生的树妖，《云笈七签》卷十八“句芒号青帝真君，坐镇东林，万木之精皆为碧衣吏。”；古籍《山海经》亦有类似奇树记载，可认为是树精的原型之一，见《中山经·中次七经》的帝屋树及无名毒树（浮戏山）。",
      boss_taihao:
        "太昊（公元前4354—公元前4239年在位），是东夷部族与华夏部族的祖先和首领，是东方祖神，也是东方天帝青帝。亦作大嗥、太皞、大皥，以木德王，是为春皇。汉朝后有时与楚帛书创世神伏羲氏合并。 太昊在位115年，据说定都于汶上（今山东济宁），后迁都宛丘（今河南淮阳）。死后葬于淮阳太昊陵。最早见于文字记载的是春秋战国前期的主要文献《左传·昭公十七年》“陈，太皞之虚也。” 中国神话中的太阳神就是伏羲，皇甫谧《帝王世纪》云：太昊帝包牺氏……继天而生，首德，于木，为百王帝。帝出于震，未有年固。故位在东方。主春，象日之明，世称太昊。 一说青帝太昊伏羲氏即为天皇氏，为开天辟地后首位中国君主。",

      boss_zhuque:
        "朱雀，是中国古代神话中的天之四灵之一，源于远古星宿崇拜，是代表炎帝与南方七宿的南方之神，于八卦为离，于五行主火，象征四象中的老阳，四季中的夏季，同时也是天之南陆。南方神兽朱雀就是鸟的外形。南方属火，朱雀等鸟的图腾在古代神话中往往属于太阳崇拜。二十八宿的南方七宿（井、鬼、柳、星、张、翼、轸），其形象鸟，位于南方，属火，色赤，总称朱雀，亦名“朱鸟”。",
      boss_huoshenzhurong:
        "祝融，号赤帝，中国古代神话中的火神、南方神、南岳神、南海神、夏神、灶神，为五行神之一。历史文献中，祝融的相关记载纷繁复杂。在部分文献中，祝融指三皇时期的祝融氏；又有文献记载祝融为炎帝之后；另有文献记载祝融为黄帝之后；而祝融也指火正、夏官的官职名称。据《山海经》记载，祝融长着兽身人面，架乘着两条龙，直接听命于天帝。据《史记》记载，祝融是楚人的祖先。据《广东新语》记载，农历二月十三为祝融生日。",
      boss_yanling:
        "焰灵可能指《山海经·海外南经》中记载的厌火国人，“厌火国在其国南，兽身黑色，火出其口中。”。厌火国是一个栖息在讙头国之南的神秘族群，属南方炎热之地，与“火神祝融”的信仰区域呼应。其国民身形如黑色兽类（郭璞注称似猕猴），通体焦黑如炭，拥有从口中喷吐火焰的生理特性；晋代《博物志》补充揭示他们以吞食火炭为生，这一习性构成其喷火能力的自然逻辑，使其成为与炽热环境共生的“活体火焰容器”。厌火国人并非主动攻击的妖魔，而是南方荒芜火域中适应极端高温的生存者，其形象承载着古人对火山地带异族的奇幻想象——既映射了现实中有火技艺或接触地火的部族（如矿工群体），又暗含原始火崇拜的图腾色彩。",
      boss_yandi:
      ...
      boss_baihu:
        "白虎，战神、杀伐之神。白虎具有避邪、禳灾、祈丰及惩恶扬善、发财致富、喜结良缘等多种神力。而它是四灵之一，当然也是由星宿变成的。是由二十八星宿之的西方七宿（奎、娄、胃、昂、毕、觜、参）。所以是西方的代表，而它的白，是因是西方，西方在五行中属金，色是白的。所以它叫白虎不是因它是白色，而是从五行中说的了。白虎原为古老神话中的西方之神，从先秦时代开始它就是代表少昊与西方七宿的神兽，而到了汉代五行学说兴起，它的象征含义又多了庚辛与秋季，四象合于五行，西方白虎又多了象征五行中金行的意义。",
      boss_jinshenrushou:
        "蓐收，又名该，中国古代神话中的金神、秋神、西方之神、天之刑神，五行神之一。少昊之子及其辅佐神。 据《国语·晋语》记载，蓐收脸上长着白毛，有老虎一样的爪，手里拿着斧子。《山海经·海外西经》记载，蓐收左耳上有条蛇，骑着两条龙。",
      boss_mingxingzhu:
        "明刑柱可能指蓐收辖地的神异之柱，据《山海经·西山经》“蓐收所居泑山 “西望日之所入，其气员（圆），神红光之所司也。”（郭璞注：红光即蓐收）与汉代谶纬《龙鱼河图》“西方白帝神曰蓐收， 立金柱三千丈，主戮奸邪；亦或者是蓐收所执法器，唐朝《道法会元》卷七六所记载：道士召请蓐收时，需 “存想西方金炁凝为铁柱，罪魂环柱哀号”。”",
      boss_shaohao:
        "少昊，华夏人文始祖，远古时期部落联盟首领称号，黄帝的长子，母为嫘祖（一说女节）。少昊，亦作“少暤”，名“挚”，又作“质”，号“金天氏”、“穷桑氏”或“朱宣氏”，生于穷桑，和太昊伏羲一样同为上古时期东夷族的祖先和首领，因修太昊之法，故称之为少昊，姬姓，一说为嬴姓，名玄嚣。 出生于江水（古代山东西部的一条大河）边。少年时，迎娶凤鸿氏女子为妻，成为凤鸿部落的首领，最终成为整个东夷部落的首领，号金天氏，一号青阳氏。少昊是东夷部族的首领。少昊部族内有20多个以鸟为名的部落，如凤鸟氏、玄鸟氏、伯赵氏、青鸟氏、丹鸟氏、祝鸿氏、鸤鸠氏、鹘鸠氏、爽鸠氏等等，其中有凤族8个，凤族在少昊集团中地位最为尊贵，掌管天文历法，指导部落农桑。",

      boss_xuanwu:
        "玄武，是一种由龟和蛇组合成的一种灵物。玄武的本意就是玄冥，武、冥古音是相通的。武，是黑的意思；冥，就是阴的意思。玄冥起初是对龟卜的形容：龟背是黑色的，龟卜就是请龟到冥间去诣问袓先，将答案带回来，以卜兆的形式显给世人。因此，最早的玄武就是乌龟。二十八宿的北方七宿（斗、牛、女、虚、危、室、壁），以后，玄冥的含义不断地扩大。龟生活在江河湖海（包括海龟），因而玄冥成了水神；乌龟长寿，玄冥成了长生不老的象征；最初的冥间在北方，殷商的甲骨占卜即‘其卜必北向’，所以玄冥又成了北方神。",
      boss_shuishenxuanming:
        "玄冥一词指水神、海神、冬神、北方之神，而道家则用来形容“道”的，道教称肾之神。到汉代，中国民间用来指阴间，九泉；同时用于汉代郊祀歌名，歌词为：“玄冥陵阴，蛰虫盖臧……”取首二字为歌名。到唐代以后，就有用来称为北方的，也有用来称为冬季的。而到了现代，“玄冥”则成为各种武侠小说里武功怪异人的代名词了。但实际上，“冥”是商族领袖之一。因其”勤其官而水死“，故被后人奉为水神，称“玄冥”。",
      boss_shuishengonggong:
      ...
      boss_zhuanxu:
        "颛顼，姬姓，高阳氏，黄帝之孙，昌意之子。上古部落联盟首领，“五帝”之一，人文始祖之一。颛顼辅佐少昊有功，封地在高阳（今河南省杞县高阳镇），故号高阳氏。少昊死后，打败争夺帝位的共工氏，成为部落联盟首领，号“高阳氏”。始都穷桑，后迁都商丘。 颛顼去世后，由黄帝曾孙帝喾继位。在流传下来的神话传说中，颛顼是主管北方的天帝。《史记·五帝本纪》记载颛顼：“静渊以有谋，疏通而知事”。",

      boss_hundun:
        "混沌，又作浑沌；是中国古代神话四大凶兽之一 。先秦《左传》记载，中国古代神话四大凶兽分别是：形象如同巨大的狗的“混沌”、人头羊身并且腋下长眼睛的“饕餮”、生有翅膀的大虎“穷奇”以及人头虎腿并长有野猪獠牙的“梼杌”。 据《神异经·浑沌》记载，混沌的形象特征：其状如犬，长毛四足，似熊而无爪，有目而不见。",
      boss_qiongqi:
        "穷奇，中国古代神话传说中的四凶之一，是西方天帝少昊的后代，因毁信恶忠，崇饰恶言，被舜流放，迁于四裔，以御魑魅；和混沌、梼杌、饕餮并称为远古“四大凶兽”。 《山海经·海内北经》记载：穷奇外貌像老虎，大小如同牛般，长有一双翅膀。传说穷奇：性情凶狠，喜欢吃人。不忠不信，不听好人之言，专门听信别人的坏话。",
      boss_taowu:
        "梼杌，别名傲狠，中国神话中上古时期的四凶之一。 所谓“四凶”，最早指的是上古年间四位残暴的部落首领，后来被杜撰为他们部落图腾的四种怪物。据《神异经》记载，梼杌是生活在偏远西方的怪物，它体格像老虎而毛类犬，毛很长，脸有点像人，腿有点像老虎，嘴巴长有像野猪一样的獠牙，尾长丈八尺，在西方称霸，能斗不退。而“梼杌”，较早见于《左传·文公十八年》，原文是：颛顼有不才子，不可教训，不知话言，告之则顽，舍之则嚚，傲狠明德，以乱天常，天下之民谓之梼杌。",
      boss_taotie:
        "饕餮，别名老饕、狍鸮，是中国古代神话传说中的一种凶恶贪食的野兽，四大凶兽之一。据《山海经》记载：饕餮其形状如羊身人面，其目在腋下，虎齿人爪，其音如婴儿。是贪欲的象征，常用来形容贪食或贪婪的人。",
      boss_zhuyin:
        "烛阴，又名烛龙、烛九阴、逴龙、火精，中国古代神话中钟山的山神，无足之龙，开眼为昼、闭眼为夜，有一子名为鼓。 据《山海经·大荒北经》记载，烛龙是章尾山之神，人面蛇身，浑身红色，眼睛竖着长。《山海经·海外北经》记载的钟山之神烛阴也是人面蛇身赤色。郭璞在《山海经图赞译注》中认为烛龙与烛阴乃是同一物。",
      boss_xiangliu:
        "相柳，又称相繇，上古时代中国神话传说中的凶神，共工的臣属，出自《山海经·海外北经》：“蛇身九头，食人无数，所到之处，尽成泽国”。 传说它喷出来的水比洪水还厉害，又苦又辣，吃了就会送命。因此，这种水泽连禽兽也不能生活。禹见相柳如此猖獗，就运用神力杀了相柳，为民除害。相柳身上流出的血，一沾土地就五谷不生，把大片地方污染了。禹尝试用泥土陉塞，但三陉三陷，禹只好把这片土地劈为池子，各方天神在池畔筑起一座高台，镇压妖魔。",
      boss_zhuyan:
        "朱厌是中国古代神话中的动物。 《山海经·西山经》记载：其状如猿，而白首赤足；传说这种野兽一出现，天下就会发生大乱。先秦时期《山海经·西山经》记载：“又西四百里，曰小次之山，其上多白玉，其下多赤铜。有兽焉，其状如猿，而白首赤足，名曰朱厌，见则大兵。”朱厌即猿，它有洁净的毛发（白首：白，彰明、清晰、洁净；首，标表），伺机奔跑（赤足：四肢为红色）。朱厌图有二形：其一为猴形；其二为人面猴身。",
      boss_bifang:
        "毕方是中国古代神话中的形象，生活在章莪山。 据《山海经·西山经》记载：毕方形状像鹤，只有一只脚，青色的羽毛之上有红色的斑纹，长着白色的嘴巴，先秦时期，《山海经·西山经》记载：“有鸟焉，其状如鹤，一足，赤文青质而白喙，名曰毕方，其鸣自叫也，见则其邑有讹火。”（山中有一种鸟，它的形状像鹤，只有一只脚，青色的羽毛之上有红色的斑纹，长着白色的嘴巴，这种鸟名叫毕方，它鸣叫起来就好像是在呼喊自己的名字，它在哪里出现，哪里就会有大片的野火。）",
      boss_yingzhao:
        "英招是上古时期中国神话传说中的神，为马身人面虎纹鸟翼，是看花园的天神，属华夏族和西戎部落；出自《山海经·西次三经》。《山海经·西山经》：“又西三百二十里，曰槐江之山，丘时之水出焉，而北流注于泑水。其中多蠃母，其上多肯雄黄，多藏琅歼、黄金、玉，其陨多丹粟，其陲多采黄金银。实惟帝之平圃，神英招司之，其状马身而人面，虎文而舄翼，徇于四海，其音如榴。”",

      boss_shikieiki:
        "四季映姬·夜摩仙那度（四季映姫・ヤマザナドゥ）是东方Project系列中的官方角色，首次登场于东方花映塚。四季映姬曾经是地藏，后来全国各地的地藏联名上书请求分担阎魔大人的工作，她也成为了阎魔。她的立绘剪影出现在了花映塚的封面上。她非常喜欢说教，且她从来不会说出失之偏颇的话语，因此在她周围会油然而生一种难以接近的氛围。但她的说教也让人有些烦。在花映塚之后，她在官方游戏和官方出版物中偶有出场。四季映姬平时生活在彼岸，偶尔也会出现在幻想乡，这时妖怪们就会躲起来。她大部分时间在是非曲直厅内进行审判的工作，经常负责幻想乡死者的裁决，幻想乡长生不老的妖怪都曾受其关照。她平时负责给死者定罪，判断让死者去地狱还是天界，或者其他地方。阎魔审判的工作采用的是二人轮换制，她休息的时候往往在彼岸享受属于自己的悠闲生活。在休假时间，她就会直截了当地警告背负着过于沉重的罪孽的长寿之人。四季映姬拥有明鉴黑白程度的能力。她拥有非常复杂而全面的标准来判断善恶，涵盖从案件的成立到有关人员的成长史等等内容，她的判决无关法律，因此她会制裁法律无法制裁的罪过。对她说谎被揭穿后会被施以重罪。但她也曾向射命丸文表示有冤罪存在，后者认为这是其不想落人话柄。她在迷途竹林也不会迷路。她拥有净玻璃之镜，用这面镜子映照受审者，受审者过去的一切言行都将在镜中得到再现，毫无隐私可言。四季映姬的净玻璃之镜大小应该和单手持的小梳妆镜相仿。她拥有强大的力量。据说对她花钱贿赂能让人起死回生。",
    },
    characterTitle: {
      boss_chi: "鬼蜮地动",
      boss_mo: "夜鬼悲鸣",
    ...
    skill: {
      ...
      niaobaidaowenha_skill: {
        ...
        filter(event, player) {
          return (
            game.hasPlayer((current) => current !== player) && event.num > 0
          )
        },
      ...
      goujiangdesidai_skill: {
        ...
        filter(event, player) {
          return (
            !player.hasSkill("kagari_zongsi") ||
            player.getStat("skill").kagari_zongsi
          )
        },
      },
      gubuzifeng_disable: {
      ...
      thedayibecomeagod: {
        ...
        content() {
          ...
          player
            .chooseTarget(
              get.prompt2("thedayibecomeagod"),
              (card, player, target) => target.isFriendOf(player),
            )
            .set("forceDie", true).ai = (target) =>
        ...
        content() {
          ...
          if (event.list[result.index][0] === "令") {
            ...
            player
              ...
              .set(
                "ignoreCard",
                player.hasSkill("shanrangzhaoshu", null, false),
              )
          }
          ;("step 3")
          ...
          if (result.index === 0) {
            ...
            player
              .chooseCard(
                "he",
                true,
                (card, player) => card !== player.getEquip(5),
              )
              .set("ai", (card) => {
        ...
        content() {
          ...
          player
            .chooseToDiscard(
              ...
              2,
              `弃置两张牌，令${get.translation(trigger.target)}本回合内不能使用或打出牌且防具技能无效。`,
              (card, player) => card !== player.getEquip(1),
            )
            ...
            .set(
              ...
              ((event, player) => {
                ...
                if (get.tag(event.card, "respondShan")) {
                  if (!player.hasShan()) {
                    return false
                  }
                  return true
      ...
      iwasawa_crowbow: {
        ...
        filter(event, player) {
          ...
          return evt?.hs && evt.hs.length > 1 && player.isPhaseUsing()
        },
        content() {
          "step 0"
        ...
        contentx() {
          ...
          if (list.length === 1) {
            ...
            player
              ...
              .set("forceDie", true).ai = () => list.randomGet()
          }
          ;("step 2")
          _status.shidianyanluo_level++
            ...
            if (current !== game.boss) {
              for (var i = 0; i < list.length; i++) {
                if (current.name === list[i] || current.name2 === list[i]) {
                  current.init(
                    ["sunce", "re_sunce", "shen_sunce", "sb_sunce"].randomGet(),
                  )
                }
              }
            }
        ...
        contentx() {
            ...
            game.boss
              ...
              .map((cur) => {
                ...
                for (const name of names) {
                  if (
                    lib.rank.s.includes(name) ||
                    lib.rank.ap.includes(name) ||
                    lib.rank.a.includes(name) ||
                    lib.rank.am.includes(name)
                  ) {
                    return name
                  }
                }
          ...
          if (list.length === 1) {
            ...
            player
              ...
              .set("ai", () => _status.event.choice).prompt =
              "选择下一个登场的武将"
          }
          ;("step 2")
          _status.shidianyanluo_level++
        ...
        filter(event, player) {
          return (
            ...
            !(
              ...
              event.getParent(2).name === "phaseDraw"
            ) &&
      ...
      boss_tiemian: {
        ...
        init(player, skill) {
          player.addExtraEquip(
            ...
            "renwang",
            true,
            (player) => !player.getEquips(2).length && lib.card.renwang,
          )
        },
        onremove(player, skill) {
      ...
      boss_pingdeng: {
        ...
        content() {
          ...
          var list = game.filterPlayer(
            (current) =>
              current !== player &&
              !game.hasPlayer((current2) => current2.hp > current.hp),
          )
          if (list.length) {
            var target = list.randomGet()
            player.line(target)
      ...
      boss_lunhui: {
        ...
        filter(event, player) {
          return (
            player.hp <= 2 &&
            game.hasPlayer((current) => current !== player && current.hp > 2)
          )
        },
        content() {
          var list = game.filterPlayer(
            (current) => current !== player && current.hp > 2,
          )
          if (list.length) {
            var target = list.randomGet()
            player.line(target)
      ...
      boss_shiyou: {
        ...
        content() {
          ...
          ;("step 1")
          var next = player
            .chooseCardButton(get.prompt("boss_shiyou"), event.cards, [
              1,
              event.cards.length,
            ])
      ...
      boss_wangshi2: {
        ...
        mod: {
          cardEnabled2(card, player) {
            if (
              player.storage.boss_wangshi2.includes(get.type(card, "trick"))
            ) {
              return false
            }
          },
        ...
        content() {
          ...
          if (list.length > 0) {
            ...
            if (get.mode() === "boss") {
              ...
              ui.cardPile.insertBefore(
                card1,
                ui.cardPile.childNodes[a.randomGet()],
              )
              a.push(a.length)
              ui.cardPile.insertBefore(
                card2,
      ...
      boss_bufo: {
        ...
        filter(event, player, name) {
          ...
          return game.hasPlayer(
            (target) => player !== target && get.distance(player, target) <= 1,
          )
        },
        content() {
      ...
      boss_wuliang: {
        ...
        content() {
          ...
          if (name === "phaseZhunbeiBegin") {
            ...
            player.draw(
              name === "gameDrawAfter" || name === "enterGame" ? 3 : 2,
            )
          }
        },
      },
        ...
        content() {
          ...
          if (result.control !== "cancel2") {
            ...
            if (result.control === "club" || result.control === "spade") {
              ...
              result.control === "heart" ||
              result.control === "diamond"
            ) {
	...
	*/
      ...
      boss_shedu: {
        ...
        content() {
          ...
          player.chooseToDiscard(
            "he",
            num,
            `弃置${chs}张牌，或失去${chs}点体力`,
          ).ai = (card) => 12 - get.value(card)
          ;("step 1")
          if (!result.bool) {
            player.loseHp(num)
          }
      ...
      boss_jiushou: {
        ...
        trigger: {
          player: ["phaseUseBegin", "phaseJieshuBegin", "phaseDrawBegin"],
        },
        forced: true,
        filter(event, player) {
      ...
      boss_echou: {
        ...
        filter(event, player) {
          return (
            !event.player.hasSkill("boss_duqu") &&
            ["tao", "jiu"].includes(event.card.name)
          )
        },
        content() {
      ...
      boss_bingxian: {
        ...
        filter(event, player) {
          return (
            event.player !== player && event.player.countUsed("sha", true) === 0
          )
        },
        forced: true,
        content() {
      ...
      boss_xushi: {
        ...
        content() {
          ...
          if (trigger.name === "phaseUse") {
            ...
            event.list = game.filterPlayer((current) => current !== player)
        ...
        content() {
          ...
          while (event.togain.length < event.num2) {
            var card = get.cardPile(
              (card) =>
                !event.togain.includes(card) && get.color(card) === "red",
            )
            if (card) {
      ...
      boss_xunyou: {
        ...
        content() {
          "step 0"
          var list = game.filterPlayer(
            (current) => current !== player && current.countCards("hej"),
          )
          if (list.length) {
            var target = list.randomGet()
      ...
      boss_zaoyi: {
        ...
        filter(event, player) {
          ...
          if (list.includes(event.player.name)) {
            return !game.hasPlayer((current) => list.includes(current.name))
          }
          return false
        },
        content() {
          ...
          targetEnabled(card, player, target, now) {
            if (target.isEnemyOf(player)) {
              ...
              if (type === "trick") {
                if (
                  game.hasPlayer(
                    (current) => current.name === "boss_shuishenxuanming",
                  )
                ) {
                  return false
                }
              ...
              if (type === "basic") {
                if (
                  game.hasPlayer(
                    (current) => current.name === "boss_shuishengonggong",
                  )
                ) {
                  return false
                }
            ...
            const { skill } = player
              ...
              .step(async (event, trigger, player) => {
                const target = game.findPlayer(
                  (current) => current.name === "boss_jinshenrushou",
                )
                if (target) {
                  player.line(target)
      ...
      boss_xingqiu: {
        ...
        async content(event, trigger, player) {
          ...
          const targets = game
              .filterPlayer((current) => current.name === "boss_mingxingzhu")
              .sortBySeat(),
      ...
      boss_shenwuzaishi: {
        ...
        content() {
          if (
            ...
            trigger.player.name === "boss_zhuyin"
          ) {
            player.draw(3)
            player.recover()
      ...
      boss_futai: {
        ...
        trigger: { player: "phaseZhunbeiBegin" },
        logTarget(event, player) {
          return game.filterPlayer((current) => current.isDamaged())
        },
        forced: true,
        content() {
          "step 0"
          var list = game
            .filterPlayer((current) => current.isDamaged())
            .sortBySeat()
          event.list = list
          ;("step 1")
          if (event.list.length) {
      ...
      boss_luanchang: {
        ...
        subSkill: {
          begin: {
            ...
            content() {
              var list = game
                .filterPlayer((current) => player.canUse("nanman", current))
                .sortBySeat()
              if (list.length) {
            ...
            content() {
              var list = game
                .filterPlayer((current) => player.canUse("wanjian", current))
                .sortBySeat()
              if (list.length) {
        ...
        ai: {
          effect: {
            target(card, player, target) {
              ...
              return lib.skill.sanshou_skill.ai.effect.target(
                card,
                player,
                target,
              )
      ...
      boss_chiyan2x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_chiyan2")
          )
        },
        content() {
          "step 0"
      ...
      boss_chiyan3x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_chiyan3")
          )
        },
        content() {
          "step 0"
      ...
      boss_qingmu2x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_qingmu2")
          )
        },
        content() {
          "step 0"
      ...
      boss_qingmu3x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_qingmu3")
          )
        },
        content() {
          "step 0"
      ...
      boss_xuanlin2x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_xuanlin2")
          )
        },
        content() {
          "step 0"
      ...
      boss_baimang2x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_baimang2")
          )
        },
        content() {
          "step 0"
      ...
      boss_baimang3x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss && event.player.hasSkill("boss_baimang3")
          )
        },
        content() {
          "step 0"
      ...
      boss_xingxia: {
        ...
        mode: ["boss"],
        filter(event, player) {
          if (!game.hasPlayer((current) => current.name === "boss_yanling")) {
            return false
          }
          return (
            !player.storage.boss_xingxia ||
            game.roundNumber - player.storage.boss_xingxia >= 2
          )
        },
        unique: true,
        filterTarget(card, player, target) {
        ...
        contentAfter() {
          ...
          player.chooseTarget(
            (card, player, target) => target.side !== player.side,
          ).ai = (target) => get.damageEffect(target, player, player, "fire")
          ;("step 1")
          if (result.bool) {
              ...
              if (get.tag(card, "damage") || get.tag(card, "recover")) {
                if (
                  ...
                  if (
                    game.hasPlayer(
                      (current) =>
                        current.isEnemyOf(game.boss) && current.isLinked(),
                    )
                  ) {
                    return
                  }
      ...
      boss_chiyi: {
        ...
        content() {
          ...
          if (game.roundNumber === 3) {
            var enemies = game.filterPlayer((current) =>
            ...
            event.targets = game
              .filterPlayer((current) => current.name === "boss_yanling")
              .sortBySeat()
            event.num = 5
          }
          ;("step 1")
              ...
              result: {
                player(player, target) {
                  ...
                  if (
                    ...
                    game.hasPlayer(
                      (current) =>
                        current.name === "boss_shujing" && current.hp === 1,
                    )
                  ) {
                  ...
                  if (
                    game.hasPlayer((current) => current.name === "boss_shujing")
                  ) {
                    return 0
                  }
                  return 1
      ...
      boss_cuidu: {
        ...
        content() {
          ...
          var boss = game.findPlayer(
            (current) => current.name === "boss_mushengoumang",
          )
          if (boss) {
            boss.draw()
          }
      ...
      boss_qingyi: {
        ...
        content() {
          ...
          if (game.roundNumber === 7) {
            ...
            event.targets = game
              .filterPlayer((current) => current.isEnemyOf(player))
              .sortBySeat()
            event.command = "loseHp"
          } else {
            event.targets = game
              .filterPlayer((current) => current.isFriendOf(player))
              .sortBySeat()
      ...
      boss_guimou: {
        ...
        content() {
          var list = game.filterPlayer(
            (target) => target !== player && !target.isMad(),
          )
          if (list.length) {
            var target = list.randomGet()
            player.line(target, "green")
      ...
      boss_guixin: {
        ...
        filter(event, player) {
          return (
            event.result && event.result.length >= 2 && event.player !== player
          )
        },
        content() {
          "step 0"
          trigger.player.chooseCard(
            (card) => trigger.result.includes(card),
      ...
      xiaoxiong: {
        ...
        filter(event, player) {
          ...
          return (
            event.player !== player && (type === "basic" || type === "trick")
          )
        },
        content() {
      ...
      boss_zhangwu: {
        ...
        content() {
          ...
          player
            .chooseToDiscard(
              get.prompt("boss_zhangwu", trigger.source),
              "he",
              [1, Infinity],
              "allowChooseAll",
            )
      ...
      yueyin: {
        ...
        skipDamage: {
          ...
          x3(player, event) {
            return game.hasPlayer(
              (current) => current !== player && current.countCards("e") >= 4,
            )
          },
          x2(player) {
        ...
        ai: {
          ...
          effect: {
            target(card, player, target) {
              if (
                !target.hasSkill("xiangxing") ||
                !target.storage.xiangxing ||
                target.storage.xiangxing_count < 6
              ) {
                return
              }
                  ...
                  if (get.tag(card, "damage") || get.tag(card, "loseHp")) {
                    if (
                      !game.hasPlayer((current) => {
                        return (
                          current !== target && current.countCards("e") >= 4
                        )
                      })
                    ) {
                      return [1, 3, 1, -3]
        ...
        content() {
          ...
          if (player.storage.xiangxing_count >= 7) {
            if (
              ...
              lib.skill.yueyin.skipDamage[`x${player.storage.xiangxing}`](
                player,
                trigger,
              )
            ) {
        ...
        content() {
          ...
          for (var i = 0; i < lib.inpile.length; i++) {
            ...
            if (lib.filter.cardEnabled({ name: name }, player)) {
              ...
              list[info.type].push([
                get.translation(lib.card[name].type),
                "",
                name,
              ])
            }
          ...
          player.chooseButton(dialog).ai = (button) => {
        ...
        ai: {
          effect: {
            target(card, player, target) {
              ...
              if (
                target.storage.xiangxing === 2 &&
                target.storage.xiangxing_count > 4 &&
                target.hasSkill("xiangxing") &&
                target.hasSkill("yueyin")
              ) {
                return
              }
              return 0.13
      ...
      mazui: {
        ...
        content() {
          ...
          game.addVideo("storage", target, [
            "mazui2",
            get.cardInfo(target.storage.mazui2),
            "card",
          ])
        },
        ai: {
      ...
      boss_nbianshen: {
        ...
        filter(event, player) {
          if (
            player.name === "boss_nianshou_heti" ||
            player.storage.boss_nbianshen
          ) {
            return true
          }
          return false
        ...
        content() {
          if (player.storage.boss_nbianshen) {
            ...
            player.storage.boss_nbianshen.remove(
            ...
            player.storage.boss_nbianshen_next =
              player.storage.boss_nbianshen.randomGet(
                player.storage.boss_nbianshen_next,
              )
            player.hp = hp
            player.maxHp = maxHp
      ...
      boss_damagecount2: {
        ...
        filter(event, player) {
          ...
          return (
            event.num > 0 &&
            player.isFriendOf(game.me) &&
            event.player.isEnemyOf(game.me)
          )
        },
        content() {
      ...
      boss_bianshen2x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss &&
            event.player.hasSkill("boss_bianshen2")
          )
        },
        content() {
          "step 0"
      ...
      boss_bianshen3x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss &&
            event.player.hasSkill("boss_bianshen3")
          )
        },
        content() {
          "step 0"
      ...
      boss_bianshen4x: {
        ...
        filter(event) {
          ...
          return (
            event.player === game.boss &&
            event.player.hasSkill("boss_bianshen4")
          )
        },
        content() {
          "step 0"
      ...
      boss_moyany: {
        ...
        content() {
          "step 0"
          player.judge((card) => (get.color(card) === "red" ? 1 : 0))
          ;("step 1")
          if (result.bool) {
            player.chooseTarget(
              true,
      ...
      boss_yushou: {
        ...
        content() {
          var list = game.filterPlayer(
            (current) =>
              player.canUse("nanman", current) && current.isEnemyOf(player),
          )
          list.sort(lib.sort.seat)
          player.useCard({ name: "nanman" }, list)
        },
      ...
      boss_xixing: {
        ...
        content() {
          "step 0"
          player.chooseTarget(
            get.prompt("boss_xixing"),
            (card, player, target) => player !== target && target.isLinked(),
      ...
      boss_suoming: {
        ...
        content() {
          ...
          player.chooseTarget(
            ...
            [1, num],
            (card, player, target) => !target.isLinked() && player !== target,
      ...
      boss_xiaoshou: {
        ...
        content() {
          "step 0"
          player.chooseTarget(
            get.prompt("boss_xiaoshou"),
            (card, player, target) =>
              player !== target && target.hp >= player.hp,
      ...
      boss_manjia: {
        ...
        init(player, skill) {
          player.addExtraEquip(
            ...
            "tengjia",
            true,
            (player) => !player.getEquips(2).length && lib.card.tengjia,
          )
        },
        onremove(player, skill) {
          player.removeExtraEquip(skill)
      ...
      boss_shanbeng: {
        ...
        logv: false,
        content() {
          var targets = game.filterPlayer((current) => current.countCards("e"))
      ...
      boss_bianshen: {
        ...
        content() {
          player.smoothAvatar()
          player.init(
            ["boss_chi", "boss_mo", "boss_wang", "boss_liang"].randomGet(),
          )
          game.addVideo("reinit2", player, player.name)
        },
      },
      ...
      boss_leiji: {
        ...
        content() {
          "step 0"
          player.chooseTarget(get.prompt("boss_leiji")).ai = (target) =>
      ...
      boss_konghun: {
        ...
        content() {
          "step 0"
          player.chooseTarget((card, player, target) => target !== player).ai =
            () => 1
          ;("step 1")
          if (result.bool) {
      ...
      fengwu: {
        ...
        content() {
          ...
          ;("step 1")
          event.current.chooseToUse({ name: "sha" }, (card, player, target) => {
            if (player === target) {
              return false
            }
            ...
            for (var i = 0; i < players.length; i++) {
              ...
              if (
                get.distance(player, players[i]) < get.distance(player, target)
              ) {
                return false
              }
            }
      ...
      jidian: {
        ...
        content() {
          ...
          player.chooseTarget(
            ...
            get.damageEffect(target, player, player, "thunder") + 0.1
          ;("step 1")
          if (result.bool) {
            event.target = result.targets[0]
            event.target.judge((card) => (get.color(card) === "red" ? 0 : -1))
        ...
        content() {
          ...
          if (result.bool) {
            player.chooseTarget(
              ...
              "选择一个目标令其失去1点体力",
              (card, player, target) => player !== target,
            ).ai = (target) => Math.max(1, 9 - target.hp)
          } else {
            event.finish()
          }
          ;("step 2")
      ...
      boss_baonu: {
        ...
        content() {
          ...
          player.discard(player.getCards("j"))
          ;("step 3")
          const evt = _status.event.getParent("phaseLoop", true)
          if (evt) {
            game.resetSkills()
      ...
      jizhen: {
        ...
        filter(event, player) {
          return game.hasPlayer(
            (current) => current.isDamaged() && current !== player,
          )
        },
        content() {
          "step 0"
          ...
          player.chooseTarget(
            ...
            [1, 2],
            (card, player, target) =>
              target.hp < target.maxHp && player !== target,
          ).ai = (target) => get.attitude(player, target)
          ;("step 1")
          if (result.bool) {
      ...
      shenqu2: {
        ...
        filter(event, player) {
          return (
            player.hasSkillTag("respondTao") ||
            player.countCards("h", "tao") > 0
          )
        },
        content() {
          player.chooseToUse(
      ...
      jiwu: {
        ...
        check(card) {
          if (
            get.position(card) === "e" &&
            _status.event.player.hasSkill("olxuanfeng")
          ) {
      ...
      boss_jiang: {
        ...
        filter2(event, player) {
          ...
          if (
            event.respondTo[1] &&
            get.itemtype(event.respondTo[1]) !== "card"
          ) {
            return false
          }
          if (
            event.respondTo[1] &&
            ["h", "e", "j"].includes(get.position(event.respondTo[1]))
          ) {
            return false
          }
        },
        ...
        content() {
          ...
          if (trigger.respondTo[0] !== player) {
            if (
              trigger.respondTo[1] &&
              get.position(trigger.respondTo[1]) === "d"
            ) {
        ...
        init(player) {
          player.boss_taoni = () => {
            ...
            for (var i = 0; i < game[keysArray[1]][keysArray[0]]; i++) {
              ...
              for (var a in Object.keys(
                lib[keysArray[3]][keysArray[2]].prototype,
              )) {
                ...
                for (var b = 0; b < playerKeysArray.length; b++) {
                  var opd2 = Object.getOwnPropertyDescriptor(
                    node,
                    playerKeysArray[b],
                  )
                  if (isDefined(opd2)) {
                ...
                for (var c = 0; c < gameKeysArray.length; c++) {
                  var opd3 = Object.getOwnPropertyDescriptor(
                    game,
                    gameKeysArray[c],
                  )
                  if (isDefined(opd3)) {
    ...
    translate: {
      ...
      boss_xiangde: "相德",
      boss_xiangde_info:
        "锁定技，其他角色对你造成伤害时，若其装备区内有武器牌，此伤害+1。",
      boss_yinzei: "隐贼",
      ...
      boss_nitai: "拟态",
      boss_nitai_info:
        "锁定技，防止你于回合内受到的伤害；你于回合外受到火属性伤害+1。",
      boss_luanchang: "乱常",
      ...
      boss_tanyu: "贪欲",
      boss_tanyu_info:
        "锁定技，跳过你的弃牌阶段；结束阶段，若你的手牌数为全场最多，失去1点体力。",
      boss_cangmu: "藏目",
      ...
      qicaishenlu: "七彩神鹿",
      qicaishenlu_info:
        "锁定技，你计算与其他角色的距离时-1，当你造成属性伤害时，你令此伤害+1。",
      ...
      boss_qingzhu: "擎柱",
      boss_qingzhu_info:
        "锁定技，你跳过弃牌阶段，若你没有“殛顶”，你于出牌阶段不能使用【杀】。",
      boss_jiazu: "枷足",
      boss_jiazu_info:
        "锁定技，回合开始时，弃置你上家和下家的敌方角色的装备区内的坐骑牌。",
      boss_jiding: "殛顶",
      boss_jiding_info:
        "锁定技，其他己方角色受到伤害后，若伤害来源为敌方角色，则你视为对伤害来源使用雷【杀】，若此【杀】造成伤害，蓐收回复1点体力。然后你失去此技能。",
      boss_xingqiu: "刑秋",
      boss_xingqiu_info:
        "锁定技，每两轮的出牌阶段开始时，你横置所有敌方角色，然后使明刑柱获得〖殛顶〗。",
      boss_kuangxiao: "狂啸",
      boss_kuangxiao_info:
        "锁定技，你的回合内，你使用【杀】没有距离限制，且指定所有敌方角色为目标。",
      boss_shenyi: "神裔",
      boss_shenyi_info:
        "锁定技，你的武将牌始终正面向上，你的判定区内的牌效果反转。",
      boss_shenen: "神恩",
      boss_shenen_info:
      ...
      boss_guimou: "鬼谋",
      boss_guimou_info:
        "结束阶段，你可以令一名随机的其他角色进入混乱状态直到其下一回合结束。",
      boss_yuance: "远策",
      boss_yuance_info:
      ...
      boss_guixin: "归心",
      boss_guixin_info:
        "锁定技，其他角色摸牌时，若摸牌数不少于2，须将摸到的牌中的一张交给你。",
      xiongcai: "雄才",
      ...
      yueyin: "月隐",
      yueyin_info:
        "锁定技，你的每一枚星对应的一个特定条件，当你失去星时，若满足此条件，则不造成伤害。",
      ...
      boss_nbianshen: "变形",
      boss_nbianshenx: "变形",
      boss_nbianshenx_info:
        "你从第二轮开始，每一轮幻化为警觉、任性、睿智、暴怒四种随机状态中的一种。",
      boss_mengtai: "萌态",
      boss_mengtai_info:
      ...
      boss_renxing: "任性",
      boss_renxing_info:
        "锁定技，你的回合外，一名角色受到1点伤害后或回复1点体力时，你摸一张牌。",
      boss_nbaonu: "暴怒",
      boss_nbaonu_info:
      ...
      boss_xixing: "吸星",
      boss_xixing_info:
        "准备阶段，对任意一名横置的其他角色造成1点雷电伤害，然后回复1点体力。",

      boss_baolian: "暴敛",
      boss_baolian_info: "锁定技，结束阶段，你摸两张牌。",
      boss_manjia: "蛮甲",
      boss_manjia_info:
        "锁定技，若你的装备区内没有防具牌，则你视为装备了【藤甲】。",
      boss_xiaoshou: "枭首",
      ...
      boss_shengshou: "圣手",
      boss_shengshou_info:
        "每当你使用一张牌，你可以进行一次判定，若为红色，你回复1点体力。",
      wuqin: "五禽戏",
      ...
      boss_gongshen: "工神",
      boss_gongshen_info:
        "锁定技，除你之外的角色没有装备区；你不能成为其他角色的延时锦囊牌的目标。",
      ...
      boss_bingfeng: "冰封",
      boss_bingfeng_info:
        "锁定技，你死亡时，若杀死你的角色武将牌是正面朝上， 你令其翻面。",
      ...
      boss_zhadao: "铡刀",
      boss_zhadao_info:
        "锁定技，你使用【杀】指定目标后，你令目标角色防具无效。",
      boss_zhuxin: "诛心",
      boss_zhuxin_info:
        "锁定技，你死亡时，你令场上血量最少的一名其他角色受到2点伤害。",
      boss_bianchengwang_ab: "卞城王",
      boss_bianchengwang: "卞城王·毕元宾",
      boss_leizhou: "雷咒",
      boss_leizhou_info:
        "锁定技，准备阶段，你对随机一名其他角色造成1点雷属性伤害。",
      boss_leifu: "雷缚",
      boss_leifu_info: "锁定技，你的回合结束时，随机横置一名其他角色。",
      boss_leizhu: "雷诛",
      boss_leizhu_info:
        "锁定技，你死亡时，对所有其他角色造成依次造成1点雷属性伤害。",
      boss_taishanwang_ab: "泰山王",
      boss_taishanwang: "泰山王·董和",
      boss_fudu: "服毒",
      boss_fudu_info:
        "锁定技，其他角色使用【桃】时，你令随机另一名其他角色失去1点体力。",
      boss_kujiu: "苦酒",
      boss_kujiu_info:
        "锁定技，其他角色准备阶段，你令其失去1点体力，然后该角色视为使用一张【酒】。",
      boss_renao: "热恼",
      boss_renao_info:
        "锁定技，你死亡时，你令随机一名其他角色受到3点火属性伤害。",
      ...
      boss_zhifen: "炙焚",
      boss_zhifen_info:
        "锁定技，准备阶段，你随机选择一名其他角色，获得其一张手牌（没有则不获得），并对其造成1点火属性伤害。",
      boss_huoxing: "火刑",
      boss_huoxing_info:
        "锁定技，你死亡时，你对所有其他角色造成1点火属性伤害。",
      ...
      boss_wangsheng: "往生",
      boss_wangsheng_info:
        "锁定技，你的出牌阶段开始时，视为你随机使用一张【南蛮入侵】或【万箭齐发】。",
      boss_zlfanshi: "反噬",
      boss_zlfanshi_info:
      ...
      boss_yingzhong: "映冢",
      boss_yingzhong_info:
        "锁定技。你登场后的第一个回合开始时，你随机获得两个“阴间武将”的全部技能。",
      ...
      boss_newhuanren_info_boss:
        "第一关：挑战秦广王<br>第二关：挑战楚江王，宋帝王，五官王，阎罗王中的一个<br>第三关：挑战卞城王，泰山王，都市王，平等王中的一个<br>第四关：挑战转轮王<br>注：孟婆将在每局前三个阶段随机一个阶段登场<br>地藏王登场规则为，50回合内通过第三关，并且在前三关中成功击杀孟婆。<br>选用的武将越阴间、占比越高，四季映姬登场概率就越大。<li>选陆逊左慈张春华于吉蒋费孔融自动变孙笨",
      lingsheji: "灵蛇髻",
      lingsheji2: "灵蛇髻",
      ...
      gubuzifeng_info:
        "出牌阶段，对一名其他角色使用。其的一个随机技能失效直到其下个回合结束。",
