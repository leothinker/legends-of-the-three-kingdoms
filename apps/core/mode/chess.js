...
export default () => {
  return {
      ...
      player: {
        ...
        chessFocus() {
          ...
          if (player.getLeft() - ui.chessContainer.chessLeft < 14) {
            ...
            dx =
              player.getLeft() - ui.chessContainer.chessLeft - ui.chessContainer.offsetWidth + 134
          }
    ...
    game: {
      ...
      setChessInfo(p) {
        ...
        for (var i = 0; i < game.players.length; i++) {
          ...
          node.style.backgroundImage = p.isUnseen()
            ? `url(${lib.assetURL}/image/character/hidden_image.jpg)`
            : p.node.avatar.style.backgroundImage
          node.link = p
            ...
            if (selected.challenge.length) {
              ...
              for (var i = 0; i < total; i++) {
                if (Math.random() < 0.7) {
                  ...
                  _status.enemylist.push(
                    Array.prototype.randomGet.apply(list, _status.enemylist.concat(_status.mylist)),
                  )
                }
              }
            } else {
          ...
          if (!glows.length && !glows2.length) {
            if (!get.config("single_control")) {
              ...
              if (get.config("additional_player")) {
                ...
                addnum =
                  parseInt(get.config("battle_number")) + parseInt(get.config("replace_number"))
              }
    ...
    translate: {
      ...
      guanchuan: "强弩",
      guanchuan_info:
        "当你使用【杀】指定唯一目标后，你可令攻击射线内的其他角色也成为此【杀】的目标。",
