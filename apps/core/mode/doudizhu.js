...
export default {
      ...
      switch (_status.mode) {
        ...
        default: {
          ...
          const version = _status.connectMode
            ? lib.configOL.feiyang_version
            : get.config("feiyang_version")
          if (version === "online") {
            list.push("feiyang")
          ...
          if (
            ["kaihei", "yinfu", "shiqiang", "qiangyi", "oldshiqiang"].includes(
              enhance,
            )
          ) {
            list.push(enhance)
  ...
  game: {
    ...
    chooseCharacterZhidou() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        const characterList = []
        const characterMap = {}
        const charactersByGroup =
          get.config("character_online") || lib.characterOnline
        for (const group in charactersByGroup) {
          ...
          for (const character of [...groupCharacters]) {
            if (
              ...
              (group === "key" && lib.filter.characterDisabled(character))
            ) {
        ...
        const dialog = ui.create.dialog(
          "你的选将框与底牌",
          [characterMap[game.me.playerid], "character"],
        ...
        while (!biddingFinished) {
          ...
          if (
            current === start &&
            (start === tempDizhu || start._control === "不叫地主")
          ) {
    ...
    chooseCharacterHuanle() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const name in lib.character) {
          if (
            replacedCharacters.includes(name) ||
        ...
        for (const current of game.players) {
          ...
          characterMap[id].addArray(
            characterList.randomRemove(4 - characterMap[id].length),
          )
          recommendedCharacters.removeArray(characterMap[id])
        }
        const dialog = ui.create.dialog("你的选将框", [
          characterMap[game.me.playerid],
          "character",
        ])
        const start = game.players.randomGet()
        let current = start
    ...
    chooseCharacterKaihei() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const current of game.players) {
          current._characterChoice = list.randomRemove(
            get.config(`choice_${current.identity}`),
          )
          if (current.identity === "fan") {
            current._friend =
              current.next.identity === "fan" ? current.next : current.previous
          }
        }
      ...
      next.setContent([
        ...
        async (event, trigger, player) => {
            ...
            for (let seat = 2; seat <= game.players.length; seat++) {
              ...
              td.addEventListener(
                ...
                (e) => {
                  ...
                  if (
                    get.distance(game.zhu, game.me, "absolute") === target.link
                  ) {
                    return
                  }
                  ...
                  for (const current of game.players) {
                    if (
                      get.distance(current, game.me, "absolute") === target.link
                    ) {
          ...
          for (const character in lib.characterReplace) {
            ...
            for (let index = replacements.length - 1; index >= 0; index--) {
              if (
                chosen.includes(replacements[index]) ||
                lib.filter.characterDisabled(replacements[index])
              ) {
          ...
          if (event.swapnodialog) {
            ...
            if (!_status.brawl?.noAddSetting && get.config("change_identity")) {
              addSetting(dialog)
            }
          }
        ...
        async (event, trigger, player) => {
          ...
          for (const current of game.players) {
            ...
            event.ai(
              current,
              event.list.splice(0, get.config(`choice_${current.identity}`)),
              null,
              event.list,
            )
          }
    ...
    chooseCharacterKaiheiOL() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        const aiList = game.filterPlayer(
        ...
        for (const name in lib.characterReplace) {
          ...
          for (const character of [...replacements]) {
            if (
              !libCharacter[character] ||
              lib.filter.characterDisabled(character, libCharacter)
            ) {
              replacements.remove(character)
            }
          }
        ...
        for (const name in libCharacter) {
          if (
            replacedCharacters.includes(name) ||
            lib.filter.characterDisabled(name, libCharacter)
          ) {
            continue
          }
          characterList.push(name)
        ...
        const map = {}
        for (const current of game.players) {
          current._characterChoice = characterList.randomRemove(
            lib.configOL[`choice_${current.identity}`],
          )
          if (current.identity === "fan") {
            current._friend =
              current.next.identity === "fan" ? current.next : current.previous
        ...
        for (const current of game.players) {
          const createDialog = [
            "请选择武将",
            [current._characterChoice, "character"],
          ]
          if (current._friend) {
            createDialog.push("队友的武将")
    ...
    chooseCharacterHuanleOL() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const current of game.players) {
          choices.push([
            current,
            {
              createDialog: [
                "选择角色",
                [characterMap[current.playerid], "character"],
              ],
              forced: true,
            },
          ])
        ...
        for (const id in result) {
          if (result[id] === "ai") {
            result[id] = recommendedCharacters.randomRemove(
    ...
    chooseCharacterBinglinOL() {
      ...
      next.setContent(async (event, trigger, player) => {
            ...
            (id, current) => {
              ...
              if (dialog) {
                dialog.content.firstChild.innerHTML =
                  game.me === current ? "是否叫地主？" : "请等待其他玩家叫地主"
              }
            },
            event.videoId,
            event.current,
        ...
        for (const current of game.players) {
          const createDialog = [
            "请选择武将",
            [event.map[current.playerid], "character"],
          ]
          if (current.identity === "fan") {
            const friend = game.findPlayer(
              (target) => target !== current && target.identity === "fan",
            )
            createDialog.push('<div class="text center">队友的选将框</div>')
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const group in chara) {
          ...
          for (const name of [...list]) {
            if (
              !lib.character[name] ||
              lib.connectBanned.includes(name) ||
              (group === "key" &&
                lib.filter.characterDisabled(name, libCharacter))
            ) {
              list.remove(name)
        ...
        for (const id in result) {
          if (result[id] === "ai") {
            result[id] = event.map[id].randomRemove(
              lib.playerOL[id] === game.zhu ? 2 : 1,
            )
          } else {
            result[id] = result[id].links
          }
    ...
    chooseCharacterOL() {
      ...
      next.setContent(async (event, trigger, player) => {
        ...
        for (const name in libCharacter) {
          if (
            availableCharacters.includes(name) ||
            lib.filter.characterDisabled(name, libCharacter)
          ) {
            continue
          }
        ...
        for (const current of game.players) {
          ...
          choices.push([
            current,
            {
              createDialog: [
                "选择角色",
                [characterList.randomRemove(choiceCount), "characterx"],
              ],
              selectButton,
              forced: true,
            },
  ...
  translate: {
    ...
    dcfeiyang: "飞扬",
    dcfeiyang_info:
      "判定阶段开始时，若你的判定区有牌，则你可以弃置两张手牌，然后弃置你判定区的所有牌。",
    mbfeiyang: "飞扬",
    mbfeiyang_info:
      "判定阶段开始时，若你的判定区有牌，则你可以弃置两张手牌，然后弃置你判定区的一张牌。",
    yinfu: "殷富",
    yinfu_info:
      "锁定技。①回合开始时，若你的已损失体力值不小于游戏轮次，你回复1点体力。②当你发动〖殷富①〗至少3次后，你失去〖殷富〗。",
    shiqiang: "恃强",
    shiqiang_info:
      "出牌阶段限一次，你可以将一张牌当无距离限制的【杀】使用。此【杀】结算结束后，若未造成伤害，你减1点体力上限。",
    oldshiqiang: "恃强",
    oldshiqiang_info:
      "出牌阶段限一次，你可以将一张牌当无距离限制的任意【杀】使用。你以此法使用【杀】时，摸一张牌。此【杀】结算结束后，若未造成伤害，你减1点体力上限。",
    qiangyi: "强易",
    qiangyi_info:
      "每名角色限一次。出牌阶段，你选择一名其他角色，获得其一张手牌，然后交给其一张手牌。",
  ...
  element: {
    player: {
      ...
      dieAfter2() {
        ...
        target.showGiveup()
        const version = _status.connectMode
          ? lib.configOL.enhance_nongmin
          : get.config("enhance_nongmin")
        if (version !== "decade") {
  ...
  skill: {
    ...
    toushiche_skill: {
      ...
      async content(event, trigger, player) {
        const targets = lib.skill.toushiche_skill
    ...
    online_juzhong: {
      ...
      async content(event, trigger, player) {
        ...
        await trigger.player.discardPlayerCard({
          target,
          forced: true,
          position: "he",
        })
    ...
    online_zhadan: {
      ...
      async content(event, trigger, player) {
        ...
        event.send = (player, card, source, targets, id, id2, skillState) => {
          ...
          const targetPrompt = targets?.length
            ? `对${get.translation(targets)}`
            : ""
        ...
        while (
          ...
          const result = response?.forResult
            ? await response.forResult()
            : response
          if (!result?.bool) {
            continue
          }
        ...
        if (!event.zhadanresult && event.list.length) {
          ...
          const players = event.list.filter(
            (current) => current.isOnline() || current === game.me,
          )
          event.list.removeArray(players)
          for (const current of game.players) {
            current.showTimer()
          }
          const winner = await Promise.any(players.map(choose)).catch(
            () => null,
          )
          if (winner) {
      ...
      ai: {
        ...
        result: {
          target(player, target, card, isLink) {
            let num1 = 0
            let num2 = Infinity
            const filterCard = (cardx) =>
              !ui.selected.cards.includes(cardx) && !card.cards?.includes(cardx)
            for (const current of game.filterPlayer()) {
    ...
    jiwangkailai: {
      ...
      enable(card, player) {
        ...
        for (const handCard of hs) {
          ...
          if (
            discard &&
            !lib.filter.cardDiscardable(handCard, player, "jiwangkailai")
          ) {
            discard = false
          }
          if (!use && !discard) {
      ...
      async content(event, trigger, player) {
        ...
        for (const name of lib.inpile) {
          if (
            name !== "jiwangkailai" &&
            get.type(name) === "trick" &&
            lib.filter.filterCard({ name, cards }, player)
          ) {
            list.push(["锦囊", "", name])
          }
        }
        ...
        result: {
          target(player, target) {
            if (
              ...
              !target.countCards("h", (card) => get.value(card, player) >= 5.5)
            ) {
              return 1
            }
            return 0
