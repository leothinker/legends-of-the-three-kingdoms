...
export class PlayerGuozhan extends lib.element.Player {
  ...
  mayChangeVice(repeat, hidden) {
    ...
    if (
      repeat ||
      !changedSkills[this.playerid] ||
      !changedSkills[this.playerid].includes(skill)
    ) {
      ...
      if (
        repeat ||
        (!_status.connectMode && get.config("changeViceType") === "online")
      ) {
        // @ts-expect-error 类型就是这么写的
        next.repeat = true
      }
  ...
  dieAfter2(source) {
    if (this.hasSkillTag("noDieAfter", null, source)) {
      return
    }
    if (source?.hasSkillTag("noDieAfter2", null, this)) {
      return
    }
    if (source?.shijun) {
      ...
      if (
        source.identity === "ye" &&
        !source.getStorage("yexinjia_friend").length
      ) {
        source.draw(3)
      } else if (source.shijun2) {
        delete source.shijun2
        source.draw(
          1 + game.countPlayer((current) => current.group === this.group),
        )
      } else if (this.identity === "ye") {
        if (
          this.getStorage("yexinjia_friend").includes(source) ||
          source.getStorage("yexinjia_friend").includes(this)
        ) {
          source.discard(source.getCards("he"))
        } else {
          source.draw(
            1 +
              game.countPlayer((current) => {
                ...
                if (current.getStorage("yexinjia_friend").includes(this)) {
                  return true
                }
                if (this.getStorage("yexinjia_friend").includes(current)) {
                  return true
                }
                return false
              }),
  ...
  checkViceSkill(skill, disable) {
    if (
      game
        .expandSkills(lib.character[this.name2][3].slice(0))
        .includes(skill) ||
      this.hasSkillTag("alwaysViceSkill")
    ) {
      return true
    }
    if (disable !== false) {
  ...
  checkMainSkill(skill, disable) {
    if (
      game
        .expandSkills(lib.character[this.name1][3].slice(0))
        .includes(skill) ||
      this.hasSkillTag("alwaysMainSkill")
    ) {
      return true
    }
    if (disable !== false) {
  ...
  $removeCharacter(num) {
    ...
    if (!lib.character[to]) {
      ...
      lib.character[to] = [
        ...
        0,
        [],
        [`character:${to.slice(3, 11)}`, "unseen"],
      ]
  ...
  changeVice(hidden) {
    ...
    next.num =
      !_status.connectMode && get.config("changeViceType") === "online" ? 1 : 3
    if (hidden) {
      // @ts-expect-error 类型就是这么写的
      next.hidden = true
    }
  ...
  $showCharacter(num, log) {
    ...
    switch (num) {
      ...
      case 2:
        ...
        this.name = this.name1
        skills = lib.character[this.name][3].concat(
          lib.character[this.name2][3],
        )
        this.sex = lib.character[this.name][0]
        this.classList.remove("unseen")
        this.classList.remove("unseen2")
    ...
    game.broadcast(
      // @ts-expect-error 类型就是这么写的
      (player, name, sex, num, identity, group) => {
        ...
        if (
          ...
          _status.clickingidentity[0] === player
        ) {
          // @ts-expect-error 类型就是这么写的
          for (var i = 0; i < _status.clickingidentity[1].length; i++) {
            // @ts-expect-error 类型就是这么写的
  ...
  perfectPair(choosing) {
    ...
    const junFilter = (name1, name2, reverse) => {
      ...
      return info.group === group || info.doubleGroup?.includes(group)
    }
    ...
    var list = Object.keys(lib.perfectPair)
      .concat(Object.values(lib.perfectPair))
      .flat()
    if (!list.includes(name1) || !list.includes(name2)) {
      return false
    }
    return (
      lib.perfectPair[name1]?.flat(Infinity).includes(name2) ||
      lib.perfectPair[name2]?.flat(Infinity).includes(name1)
    )
  }
    ...
    if (typeof targets === "number") {
      ...
      if (info.ai?.expose) {
        if (_status.event.name === "_wuxie") {
          if (_status.event.source?.ai.shown) {
            // @ts-expect-error 类型就是这么写的
            this.ai.shown += 0.2
          }
        } else {
