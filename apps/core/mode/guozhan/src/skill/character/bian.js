  ...
  fake_yigui: {
    ...
    hiddenCard(player, name) {
      ...
      return (
        ...
        player.getStorage("fake_yigui").length &&
        !player.getStorage("fake_yigui2").includes(get.type2(name))
      )
    },
    chooseButton: {
      select: 2,
          ...
          onuse(result, player) {
            ...
            if (!player.storage.fake_yigui2) {
              player
                ...
                .step(async () => delete player.storage.fake_yigui2)
            }
      ...
      zhiheng: {
        ...
        async cost(event, trigger, player) {
          const { bool, links } = await player
            .chooseButton(
              [
                get.prompt("fake_jihun"),
                '<div class="text center">弃置至多两张“魂”，然后获得等量的“魂”</div>',
                [player.getStorage("fake_yigui"), "character"],
              ],
              [1, 2],
            )
            .set("ai", (button) => {
