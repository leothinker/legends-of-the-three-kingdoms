  ...
  return {
    ...
    start: [
      ...
      async (_event, _trigger, _player) => {
        ...
        async function tutorial() {
          if (!lib.config.phonelayout) {
            ...
            var _lcontrol = ui.create.control("使用移动布局", () => {
              if (lib.config.phonelayout) {
          ...
          await new Promise((resolve) =>
            ui.controls[0].replace(
              "在控制/命令一栏中，可以执行一些常见的操作/输入游戏命令",
              resolve,
            ),
          )
          await new Promise((resolve) =>
            ui.controls[0].replace("在录像一栏中，可以管理游戏录像", resolve),
          )
          ui.click.configMenu()
      ...
      async (event, _trigger, _player) => {
        var yearLimitCheck = () => {
          ...
          next.setContent(async (_event, _trigger, _player) => {
    ...
    game: {
      ...
      chooseCharacterPurpleOL: () => {
        ...
        next.setContent(() => {
          ...
          ;("step 1")
          var _list = ["rZhu", "rZhong", "rNei", "rYe"]
          var _list2 = ["bZhu", "bZhong", "bNei", "bYe"]
          list.randomSort()
          list2.randomSort()
      ...
      chooseCharacterStratagemOL: () => {
        ...
        next.setContent(() => {
          ...
          for (var i = 0; i < game.players.length; i++) {
            const num2 = lib.configOL[`choice_${game.players[i].identity}`]
            var str = "选择角色"
            list.push([
              game.players[i],
              [
          ...
          for (var i in result) {
            if (result[i]?.links) {
              ...
              result[i] = (() =>
                ["wei", "shu", "wu", "qun", "jin", "key"].randomGet())()
            }
          }
      ...
      chooseCharacter: () => {
        ...
        next.ai = (player, list, list2, back) => {
          ...
          if (_status.event.zhongmode) {
            ...
            if (lib.characterReplace[choice]?.length) {
              choice = lib.characterReplace[choice].randomGet()
            }
            if (lib.characterReplace[choice2]?.length) {
        ...
        next.setContent(() => {
          ...
          if (
            ...
            var zhongs = game.filterPlayer(
              (current) => current.identity === "zhong",
            )
            var fans = game.filterPlayer(
          ...
          if (!event.chosen.length) {
            game.me
              ...
              if (_status.brawl?.doubleCharacter) {
                return 2
              }
      ...
      chooseCharacterOL: () => {
        ...
        next.setContent(() => {
          ...
          _status.characterlist = list4.slice(0)
          if (event.zhongmode) {
            _list = event.list.randomGets(8)
          } else {
            var getZhuList = (list2) => {
          ...
          next.set("ai", (_button) => Math.random())
          ;("step 1")
          if (!game.zhu.name) {
    ...
    get: {
      rawAttitude: (from, to) => {
        ...
        if (_status.mode === "stratagem") {
          ...
          if (
            from === to ||
            to.identityShown ||
            ((stratagem_expose?.includes(to) || zhibi?.includes(to)) &&
                ...
                if (zhibi) {
                  ...
                  for (var current of dead) {
                    if (
                      ...
                      if (from.storage.stratagem_expose?.includes(to)) {
                        return -7
                      }
                    }
                  }
        ...
        if (_status.mode === "purple") {
          ...
          if (
            ...
            game.hasPlayer((current) =>
              ["rZhong", "bZhong", "rNei", "bNei"].includes(current.identity),
            )
          ) {
            return 6.5
          }
      ...
      situation: (absolute) => {
        ...
        var result = parseInt(10 * Math.abs(zhuzhong / total), 10)
        if (zhuzhong < 0) {
          result = -result
        }
        if (!game.zhong) {
    ...
    skill: {
      ...
      yexinbilu: {
        enable: "phaseUse",
        filter: (_event, player) =>
          player.identity === "rYe" || player.identity === "bYe",
        charlotte: true,
        ruleSkill: true,
