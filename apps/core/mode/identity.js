    ...
    chooseCharacter() {
      ...
      const contents = [
        ...
        async (event, trigger, player) => {
          ...
          const addSetting = (dialog) => {
            ...
            for (
              ...
              if (
                !stratagemMode &&
                get.distance(game.zhu, game.me, "absolute") === seatNumber - 1
              ) {
                td.classList.add("bluebg")
              }
              td.addEventListener(
        ...
        async (event, trigger, player, result) => {
          ...
          for (const currentPlayer of game.players) {
            if (
              (event.stratagemMode || currentPlayer !== game.zhu) &&
          ...
          for (const id in result) {
            if (result[id]?.links) {
              ...
              result[id] = (() => {
                ...
                const groups = ["wei", "shu", "wu", "qun", "jin", "key"].filter(
                  (group) => lib.group.includes(group),
                )
                if (_status.mode === "zhong" || !game.zhu || !game.zhu.group) {
                  return groups.randomGet()
                }
    ...
    content: {
      stratagemInsight: [
        async (event, _trigger, player) => {
          ...
          const send = (clientTarget, clientInsightResult, id) => {
            ...
            const nonStratagemInsightFlashing = classList.contains(
      ...
      async stratagemCamouflage(event) {
        ...
        if (event.players.includes(me) && me.identity === "nei") {
          ...
          const dialog = ui.create.dialog(
            `${get.translation(targets)}是${rebel}<br>`,
            "forcebutton",
          )
          ...
          for (const victim of targets) {
            ...
            const nonCamouflageFlashing = classList.contains(
              "flash-animation-iteration-count-infinite",
            )
    ...
    sixiang_baihu: {
      ...
      ai: {
        ...
        result: {
          player(player, target) {
            return (
              -ui.selected.cards.reduce((p, c) => p + get.value(c, player), 0) /
              6
            )
          },
        },
