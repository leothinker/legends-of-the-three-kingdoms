  ...
  return {
            ...
            },
            {
              seperate(list) {
                ...
                for (var i = 0; i < lib.careerList.length; i++) {
                  ...
                  result[
                    "法术·" + get.translation(lib.careerList[i]) + "_link:" + lib.careerList[i]
                  ] = careerspell[lib.careerList[i]]
                  result[
                    "随从·" + get.translation(lib.careerList[i]) + "_link:" + lib.careerList[i]
                  ] = career[lib.careerList[i]]
                }
    ...
    card: {
      ...
      spell_conglinzhihun: {
        ...
        filterTarget(card, player, target) {
          return (
            player.side == target.side && target.isMin() && !target.hasSkill("druid_conglinzhihun")
          )
        },
        selectTarget: -1,
        content() {
    ...
    skill: {
      ...
      mage_minghuo: {
        ...
        filter(event, player) {
          return (
            event.source && event.source == player.getLeader() && event.parent.name == "_mage_skill"
          )
        },
        content() {
          trigger.num++
    ...
    translate: {
      ...
      spell_anyingxingtai_info:
        "你的职业技能改为造成1点伤害，若已进入暗影形态，则改为造成2点伤害。",
      ...
      rogue_shoudao: "授道",
      rogue_shoudao_info:
        "在己方主将的结束阶段，令一名随机友方随从增加1点体力和体力上限并摸一张牌。",
      rogue_lifa: "理发",
      ...
      hunter_dusha: "渡沙",
      hunter_dusha_info:
        "你出场时，令双方主将各随机使用一张牌库中的1费随从牌（不计入行动值消耗）。",
      hunter_chuanlin: "穿林",
