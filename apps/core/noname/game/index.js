...
import { _status, ai, get, lib, ui } from "noname"
    ...
    if (
      ...
      await Promise.all([
        game.$elementGoto(
          elementA,
          parentB,
          elementB.nextElementSibling || "last",
          duration,
          timefun,
        ),
        game.$elementGoto(
          elementB,
          parentA,
          elementA.nextElementSibling || "last",
          duration,
          timefun,
        ),
      ])
    ...
    function getCurrentPosition(element) {
      ...
      if (animation) {
        ...
        if (animation.actualVisual) {
          // 如果动画是使用了复制节点喵
          const _actualVisual = animation.actualVisual
...
${e instanceof Error ? e.stack : String(e)}`)
   ...
   * @param {*} [dev]
   */
  multiDownload(list, _onsuccess, _onerror, onfinish, _process, _dev) {
    if (lib.config.dev) {
      game.print(get.url())
    }
    const args = Array.from(arguments)
  ...
  videoContent = {
    ...
    $disableJudge: (player, _map) => {
      player.$disableJudge()
    },
    $enableJudge: (player, _map) => {
      player.$enableJudge()
    },
    jiuNode: (player, bool) => {
      //Powered by 升麻
    ...
    changeSkin: (player, map) => {
      ...
      if (goon) {
        lib.character[map.to] = get.convertedCharacter([
          "",
          "",
          0,
          [],
    ...
    mark: (player, content) => {
      if (player && content) {
        var _mark = player.mark(content.id, content)
      } else {
        console.log(player)
      }
    },
  ...
  prompt() {
    ...
    const _strnode = ui.create.div("", str || "", dialog)
  ...
  addCharacterPack(pack, packagename) {
    ...
    function processCharacter(content) {
      for (const name in content) {
        ...
        if (lib.config.forbidai_user?.includes(name)) {
          lib.config.forbidai.add(name)
        }
  ...
  addMode(name, info, info2) {
    ...
    lib.init[`setMode_${name}`] = async () => {
      await game.import("mode", (_lib, _game, _ui, _get, _ai, _status) => {
        info.name = name
        return info
      })
    }
  ...
  over(result, _bool) {
    ...
    if (game.online) {
      ...
      for (let i = 0; i < game.players.length; i++) {
        const hs = game.players[i].getCards("h")
        if (hs.length) {
          dialog.add(
            `<div class="text center">${get.translation(game.players[i])}</div>`,
          )
          dialog.addSmall(hs)
        }
      }
      ...
      for (let j = 0; j < game.dead.length; j++) {
        const hs = game.dead[j].getCards("h")
        if (hs.length) {
          dialog.add(
            `<div class="text center">${get.translation(game.dead[j])}</div>`,
          )
          dialog.addSmall(hs)
        }
      }
    ...
    for (let i = 0; i < game.dead.length; i++) {
      ...
      const hs = game.dead[i].getCards("h")
      if (hs.length) {
        dialog.add(
      ...
      (event) => {
        ...
        if (get.config("free_choose")) {
          const createCharacterDialog = () => {
            ...
            event.freechoosedialog.listen((_e) => {
              if (!event.replacing) {
  ...
  finishCard(cardId) {
    const mode = get.mode(),
      ...
      aiBasicValue = (card, player, _index, method) => {
        if (
  ...
  findSkill(
    event,
    includeCharlotteSkill = false,
    includeEquipSkill = false,
    includeGlobalSkill = false,
  ) {
    let skill = ""
    let count = 0
    let evt = event
  ...
  async addPlayerOL(target, character, character2, isNext, config = {}) {
    ...
    const addPlayer = async (
      ...
      animate ??= (player) => {
        ...
        return drop.finished.then((_result) => {
          const list = []
          //落地后开始震动
  ...
  async removePlayerOL(player, config = {}) {
    ...
    const removePlayer = async (player, config, configOL) => {
      ...
      //移除角色的动画，默认为变成碎片消逝
      animate ??= (player) => {
        const _rect = player.getBoundingClientRect()
        const shardCount = 30
        const _container = player.parentElement
        const list = []
        //生成碎片
