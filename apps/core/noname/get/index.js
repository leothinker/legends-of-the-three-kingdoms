...
import { _status, game, lib, rootURL, ui } from "noname"
...
export class Get {
  ...
  cardsetion(player, sourceEvent) {
    ...
    switch (name) {
      ...
      case "useSkill": {
        const _skill = _status.event.skill
        // /-?
  ...
  eventInfoOL(item, _level, noMore) {
  ...
  parsedResult(item) {
    ...
    if (typeof item === "string") {
      ...
      if (item.startsWith("_noname_vcard:")) {
        return get.infoVCardOL(item)
      }
      if (item.startsWith("_noname_player:")) {
        return get.infoPlayerOL(item)
      }
      if (item.startsWith("_noname_event:")) {
        return get.infoEventOL(item)
      }
      if (item === "_noname_infinity") {
        return Infinity
      }
      return item
    }
  ...
  itemtype(obj) {
    ...
    if (
      ...
      (get.is.object(name) && obj.name && obj.name in lib.card)
    ) {
      return "vcard"
    }
  ...
  *iterableChildNodes(_node) {
  ...
  cardPile(name, position, start = "top") {
    ...
    if (start === "bottom") {
      ...
      if (position === "field") {
        const curs = game.filterPlayer(() => true)
  ...
  nodeintro(node, simple, evt, uiintro) {
    ...
    var _i, translation, _intro, _str
    if (node._nointro) {
      return
    }
    let created = false
  ...
  linkintro(dialog, _content, _player) {
  ...
  equipResult(_player, target, name) {
    let card = name
    if (typeof name === "string") {
      card = { name }
    } else {
  ...
  order(item, player = get.player() || game.me) {
    const _cache = CacheContext.requireCacheContext()
