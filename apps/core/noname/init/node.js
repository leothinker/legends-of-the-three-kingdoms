...
export default function nodeReady({ lib, game, get, _status, ui }) {
  ...
  game.download = (url, folder, onsuccess, onerror, dev, onprogress) => {
    ...
    game.ensureDirectory(
      ...
      () => {
        ...
        if (!lib.node.http) {
          lib.node.http = require("node:http")
        }
        if (!lib.node.https) {
          lib.node.https = require("node:https")
        }
        var opts = require("node:url").parse(encodeURI(url))
        opts.headers = { "User-Agent": "AppleWebkit" }
        ;(url.startsWith("https") ? lib.node.https : lib.node.http).get(
          opts,
          (response) => {
  ...
  game.readFile = (filename, callback, onerror) => {
    lib.node.fs.readFile(`${__dirname}/${filename}`, (err, data) => {
      if (err) {
        onerror(err)
      } else {
        callback(data)
  ...
  game.readFileAsText = (filename, callback, onerror) => {
    lib.node.fs.readFile(`${__dirname}/${filename}`, "utf-8", (err, data) => {
      if (err) {
        onerror(err)
      } else {
        callback(data)
  ...
  game.removeFile = (filename, callback) => {
    lib.node.fs.unlink(`${__dirname}/${filename}`, callback || (() => {}))
  }
