...
export const Content = {
  ...
  async chooseNumbers(event, _trigger, _player) {
      ...
      result = await new Promise((resolve, _reject) => {
        ...
        for (let index = 0; index < event.list.length; index++) {
          ...
          select.onchange = function () {
            event.numbers[parseInt(this.id.slice(6), 10)] = parseInt(
              this.value,
              10,
            )
  ...
  //变更武将牌
  async changeCharacter(event, _trigger, player) {
    ...
    if (event.log === false) {
      next.$handle = (current, add, remove, _evt) => {
        if (add.length) {
  ...
  //变更技能
  async changeSkills(event, _trigger, player) {
    //获取玩家当前已有的技能
  ...
  //连接牌
  async connectCards(event, _trigger, player) {
  ...
  //重置连接牌
  async resetConnectedCards(event, _trigger, player) {
    const { source, cards, log } = event
  ...
  //执行延时锦囊牌效果
  //TODO: 修改此处的虚拟牌/实体牌判断
  async executeDelayCardEffect(event, _trigger, player) {
  ...
  //Gift
  //赠予
  async gift(event, _trigger, player) {
  ...
  //Recast
  //重铸
  async recast(event, _trigger, player) {
    const { cards } = event
  ...
  //装备栏相关
  async disableEquip(event, _trigger, player) {
    const cards = []
    event.cards = cards
    const slots = []
  ...
  async enableEquip(event, _trigger, player) {
    const { slots } = event
    if (!slots.length) {
      return
    }
  ...
  async expandEquip(event, _trigger, player) {
    const { slots } = event
  ...
  //选择顶装备要顶的牌
  async replaceEquip(event, _trigger, player) {
    const vcards = []
    vcards.push(
  ...
  //装备牌
  async equip(event, _trigger, player) {
    event.visible = true
    ...
    const handleEquip = async (card) => {
      ...
      if (event.log !== false) {
        ...
        if (isViewAsCard && cards.length) {
          game.log(
            player,
            `装备了<span class="yellowtext">${get.translation(card)}</span>（`,
            cards,
            "）",
          )
        } else {
    ...
    if (
      cardInfo.prepareEquip &&
      (!cardInfo.filterEquip || cardInfo.filterEquip(event.card, player))
    ) {
  ...
  //装备栏 END
  async changeGroup(event, _trigger, player) {
  ...
  async chooseToDebate(event, _trigger, player) {
    const { list } = event
      ...
      if (targets.length) {
        const next = player
          ...
          .set(
            ...
            event.aiCard ??
              ((target) => {
                const getAi = get.event().ai || ((_card) => Math.random())
                const hs = target
                  .getCards("h")
    ...
    for (const color of event.opinions) {
      ...
      if (pairs.length) {
        const _filteredPair = pairs.filter(
  ...
  async chooseCooperationFor(event, _trigger, player) {
    const { target } = event
  ...
  // 苏婆！
  async chooseToPlayBeatmap(event, _trigger, player) {
    if (game.online) {
      return
    }
    ...
    if (event.isMine()) {
      ...
      const songPlaybackDelay = delayExt(songDuration, {
      ...
      const chooseToPlayBeatmapAccuracies = (
        lib.config.choose_to_play_beatmap_accuracies || []
      ).concat(
        Array.from(
          {
  ...
  async chooseToMove(event, _trigger, _player) {
    if (event.chooseTime && _status.connectMode && !game.online) {
      event.time = lib.configOL.choose_timeout
      game.broadcastAll((time) => {
        lib.configOL.choose_timeout = time
    ...
    if (event.isMine()) {
      result = await new Promise((resolve) => {
         ...
         */
        var filterBatchMove = (buttonList, buttonsDiv, position) => {
          ...
          const addChildren =
            position === "first"
              ? (b) => buttonsDiv.insertBefore(b, buttonsDiv.firstChild)
              : (b) => buttonsDiv.appendChild(b)
          const addMovedList =
            position === "first"
              ? (l) => event.moved[buttonsDiv._link].unshift(l)
              : (l) => event.moved[buttonsDiv._link].push(l)
        ...
        var dragEnd = (e) => {
          ...
          const isDragging =
            ui.selected.guanxing_buttons.length === 1 &&
            document.contains(ui.selected.guanxing_buttons[0]?.copy)
          ...
          if (
            isDragging ||
            (!canMultiselect && ui.selected.guanxing_buttons.length === 1) ||
            spannedSingle
          ) {
            const curCard = ui.selected.guanxing_buttons[0]
            // 鼠标当前处于哪个元素上
            ...
            if (!card) {
              ...
              // 如果拖动到按钮上面，我们交换两个按钮喵
              const _buttons2 = curCard.parentElement
              const _pos1 = card.nextElementSibling || "last"
              const _pos2 = curCard.nextElementSibling || "last"
            ...
            const selected = filterBatchMove(
              ui.selected.guanxing_buttons,
              buttons,
  ...
  async showCharacter(event, _trigger, player) {
  ...
  async removeCharacter(event, _trigger, player) {
  ...
  async chooseToDuiben(event, _trigger, player) {
    const { target } = event
    ...
    target.$compare(
      game.createCard(targetResult, "", ""),
      player,
      game.createCard(playerResult, "", ""),
    )
  ...
  async chooseToPSS(event, _trigger, player) {
    const { target } = event
  ...
  async cardsDiscard(event, _trigger, player) {
    const { cards } = event
  ...
  async orderingDiscard(event, _trigger, _player) {
  ...
  async cardsGotoOrdering(event, _trigger, player) {
    const { cards } = event
    game.getGlobalHistory().cardMove.push(event)
    let withPile = false
    for (const card of cards) {
  ...
  async cardsGotoSpecial(event, _trigger, player) {
    const { cards } = event
  ...
  async cardsGotoPile(event, _trigger, player) {
    const { cards } = event
    const waitings = []
    if (event.washCard) {
  ...
  async chooseToEnable(event, _trigger, player) {
    const { source } = event
    if (event.selectButton) {
      ...
      if (list.length === 1) {
        ...
        event.ai ??= (_event, _player, list) => list.randomGet()
  ...
  async chooseToDisable(event, _trigger, player) {
    const { source } = event
    if (event.selectButton) {
      /** @type {string[]} */
      const list = Array(5)
  ...
  async swapEquip(event, _trigger, player) {
    const { target } = event
  ...
  async disableJudge(_event, _trigger, player) {
    ...
    game.broadcastAll((player, _card) => {
      player.$disableJudge()
    }, player)
  },
  async enableJudge(_event, _trigger, player) {
  ...
  /*----分界线----*/
  async phasing(event, _trigger, player) {
  ...
  async toggleSubPlayer(event, _trigger, player) {
  ...
  async exitSubPlayer(event, _trigger, player) {
    if (player.storage.subplayer) {
      const current = player.storage.subplayer.name2
  ...
  async callSubPlayer(event, _trigger, player) {
  ...
  async addExtraTarget(event, _trigger, player) {
  ...
  async reverseOrder(event, _trigger, player) {
    const { card } = event
    await game.delay()
    ...
    const result = await player
      .chooseControl(
        "顺时针",
        "逆时针",
        (_event, _player) => _status.event.choice || "逆时针",
      )
  ...
  async gameDraw(event, _trigger, player) {
    const { num } = event
  ...
  async phaseLoop(event, _trigger, player) {
    let num = 1,
      current = player
  ...
  async loadPackage(event, _trigger, _player) {
        ...
        for (const itemName in charaPack) {
          ...
          for (const termName in item) {
            ...
            if (itemName === "character") {
              ...
              if (lib.config.forbidai_user?.includes(termName)) {
                lib.config.forbidai.add(termName)
              }
        ...
        for (const cardPackName in cards) {
          ...
          if (cards[cardPackName].card) {
            for (const cardName in cards[cardPackName].card) {
              if (
                !cards[cardPackName].card[cardName].hidden &&
                cards[cardPackName].translate[`${cardName}_info`]
              ) {
  ...
  async arrangeTrigger(event, trigger, _player) {
  ...
  async playVideoContent(event, _trigger, _player) {
  ...
  async waitForPlayer(event, _trigger, _player) {
    ui.auto.hide()
    ui.pause.hide()
  ...
  async replaceHandcards(event, _trigger, _player) {
  ...
  async replaceHandcardsOL(event, _trigger, _player) {
    const send = () => {
  ...
  async phaseZhunbei(event, _trigger, player) {
  ...
  phaseJudge: [
    ...
    async (event, _trigger, player) => {
      ...
      const cardName = event.card.name,
        ...
        _VJudge = event.card[event.card.cardSymbol]
  ...
  async phaseJieshu(event, _trigger, player) {
  ...
  chooseToUse: [
    async (event, _trigger, player) => {
      ...
      if (event.isMine()) {
        ...
        if (
          !ok ||
          !lib.config.auto_confirm ||
          cardinfo?.manualConfirm ||
          skillinfo?.manualConfirm
        ) {
          game.pause()
          ...
          next.set(
            ...
            (() => {
              if (info.chooseButton.complexSelect !== false) {
                if (
                  info.chooseButton.complexSelect === undefined &&
                  info.chooseButton.allowChooseAll === true
                ) {
      ...
      if (event.result?.bool && !game.online && !event.nouse) {
        if (event.result?.cancel) {
          ...
          if (!event.chooseonly) {
            ...
            const _next = player.useResult(event.result, event)
          } else {
  ...
  async gaincardMultiple(event, _trigger, player) {
    const { cards } = event
    event.type = "gain"
  ...
  async discardMultiple(event, _trigger, _player) {
    event.type = "discard"
    event.visible = true
    if (!event.position) {
    ...
    async (event, _trigger, _player) => {
      ...
      for (var j in event.gaining_map) {
        ...
        if (event.animate === "draw") {
          ...
          var entries = Object.entries(evtmap).map((entry) => [
            entry[0],
            entry[1][0],
          ])
          var getOwner = (card) => {
    ...
    async (event, _trigger, player) => {
      ...
      if (
        ...
        player
          ...
          .set("type", "compare")
          .set("ai", event.ai)
          .set("source", player).aiCard = (target) => {
    ...
    async (event, _trigger, player) => {
      ...
      if (event.isDelay) {
        ...
        player
          ...
          .step(async (_event, _trigger, _player) => {
  ...
  async chooseSkill(event, _trigger, _player) {
    let list
  ...
  async discoverCard(event, _trigger, player) {
    const { num = 3 } = event
  ...
  chooseCardOL: [
    ...
    async (_event, _trigger, _player, result) => {
      game.me.unwait(result)
    },
    async (event, _trigger, _player) => {
      if (!event.resultOL) {
  ...
  async chooseAnyOL(event, _trigger, _player) {
    ...
    if (humans.length > 0) {
      const solve = (resolve, _reject) => (result, player) => {
        if (result) {
  ...
  chooseButtonTarget: [
    ...
    async (event, _trigger, _player, _result) => {
      //处理ai的选择结果
      if (event.result !== "ai") {
        return
      }
  ...
  async chooseBool(event, _trigger, player) {
    if (event.isMine()) {
      if (
        event.frequentSkill &&
  ...
  async chooseDrawRecover(event, _trigger, player) {
   ...
   * @deprecated
   */
  async showHandcards(event, _trigger, player) {
  ...
  showCards: async function (event, _trigger, player) {
  ...
  async viewCards(event, _trigger, player) {
  ...
  async moveCard(event, _trigger, player) {
    if (
      !player.canMoveCard(
        null,
        event.nojudge,
    ...
    const next = player.chooseTarget(2, (_card, _player, target) => {
        ...
        for (const id in event.lose_map) {
          ...
          if (owner === player) {
            if (!throw_cards.length && lib.config.card_animation_info) {
              ...
              const _info = lib.card[card.name],
                number = card.number
              virtualCard.init([
                get.suit(card),
      ...
      if (event.cards.length) {
        ...
        if (ownerCards.length) {
          ...
          if (
            ...
            await game
              .loseAsync({ player: player, cards: ownerCards })
              .setContent(async (event, _trigger, _player) => {
                const cards = event.cards
                const cards_noowner = []
                while (cards.length) {
  ...
  async draw(event, _trigger, player) {
    // if(lib.config.background_audio){
  ...
  async discard(event, _trigger, player) {
    const { cards } = event
  ...
  async loseToDiscardpile(event, _trigger, player) {
    const { cards } = event
    if (event.log !== false) {
  ...
  async swapHandcards(event, _trigger, player) {
    const { target } = event
  ...
  async swapHandcardsx(event, _trigger, player) {
    const { target } = event
  ...
  async gainMultiple(event, _trigger, player) {
    const { targets } = event
    let delayed = false
    const cards = []
      ...
      game.broadcast(
        (player, cards, num) => {
          for (var i = 0; i < cards.length; i++) {
            ...
            //仅移除非永久标记
            const tags = cards[i].gaintag.filter(
              (tag) => !tag.startsWith("eternal_"),
            )
  ...
  async recover(event, _trigger, player) {
    let { num } = event
  ...
  async loseHp(event, _trigger, player) {
    const { num } = event
    if (event.num <= 0) {
      event._triggered = null
      return
  ...
  async doubleDraw(_event, _trigger, player) {
    const result = await player
      .chooseBool("你的主副将体力上限之和是奇数，是否摸一张牌？")
      .forResult()
    if (result.bool) {
      await player.draw()
    }
  ...
  //进入休整状态
  async rest(event, _trigger, player) {
  ...
  //结束休整状态
  async restEnd(event, _trigger, player) {
  ...
  //复活事件
  async revive(event, _trigger, player) {
    const { hp, log } = event
    if (log !== false) {
      game.log(player, "复活")
    }
  ...
  //一次加一张够用了
  async addJudge(event, _trigger, player) {
    let card, cardName
    ...
    game.broadcastAll(
      (player, _card, cards) => {
        const cardsCloned = cards.filter((_card) => {
          return (
            cards[0].clone &&
    ...
    if (isViewAsCard && cards?.length) {
      if (cardInfo.blankCard) {
        ...
        game.log(
          player,
          `被贴上了<span class="yellowtext">${get.translation(cardName)}</span>（`,
          cards,
          "）",
        )
      }
  ...
  async chooseToGuanxing(event, _trigger, player) {
  ...
  async chooseToMove_new(event, _trigger, player) {
    //联机时间
    if (player.isUnderControl()) {
      game.swapPlayerAuto(player)
    }
    ...
    if (event.isMine()) {
      ...
      //点击每个卡的事件
      function clickItem(card, _container, _allContainers) {
      ...
      //点击容器的事件回调
      function clickItemContainer(itemContainer, _i, _allContainers) {
