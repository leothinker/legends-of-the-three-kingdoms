...
export const defaultHooks: {
  ...
  checkCard: [
    function updateTempname(card, _event) {
  ...
  checkTarget: [
    function updateInstance(target, _event) {
      // @ts-expect-error ignore
      if (!target.instance) {
        return
      }
  ...
  checkEnd: [
    ...
    function createChooseAll(event, _) {
      ...
      if (
        ...
        event.isMine() &&
        !(event.cardChooseAll instanceof lib.element.Control)
      ) {
        // 判断技能是否可以使用全选按钮喵
  ...
  uncheckCard: [
    function removeTempname(card, _event) {
      // @ts-expect-error ignore
      if (!card._tempName) {
        return
      }
  ...
  uncheckTarget: [
    function removeInstance(target, _event) {
      // @ts-expect-error ignore
      if (!target.instance) {
        return
      }
    ...
    function removeTargetPrompt(target, _event) {
      if (target.node.prompt2) {
  ...
  checkOverflow: [
    function updateDialog(_itemOption, itemContainer, addedItems, _game) {
      //计算压缩折叠的量
      const gap = 3
  ...
  checkDamage1: [
    ...
    function jyliezhou(event, _player) {
  ...
  removeSkillCheck: [
    function checkCharge(_skill, player) {
