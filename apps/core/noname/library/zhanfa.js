...
const _zhanfa = {
  ...
  zf_tianrenheyi: {
    ...
    skill: {
      ...
      filter(event, player) {
        return (
          player.hasCard((card) => get.suit(card) == get.suit(event.card), "j") &&
  ...
  zf_quanli: {
    ...
    skill: {
      ...
      filter(event, player) {
        ...
        const index = player
          .getHistory("useCard", (evt) => get.type(evt.card) == "trick")
          .indexOf(event)
  ...
  zf_banyun: {
    ...
    skill: {
      ...
      async content(event, trigger, player) {
        const target = player
          .getEnemies((target) => target.countGainableCards(player, "h"), false)
          .randomGet()
      ...
      filter(event, player) {
        if (event.name == "wuxie") {
          ...
          return (
            ...
            trigger.cancelled &&
            trigger.card[trigger.card.cardSymbol].cards.someInD()
          )
        }
  ...
  zf_cuixue: {
    ...
    skill: {
      ...
      filter(event, player) {
        return (
          player.getRoundHistory("sourceDamage", (evt) => evt.card?.name == "sha").indexOf(event) ==
          0
        )
      },
    },
      ...
      subSkill: {
        global: {
          ...
          mod: {
            cardUsable(card, player, num) {
              if (card.name == "sha") {
                const count = player.getEnemies(
                  (target) => target.hasSkill("zf_danliangboduo", null, null, false),
                  false,
                ).length
                if (count) {
    ...
    skill: {
      ...
      subSkill: {
        global: {
          ...
          async content(event, trigger, player) {
            const num = player.getEnemies(
              (target) => target.hasSkill("zf_duanliangcao2", null, null, false),
              false,
            ).length
  ...
  zf_fanci: {
    ...
    skill: {
      ...
      filter(event, player) {
        return (
          player.getHistory("damage", (evt) => evt.num > 0).indexOf(event) == 0 &&
          player.getEnemies(() => true, false).length
        )
      },
      select: "all",
    },
      ...
      filter(event, player, name) {
        if (name == "damageBegin1") {
          return (
            ...
            game
              .getGlobalHistory("everything", (evt) => evt.name == "damage" && evt.source == player)
              .indexOf(event) == 0
          )
        } else {
  ...
  zf_houshi: {
    ...
    skill: {
      ...
      filter(event, player, name) {
        const index = game
          .getRoundHistory("everything", (evt) => evt.name == "damage" && evt.player == player)
          .indexOf(event)
        return index == 0
      },
      async content(event, trigger, player) {
    ...
    skill: {
      ...
      async content(event, trigger, player) {
        const num = player
          .getHistory(
            "gain",
            (evt) => evt.getParent()?.name == "draw" && evt.getParent("phaseDraw") == trigger,
          )
  ...
  zf_leihuoshi: {
    ...
    skill: {
      ...
      filter(event, player) {
        return (
          player
            .getHistory("useCard", (evt) => evt.card.name == "sha" && !!evt.card.nature)
            .indexOf(event) == 0
        )
      },
      usable: 1,
  ...
  zf_miaoji: {
    ...
    skill: {
      ...
      filter(event, player) {
        return (
          player.getHistory("useCard", (evt) => get.type2(evt.card) == "trick").indexOf(event) ==
            0 && get.is.damageCard(event.card)
        )
      },
    },
  },
  ...
  zf_miaoji2: {
    ...
    skill: {
      ...
      filter(event, player) {
        const index = player
          .getHistory("useCard", (evt) => get.type2(evt.card) == "trick")
          .indexOf(event)
  ...
  zf_sanbanfu: {
    ...
    skill: {
      ...
      filter(event, player) {
        ...
        const index =
          player.getAllHistory("useCard", (evt) => evt.card.name == "sha").indexOf(event) + 1
    ...
    skill: {
      ...
      async cost(event, trigger, player) {
        ...
        event.result = await player
          ...
          .set("ai", (target) =>
            get.effect(target, get.event().getTrigger().card, get.player(), get.player()),
          )
          .forResult()
      },
      async content(event, trigger, player) {
