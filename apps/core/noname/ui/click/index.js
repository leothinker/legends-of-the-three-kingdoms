...
export class Click {
  ...
  chat() {
    ...
    var _node = uiintro.add(`<input type="text" value="${_status.chatValue}">`)
    ...
    const createEmotion = (name) => {
      ...
      for (const file of files) {
        const emotionButton = ui.create.div(
          ".card.fullskin",
          `<img src="${srcBase}${file}" width="50" height="50">`,
          function () {
            let player = game.me
            if (!player) {
              if (game.connectPlayers) {
    ...
    for (const folder in _status.emotion_cache) {
      const emotionPack = ui.create.div(
        ".card.fullskin",
        `<img src="${srcBase}${folder}/1.gif" width="50" height="50">`,
        function () {
  ...
  dieswap() {
    if (game.me.isDead()) {
      _status.clicked = true
      var _i, _translation, _intro, _str
      if (ui.intro) {
        ui.intro.close()
  ...
  checkroundtranslate(_translate) {
  ...
  checkdialogtranslate(_translate, dialog) {
  ...
  charactercard(name, sourcenode, noedit, resume, avatar, audioName) {
    ...
    if (lib.config.change_skin) {
      let node, avatars
      const _info = get.character(name),
  ...
  intro(e) {
    ...
    var clickintro = function (e) {
      ...
      const isPoptip =
        e.target?.matches("noname-poptip") ||
