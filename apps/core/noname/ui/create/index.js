...
export class Create {
  ...
  async editor(config = {}) {
    ...
    const _discardConfig = ui.create.div(
      ".editbutton",
      "取消",
      editorpage,
      () => {
    ...
    const _saveConfig = ui.create.div(".editbutton", "保存", editorpage, () => {
      callback(view)
    })
    window.saveNonameInput = () => callback(view)

    const _fontSize = ui.create.div(".editbutton", "字号", editorpage, () => {
      game.promises
        .prompt(`###请输入字号###${lib.config.codeMirror_fontSize || "16px"}`)
        .then((size) => {
          if (size === false) {
            return
          }
    ...
    const _searchPanel = ui.create.div(
      ".editbutton",
      "搜索",
      editorpage,
      () => {
    ...
    if (language === "javascript" || language === "typescript") {
      ...
      const _mixedHTML = LRLanguage.define({ parser: mixedHTMLParser })
  ...
  otherMenu = otherMenu
  statictable() {
    var str, row, col, position, position2, fixed, _style, _divposition
    for (var i = 0; i < arguments.length; i++) {
      if (typeof arguments[i] === "string") {
        str = arguments[i]
      } else if (typeof arguments[i] === "number") {
  ...
  characterDialog() {
    ...
    const _div = ui.create.div(".searcher.find")
  ...
  arena() {
    ...
    ui.deckMonitor = ui.create.system(
      ...
      async () => {
        ...
        let flag = true
        const titleContainer = ui.create.div(".title", container, (_e) => {
          if (flag) {
    ...
    var setTimerPosition = function (_e) {
      this.position++
      if (this.position > 4) {
        this.position = 1
      }
  ...
  buttonPresets = {
     ...
     */
    tdnodes: (item, _type, position, _noclick, node) => {
      node = ui.create.div(
     ...
     * @returns { import("../../library/index.js").Button }
     */
    blank: (item, _type, position, _noclick, node) => {
     ...
     * @returns { import("../../library/index.js").Button }
     */
    card: (item, _type, position, noclick, node) => {
     ...
     * @returns { import("../../library/index.js").Button }
     */
    vcard: (item, _type, position, noclick, node) => {
    ...
    character: (item, type, position, noclick, node) => {
      ...
      var func = (node, item) => {
        ...
        if (
          ...
          var _textnode = ui.create.div(".text", str, node.node.hp)
          if (infoitem[2] === 0) {
            node.node.hp.hide()
     ...
     * @returns { import("../../library/index.js").Button }
     */
    player: (item, _type, position, _noclick, node) => {
      if (node) {
        node.classList.add("button")
        node.classList.add("character")
        node.style.display = ""
     ...
     * @returns { import("../../library/index.js").Button }
     */
    skill: (item, _type, position, noclick, node) => {
      //搜索拥有这个技能的角色
      let characterName
      if (!Array.isArray(item)) {
        item = [item, null]
  ...
  buttons(list, type, position, noclick, _zoom) {
    var buttons = []
