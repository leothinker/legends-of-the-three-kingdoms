...
export const cardPackMenu = (connectMenu) => {
  /**
  ...
  var createModeConfig = (mode, position) => {
    ...
    node._initLink = () => {
      ...
      if (!mode.startsWith("mode_") || cardPack?.closeable) {
      ...
      if (
        (!mode.startsWith("mode_") || cardPack?.closeable) &&
        lib.cardPile[mode]
      ) {
        var cardpileNodes = []
        ...
        var _cfgnode = ui.create.div(
          page,
          ".config.pointerspan.cardpilecfg.toggle",
        )
        var _cfgaddcard = ui.create.node(
          "button",
          "",
          "添加卡牌",
          cfgnode,
  ...
  ;(() => {
    ...
    node.create = () => {
      ...
      exportCardPile.firstChild.lastChild.onclick = () => {
