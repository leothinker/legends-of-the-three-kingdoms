...
import { createConfig, menuxpages } from "../index.js"

export const extensionMenu = (connectMenu) => {
  if (connectMenu) {
    return
  }
  /**
  ...
  var updateNodes = () => {
    for (var i = 0; i < start.firstChild.childNodes.length; i++) {
      ...
      if (node.mode?.startsWith("extension_")) {
  ...
  var createModeConfig = (mode, position) => {
    ...
    const node = ui.create.div(
      ".menubutton.large",
      mode.startsWith("extension_")
        ? lib.translate[mode] || mode.slice(10)
  ...
  const extensionsInMenu = Object.keys(lib.extensionMenu)
  ...
  for (const i of extensionsInMenu) {
    if (
    ...
    node._initLink = () => {
      ...
      game.editExtension = (name) => {
        ...
        if (name && lib.extensionPack[name]) {
          authorExtLine.querySelector("input").value =
            lib.extensionPack[name].author || ""
          introExtLine.querySelector("input").value =
            lib.extensionPack[name].intro || ""
          diskExtLine.querySelector("input").value =
            lib.extensionPack[name].diskURL || ""
          forumExtLine.querySelector("input").value =
            lib.extensionPack[name].forumURL || ""
          versionExtLine.querySelector("input").value =
            lib.extensionPack[name].version || ""
        } else {
          authorExtLine.querySelector("input").value =
            get.connectNickname() || ""
          introExtLine.querySelector("input").value = ""
      ...
      var processExtension = (exportext) => {
        ...
        setTimeout(() => {
          ...
          for (var i in dash4.content) {
            try {
              if (
                ["arenaReady", "content", "prepare", "precontent"].includes(i)
              ) {
          ...
          str +=
            ...
            get.stringify({
              ...
              character: ((pack) => {
                ...
                for (const [_key, info] of Object.entries(character)) {
      ...
      buttonExport.onclick = () => {
        ...
        if (
          ...
          typeof window.noname_shijianInterfaces
            .shareExtensionWithPassWordAsync === "function" &&
          ...
          game.readFile(
            ...
            () => {
              ...
              if (pwd === "" || pwd === null) {
                ...
                window.noname_shijianInterfaces.shareExtensionWithPassWordAsync(
                  extName,
                  pwd,
                )
              }
      ...
      if (lib.device === "ios") {
        exportExtLine.innerHTML =
          '已保存。退出游戏并重新打开后生效<span class="closenode">×</span>'
        exportExtLine.querySelectorAll("span")[0].onclick = () => {
          exportExtLine.style.display = "none"
        }
      } else {
        exportExtLine.innerHTML =
        ...
        exportExtLine.querySelectorAll("span")[1].onclick = () => {
          exportExtLine.style.display = "none"
        }
      }
      ...
      shareExtLine.querySelectorAll("span")[0].onclick = () => {
        //这个链接404了
        //game.open('https://tieba.baidu.com/p/5439380222');
        //无名杀贴吧首页
        game.open(
          "https://tieba.baidu.com/f?ie=utf-8&kw=%E6%97%A0%E5%90%8D%E6%9D%80",
        )
      }
      shareExtLine.querySelectorAll("span")[1].onclick = () => {
        shareExtLine.style.display = "none"
      }
      ...
      var dash1 = (() => {
        ...
        var clickButton = async function () {
          ...
          newCharacter.querySelector(".new_hp").value = get.character(
            this.link,
            2,
          )
          ...
          if (Array.isArray(info[4])) {
            ...
            for (var i = 0; i < info[4].length; i++) {
              if (info[4][i].startsWith("des:")) {
                newCharacter.querySelector(".new_des").value =
                  info[4][i].slice(4)
              }
                ...
                await new Promise((resolve) => {
                  if (typeof game.readFile === "function") {
                    game.readFile(
                      ...
                      () => {
                        console.warn(
                          `未找到${info[4][i].slice(4).replace("ext:", "extension/")}阵亡配音`,
                        )
                        resolve()
                      },
                    )
            ...
            for (var i = 0; i < options.childNodes.length - 1; i++) {
              const option = options.childNodes[i].lastChild
              if (
                optionMap.has(option?.name) &&
                info[optionMap.get(option.name)]
              ) {
                option.checked = true
              } else if (option) {
                option.checked = false
            ...
            const description = info.trashBin?.find((item) =>
              item.startsWith("des:"),
            )
            if (description) {
              newCharacter.querySelector(".new_des").value =
        ...
        var grouplist = lib.group.map((_group, i) => [
          lib.group[i],
        ...
        var dieaudio = ui.create.div(".die_audio", newCharacter, {
          textAlign: "left",
        })
        var _dieaudiolabel = ui.create.node("label", "阵亡配音:", dieaudio)
        var dieaudioUpload = dieaudio.appendChild(
          document.createElement("input"),
        )
        dieaudioUpload.type = "file"
        dieaudioUpload.accept = "audio/*"
        dieaudioUpload.style.width = "calc(100% - 100px)"
        dieaudioUpload.onchange = () => {
          ...
          if (fileToLoad) {
            ...
            fileReader.onload = (fileLoadedEvent) => {
              ...
              dieaudio.blob = blob
              var _new_name = newCharacter.querySelector("input.new_name")
        ...
        selectname.onchange = function () {
          ...
          if (this.value === "current_extension") {
            ...
            for (const skill of skills) {
              if (lib.skill[skill]?.derivation) {
                const derivation = lib.skill[skill]?.derivation
                skills[Array.isArray(derivation) ? "addArray" : "add"](
                  derivation,
                )
              }
            }
        ...
        var editnode = ui.create.div(
          ...
          function () {
            ...
            const skills = Array.from(skillList.firstChild.childNodes).map(
              (item) => item.skill,
            )
            ...
            if (des) {
              ...
              if (bool) {
                ...
                oldCharacter.trashBin = oldCharacter.trashBin.filter(
                  (tag) => !tag.startsWith("des:"),
                )
            ...
            if (dieaudio.file && dieaudio.arrayBuffer) {
              var audioname =
                name + dieaudio.file.name.slice(dieaudio.file.name.indexOf("."))
            ...
            if (!page.content.pack.character[name]) {
              ...
              page.content.pack.character[name] = [
                sexes.value,
                groups.value,
                hp,
                skills,
      ...
      var dash2 = (() => {
        ...
        var createButton = (name, image, fullskin) => {
        ...
        page.reset = (name) => {
          ...
          if (lib.extensionPack[name]) {
            ...
            for (var i in page.content.pack.card) {
              var file
              var fullskin = !!page.content.pack.card[i].fullskin
              if (fullskin) {
                file = `${i}.png`
              } else {
                file = `${i}.jpg`
          ...
          function () {
            ...
            if (this.innerHTML === "创建卡牌") {
              createButton(
                name,
                fakeme.image64,
                fakeme.classList.contains("fullskin"),
              )
            } else if (currentButton) {
              if (fakeme.image64) {
                if (fakeme.classList.contains("fullskin")) {
        ...
        var cardpileadd = ui.create.div(
          ".config.toggle.cardpilecfg.cardpilecfgadd",
          editPile,
        )
      ...
      var dash3 = (() => {
        ...
        for (var i in lib.skill) {
          if (
            i !== "global" &&
            !get.is.empty(lib.skill[i]) &&
            !lib.skilllist.includes(i)
          ) {
            list3.push(i)
          }
        }
          ...
          code =
            ...
            (() => {
              const obj = Object.defineProperty(
                get.copy(lib.skill[skillopt.value]),
                "_priority",
                {
                  enumerable: false,
                  writable: true,
      ...
      var dash4 = (() => {
        ...
        var dashes = {}
        var createCode = (shortName, fullName, link, defaultCode) => {
          var dash = ui.create.div(".menubutton.large.dashboard")
          dashes[link] = dash
          ...
          dash.listen(() => {
            ui.create.editor({
              ...
              saveInput: (result) => {
                if (
                  ["arenaReady", "content", "prepare", "precontent"].includes(
                    link,
                  )
                ) {
  ...
  ;(() => {
    ...
    node._initLink = () => {
      ...
      ui.create.div(".config", "修改下载地址", page, () => {
        alert(
      ...
      var reloadnode = ui.create.div(
        ".config.toggle.pointerdiv",
        "重新启动",
        page,
        game.reload,
      )
      ...
      var clickExtension = function () {
        ...
        page.currenttimeout = setTimeout(() => {
      ...
      var downloadExtension = function (e) {
        ...
        var _url = `${extensionURL + this.info.name}.zip`
        var onprogress = (byte, total) => {
          if (total) {
            size = total
          }
          if (byte === -1) {
        ...
        for (var i = 0; i < files.length; i++) {
          files[i] = `extension/${this.info.name}/${files[i]}`
        }
        game.checkFileList(files, () => {
          files.unshift(`extension/${this.info.name}/extension.js`)
          for (var i = 0; i < files.length; i++) {
            files[i] =
              `${extensionURL + this.info.name}/${files[i].slice(10 + this.info.name.length + 1)}`
          }
          var n1 = 0,
            n2 = files.length
          game.multiDownload(
            ...
            () => {
              ...
              window.game = game
              lib.init.js(
                `${lib.assetURL}extension/${this.info.name}`,
                "extension",
                () => {
                  if (!lib.config.dev) {
                    delete window.game
                  ...
                  if (game.importedPack) {
                    ...
                    this.childNodes[0].innerHTML = "安装成功"
                    this.childNodes[1].innerHTML = "安装成功"
            ...
            (current) => `extension/${current.slice(extensionURL.length)}`,
          )
        })
      }
            ...
            if (game.download) {
              ...
              if (lib.config.extensions.includes(list[i].name)) {
                ...
                if (
                  ...
                  lib.config[`extension_${list[i].name}_version`] !==
                  list[i].version
                ) {
