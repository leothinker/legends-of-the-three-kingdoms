...
declare interface CardInfo {
   ...
   *
   * 逻辑：get.usableCards()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 逻辑：get.types()（apps/core/wtk/get/index.js）
   * ```js
   ...
   * （apps/core/wtk/library/element/content.ts）
   * ```js
   ...
   *
   * 逻辑：get.subtypes()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 消费逻辑：player.js 的 inRangeOf（攻击范围判断的核心，apps/core/wtk/library/element/player.js）
   * ```js
   ...
   * 在取装备技能时统一收集。
   *
   * 逻辑：get.skillsFromEquips()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 逻辑：card.js 的 Card.$init()（apps/core/wtk/library/element/card.js）
   * ```js
   ...
   *
   * 逻辑：content.ts 的 damage 结算中（apps/core/wtk/library/element/content.ts）
   * ```js
   ...
   * ```
   * card.js 的 Card.getSource()（apps/core/wtk/library/element/card.js）
   * ```js
   * getSource(name) {
   ...
   *
   * 逻辑：player.js 的 Player.canSaveCard()（apps/core/wtk/library/element/player.js）
   * ```js
   ...
   *
   * 逻辑：library/index.js 的 lib.filter.cardRecastable()（apps/core/wtk/library/index.js）
   * ```js
   * const info = get.info(card);
   ...
   * 为函数时按（牌, 使用者, 事件）动态判定。
   *
   * 逻辑：lib.filter.cardEnabled()（apps/core/wtk/library/index.js）
   * ```js
   ...
   * return false;
   * ```
   * game.finishCard() 自动补全（apps/core/wtk/game/index.js）
   * ```js
   * if (card.type == "equip") {
   ...
   *
   * 逻辑：lib.filter.cardUsable()（apps/core/wtk/library/index.js）
   * ```js
   * let num = info.usable;
   ...
   * 在阶段结束时会把已用次数清零。
   *
   * 逻辑：lib.filter.cardUsable2()（apps/core/wtk/library/index.js）
   * ```js
   ...
   *
   * 逻辑：player.js 的 Player.canUse()（apps/core/wtk/library/element/player.js）
   * ```js
   ...
   *
   * 由 lib.filter.targetInRange 消费（apps/core/wtk/library/index.js），决定能否对 target 使用此牌。
  ...
  range?:
    | ((
        card: Card | VCard | CardBaseUIData,
        player: Player,
        target: Player,
      ) => any)
    | {
   ...
   *
   * 逻辑：game.finishCard()（apps/core/wtk/game/index.js）
   * ```js
   * if (card.type == "equip") {
   ...
   * 不选择目标就可以发动效果。
   *
   * 逻辑：lib.element.content 的 useCard 结算中（apps/core/wtk/library/element/content.ts）
   * ```js
   ...
   * 目标合法性检查（决定该目标是否可选）
   *
   * 由 lib.filter.targetEnabled 消费（apps/core/wtk/library/index.js）：
   * - `true`：任意目标均可；
   * - `false`：任意目标均不可；
   ...
   */
  filterTarget?:
    | ((card: Card, player: Player, target: Player) => number | boolean)
    | boolean
    | null
  /**
   ...
   *
   *
   * 由 lib.filter.targetEnabled2 / targetEnabled3 消费（apps/core/wtk/library/index.js）：
   * - `true`：任意目标均可指定；
   * - `false`：不提供额外放宽；
   ...
   *
   * 消费逻辑（targetEnabledx，apps/core/wtk/library/index.js）：
   * ```js
   ...
   *
   * 由 get.select()（apps/core/wtk/get/index.js）归一化：
   * ```js
  ...
  selectTarget?:
    | number
    | Select
    | ((card: Card, player: Player) => number | Select)
  /**
   * 改变目标（在正式结算前动态修改目标数组）
   *
   ...
   *
   * 消费逻辑：content.ts 的 useCard 结算（apps/core/wtk/library/element/content.ts）
   * ```js
   ...
   *
   * 消费逻辑：card.js 的 Card.$init（apps/core/wtk/library/element/card.js）
   * ```js
  ...
  contentBefore?:
    | ContentFuncByAll
    | OldContentFuncByAll
    | (ContentFuncByAll | OldContentFuncByAll)[]
  /**
   * 卡牌使用效果（使用卡牌后执行的内容）
   *
   ...
   *
   * 消费逻辑：content.ts 的 useCard（apps/core/wtk/library/element/content.ts）
   * ```js
   * // 初始化（默认 1）
   ...
   * 由 get.autoViewAs()（apps/core/wtk/get/index.js）消费。
   *
   ...
   * 或 null 表示无默认应变。
   *
   * 消费逻辑一：get.defaultYingbianEffect()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 消费逻辑：card.js 的 selfDestroy（apps/core/wtk/library/element/card.js）
   * ```js
   * selfDestroy(event) {
   ...
   *
   * 注意：核心引擎 `apps/core/wtk` 与 `apps/core/extension` 中均未检索到该字段的消费点，
   ...
   * 实际上并没有卡牌使用此字段。
   *
   * 引擎在 `get.is.attackingMount` / `get.is.defendingMount` 中消费（apps/core/wtk/get/is.js）：
   * ```js
   ...
   *
   * 消费逻辑：content.ts 的 equip 流程（apps/core/wtk/library/element/content.ts）
   * ```js
   * // 穿戴后
   ...
   * 入口：player.lose(params)（apps/core/wtk/library/element/player.js）→ next.setContent("lose")；
   * 事件内容：lib.element.content.lose（apps/core/wtk/library/element/content.ts 的 Content.lose）；
   ...
   * @since 1.11.5
   */
  filterLose?: (VEquip: Card, player: Player) => boolean | undefined
  /**
   ...
   *
   * 逻辑：player.js 的 removeEquipTrigger()（apps/core/wtk/library/element/player.js）
   * ```js
   ...
   *
   * 消费逻辑：content.ts 的 replaceEquip（apps/core/wtk/library/element/content.ts）
   * ```js
   * const info = get.info(card, false);
   ...
   *
   * 消费逻辑：get.bingzhu()（apps/core/wtk/get/index.js）
   * ```js
   * bingzhu(name) {
   * 	const list = [];
   ...
   * 为 true 时允许同名多张并存。
   *
   * 消费逻辑：player.js 的 canAddJudge()（apps/core/wtk/library/element/player.js）
   * ```js
   ...
   *
   * 消费逻辑：get.judgeSlots()（apps/core/wtk/get/index.js）
   * ```js
   ...
   * 未配置时为 undefined。
   *
   * 消费逻辑：get.judge2()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 消费逻辑：content.ts 的 phaseJudge（apps/core/wtk/library/element/content.ts）
   * ```js
  ...
  cancel?:
    | boolean
    | ((event: GameEvent, trigger: GameEvent, player: Player) => any)
   ...
   *
   * 消费逻辑：player.js 的 useCard（apps/core/wtk/library/element/player.js）
   * ```js
   * if (
   ...
   *
   * 消费逻辑一（player.js 的 useCard，apps/core/wtk/library/element/player.js）：
   * ```js
   * var postAi = get.info(next.card).postAi;
   ...
   *
   * 消费逻辑：game/index.js 的 useAudio（apps/core/wtk/game/index.js）
   * ```js
   ...
   *
   * 消费逻辑一（card.js 的 Card.$init，见 apps/core/wtk/library/element/card.js）：
   * ```js
   * let img = get.dynamicVariable(lib.card[bg].image, this);
   ...
   *
   * 逻辑：card.js 的 Card.$init() 渲染卡图信息时（apps/core/wtk/library/element/card.js）
   * ```js
   * if (info.addinfo) {
   ...
   *
   * 逻辑：wtk/init/index.ts 加载卡包时过滤，以及 wtk/get/index.ts 卡牌描述（uiintro）
   * ```js
   * // init/index.ts
   ...
   * --- 生效路径 1：阻止其他玩家的知牌权 ---✔
   *
   * 进入判定区时（content.ts 的 addJudge，apps/core/wtk/library/element/content.ts）：
   * ```js
   ...
   *
   * 逻辑：wtk\library\element\content.ts useCard()
   * ```js
   ...
   *
   * 逻辑：content.ts 的 equip 替换流程中（apps/core/wtk/library/element/content.ts）
   * ```js
   ...
   *
   * 消费逻辑：content.ts 的失去装备流程（apps/core/wtk/library/element/content.ts）
   * ```js
   * if (info.loseDelay != false && (player.isAlive() || info.forceDie)) {
   ...
   *
   * 逻辑：lib.element.content 的多目标结算中（apps/core/wtk/library/element/content.ts）
   * ```js
...
declare interface CardAI {
   ...
   *
   * 逻辑：get.order()（apps/core/wtk/get/index.js）
   * ```js
   * var num = order;
   ...
   * ```
   * 注意：濒死求桃的 ai1（apps/core/wtk/library/element/content.ts）以 `(null, player)` 调用，
   ...
   *
   * 逻辑：get.value()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 逻辑：get.useful_raw()（apps/core/wtk/get/index.js）
   * ```js
   ...
   *
   * 逻辑：get.equipValue()（apps/core/wtk/get/index.js）
   * ```js
  ...
  equipValue?:
    | number
    | ((card: Card, player: Player, arg3?: null, method?: string) => number)
  ...
  result?: {
     ...
     *
     * 逻辑：get.effect_use()（apps/core/wtk/get/index.js）
     * ```js
     ...
     * ```
     * get.effect()（apps/core/wtk/get/index.js）同理，但直接取 `result.player`。
     *
     * @param player 使用者
     * @param target 目标
     * @param card 卡牌
    ...
    player?:
      ...
      | ((
          ...
          target: Player,
          card: Card,
          linking?: boolean | Record<string, any>,
        ) => number | undefined)
    /**
     * 主动使用牌时对发动者自身的收益
     *
     * 仅 get.effect_use()（apps/core/wtk/get/index.js）使用，优先于 player；
     ...
     * ```
     * get.effect()（apps/core/wtk/get/index.js）同理，但直接取 `result.target`。
     *
     * @param player 使用者
     * @param target 目标
     * @param card 卡牌
     ...
     *
     * 仅 get.effect_use()（apps/core/wtk/get/index.js）使用，优先于 target；
     ...
     *
     * 逻辑：get.effect() 与 get.effect_use()（apps/core/wtk/get/index.js）
     * ```js
   ...
   *
   * 逻辑：get.effect_use() 与 get.effect()（apps/core/wtk/get/index.js）
   * ```js
   * const info = get.info(card);
   ...
   *
   * 逻辑：apps/core/wtk/library/index.js 的应变助战 ai
   * ```js
...
declare interface CardTag {
  ...
  // ===== 伤害与体力变化 =====
  /** 伤害 */
  damage?: number | ((card: Card, nature?: any) => number | undefined)
  /** 属性伤害 */
  natureDamage?: number | ((card: Card, nature?: any) => number | undefined)
  /** 火伤 */
  fireDamage?: number | ((card: Card, nature?: any) => number | undefined)
  /** 雷伤 */
  thunderDamage?: number | ((card: Card, nature?: any) => number | undefined)
  /** 冰伤 */
  iceDamage?: number | ((card: Card, nature?: any) => number | undefined)
  /** 毒伤 */
  poisonDamage?: number | ((card: Card, nature?: any) => number | undefined)
  /** 回复 */
  recover?: number
  /** 救人 */
  save?: number
