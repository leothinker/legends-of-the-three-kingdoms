...
declare interface Mod {
  ...
  cardEnabled?(
    ...
    player: Player,
    event: GameEvent | "forceEnable" | undefined,
    result: boolean | "unchanged",
  ): boolean | "unchanged" | undefined
  /**
  ...
  targetInRange?(
    ...
    player: Player,
    target: Player,
    result: boolean | "unchanged" | number,
  ): boolean | number | "unchanged" | undefined
  /**
   * 弃牌阶段时，忽略弃置的手牌
   * @param card
   * @param player
...
type CardTagType =
  | number
  | ((arg0: Card, arg1: string) => boolean | number)
  | ((card: Card) => boolean | number)
