...
type Row_Item_Option<T = Row_Item> = {
  ...
  clickItem?: (
    ...
    AllItemConatainers: HTMLDivElement[],
    e: MouseEvent,
  ) => void
  /**
   * 点击框的回调事件
...
declare type ContentFuncsByAll = ((
  event: GameEvent,
  trigger: GameEvent,
  player: Player,
...
type EventWithTrigger =
  | `${_AllCardName}`
  | `${_AllCardName}Cancel`
  | `${_AllCardName}ContentAfter`
  | `${_AllCardName}ContentBefore`
  | `[skillname]`
  | `[skillname]ContentAfter`
  | `[skillname]ContentBefore`
  | `[skillname]_cost`
  | `_save`
  | `addFellowAuto`
  | `addJudge`
...
//所有的由event派生的额外的时机信号
type Signal_Event =
  | `${EventWithTrigger}Before`
  | `${EventWithTrigger}Begin`
  | `${EventWithTrigger}End`
  | `${EventWithTrigger}After`
  | `${EventWithTrigger}Skipped`
...
type CheckMod = <T extends string & keyof Mod>(
  ...args: [...Parameters<Required<Mod>[T]>, name: T, skills: string[]]
