...
export class Check {
  processSelection({ type, items, event, useCache, isSelectable }) {
    ...
    if (event[`${type}Required`] && uiSelected.length === 0) {
      ...
      uiSelected.length < range[0] &&
      (!event.forced || selectableItems || event.complexSelect)
    ) {
      ok = false
    }
  ...
  button(event, useCache) {
    ...
    return game.Check.processSelection({
      type: "button",
      items: buttons,
      event,
      useCache,
  ...
  target(event, useCache) {
    ...
    const isSelectable = (target, event) => {
      if (
        game.chess &&
        !(event.chessForceAll || (card && get.info(card)?.chessForceAll)) &&
        player &&
        get.distance(player, target, "pure") > 7
      ) {
        return false
      }
      if (
        target.isOut() &&
        !event.includeOut &&
        !(event.skill && get.info(event.skill)?.includeOut) &&
        !(card && get.info(card)?.includeOut)
      ) {
        return false
      }
