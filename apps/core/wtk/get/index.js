...
export class Get {
  ...
  plainText(htmlContent) {
    if (htmlContent.includes("<") || htmlContent.includes(">")) {
      ...
      // 初始化poptip名称
      doc.querySelectorAll("wtk-poptip").forEach((poptip) => {
  ...
  itemtype(obj) {
    ...
    if (
      obj instanceof lib.element.VCard ||
      (get.is.object(obj) && obj.name && obj.name in lib.card)
    ) {
      return "vcard"
    }
  ...
  *iterableChildNodes(...elements) {
    ...
    for (const i in lib.character) {
      ...
      for (const skill of character.skills) {
        ...
        if (player?.hasSkill && info?.ai?.combo) {
          ...
          if (
            !skills.every((comboSkill) =>
              player.hasSkill(comboSkill, null, null, false),
            )
          ) {
            continue
          }
  ...
  static #cardProperFunctions = new Set([
    "name",
    "type",
    "subtype",
    "color",
    ...
    if (typeof node._customintro === "function") {
      ...
      if (
        ...
        if (node.extraEquip) {
          ...
          uiintro.add(
            `${get.translation(node.extraEquip[0])} ${get.translation(node.extraEquip[1])}`,
          )
        } else {
  ...
  selectGroup(name, type) {
    ...
    if (
      lib.selectGroup.includes(info.group) &&
    ...
    for (const skill of skills1) {
      ...
      if (
        ...
        temp1 = cache
          ...
          .player_use(card, player, target, result1, linking)
      } else if (
        temp1 &&
        typeof temp1.effect === "object" &&
        ...
        temp1 = cache
          .delegate(temp1.effect)
          .player(card, player, target, result1, linking)
      } else {
        temp1 = undefined
      }
      ...
      for (const skill of skills2) {
        ...
        if (temp2 && typeof temp2.effect === "function") {
          if (
            ...
            temp2 = cache
              .delegate(temp2)
              .effect(card, player, target, result2, linking)
          } else {
            temp2 = undefined
          }
        } else if (
          ...
          if (
            ...
            temp2 = cache
              ...
              .target_use(card, player, target, result2, linking)
          } else {
            temp2 = undefined
          }
        } else if (
          ...
          if (
            ...
            temp2 = cache
              .delegate(temp2.effect)
              .target(card, player, target, result2, linking)
          } else {
            temp2 = undefined
          }
        } else {
   ...
   * ---
   *
   * 在三国杀正式过渡到http协议前，三国杀的路径在不同端拥有不同的情况:
   * - 网页端: 除了`db`外，没任何可能
   ...
   *   - 在网页端和电脑端为空字符串
   *   - 在手机端为三国杀包的`externalApplicationStorageDirectory`里（也就是`Android/data/<app-id>/`）
   *
   * 现在三国杀即将踏入http协议，也早已用上了ES Module，故活用`import.meta.url`来提供路径理应被重视，`URL`也理应成为路径的主要构成
   *
   ...
   *
   * 在File协议下通过三国杀自带的文件处理函数读取内容，其他协议通过`fetch`读取内容
   *
   * @async
...
pinyinPro.addDict(WTKDictionary)
