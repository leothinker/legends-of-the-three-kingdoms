...
class BrowserAdapter {
  ...
  async open(path, options = {}) {
    ...
    if (
      (options.create || options.createNew || options.truncate) &&
      !writable
    ) {
      throw createFileSystemError(
        FileSystemErrorCode.IoError,
        path,
        "create, createNew and truncate require write access",
      )
    }
  ...
  async read(path) {
    ...
    throw createFileSystemError(
      FileSystemErrorCode.IoError,
      path,
      "Invalid readFile response data",
    )
  }
  ...
  async list(path) {
    ...
    if (!isStringArray(data?.folders) || !isStringArray(data?.files)) {
      throw createFileSystemError(
        FileSystemErrorCode.IoError,
        path,
        "Invalid getFileList response data",
      )
    }

    return [
      ...data.folders.map((name) => ({ name, type: "directory" })),
      ...data.files.map((name) => ({ name, type: "file" })),
    ]
  }
  ...
  async truncate(size = 0) {
    ...
    if (!Number.isSafeInteger(size) || size < 0) {
      throw createFileSystemError(
        ...
        this.path,
        "truncate size must be a non-negative safe integer",
      )
    }
...
async function requestBackend(route, path, query, init) {
  ...
  try {
    ...
    throw createFileSystemError(
      FileSystemErrorCode.IoError,
      path,
      `Backend returned invalid JSON (${detail})`,
    )
  }
  ...
  if (!response.ok || !result.success) {
    const message =
      result.errorMsg || `Backend request failed with HTTP ${response.status}`
...
function assertUint8Array(data, path) {
  if (!(data instanceof Uint8Array)) {
    throw createFileSystemError(
      FileSystemErrorCode.IoError,
      path,
      "write data must be a Uint8Array",
    )
  }
}
...
function toFileSystemError(error, path) {
  ...
  if (/\bENOENT\b|not found|不存在/i.test(message)) {
    ...
    /\b(?:EACCES|EPERM|EROFS)\b|permission denied|无权限|拒绝访问/i.test(
      message,
    )
  ) {
    code = FileSystemErrorCode.PermissionDenied
  } else if (
    /\b(?:EINVAL|ENAMETOOLONG)\b|invalid path|只能访问/i.test(message)
  ) {
