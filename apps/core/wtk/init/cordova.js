...
export default async function cordovaReady({ lib, game, get, _status, ui }) {
  ...
  const wtkInitialized = localStorage.getItem("wtk_inited")
  ...
  if (lib.device == "android") {
    ...
    // if (typeof window.WTKAndroidBridge == "undefined" || typeof window.WTKAndroidBridge.getPackageName != "function" || typeof window.WTKAndroidBridge.getPackageVersionCode != "function") {
    // 	throw new Error("您的安卓客户端版本过低，请升级至最新版");
    // }
    // const versionCode = window.WTKAndroidBridge.getPackageVersionCode();
    // switch (window.WTKAndroidBridge.getPackageName()) {
    // 	case "com.wtk.shijian":
    // 		if (versionCode < 16007) {
    // 			throw new Error("您的安卓诗笺版客户端版本过低，请升级至v1.6.7或以上");
    // 		}
    // 		break;
    // 	case "yuri.nakamura.wtk_android":
    // 		if (versionCode < 10904) {
    // 			throw new Error("您的安卓由理版客户端版本过低，请升级至v1.9.4或以上");
    // 		}
    // 		break;
    // 	case "yuri.nakamura.wtk":
    // 		if (versionCode < 108004) {
    // 			throw new Error("您的安卓兼容版客户端版本过低，请升级至v1.8.4或以上");
    // 		}
    // 		break;
    // 	case "com.widget.wtk.cola":
  ...
  game.download = function (url, folder, onsuccess, onerror, dev, onprogress) {
    ...
    game.ensureDirectory(
      folder,
      function () {
        // folder = wtkInitialized + folder;
        if (onprogress) {
          fileTransfer.onprogress = function (progressEvent) {
            onprogress(progressEvent.loaded, progressEvent.total)
          }
        }
        lib.config.brokenFile.add(wtkInitialized + folder)
        game.saveConfigValue("brokenFile")
        fileTransfer.download(
          encodeURI(url),
          encodeURI(wtkInitialized + folder),
          function () {
            lib.config.brokenFile.remove(wtkInitialized + folder)
            game.saveConfigValue("brokenFile")
            if (onsuccess) {
              onsuccess()
            }
  ...
  game.checkFile = function (fileName, callback, onerror) {
    let path = lib.path.join(wtkInitialized, fileName)
  ...
  game.checkDir = function (dir, callback, onerror) {
    let path = lib.path.join(wtkInitialized, dir)
  ...
  game.createDir = (directory, successCallback, errorCallback) => {
    ...
    new Promise((resolve, reject) =>
      window.resolveLocalFileSystemURL(wtkInitialized, resolve, reject),
    ).then(
      (directoryEntry) => {
        const redo = (entry) =>
          new Promise((resolve, reject) =>
