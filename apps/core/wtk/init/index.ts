...
export async function boot() {
  ...
  const pack = window.wtk_package
  delete window.wtk_package
  ...
  if (window.wtk_update) {
    lib.version = window.wtk_update.version
    // 更全面的更新内容
...
async function loadConfig() {
  ...
  let result
  // 懒人包配置
  const hasConfigTxt = (await game.promises.checkFile("wtk.config.txt")) === 1
