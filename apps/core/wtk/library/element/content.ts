      ...
      if (lose > 0) {
        ...
        const discardingCards = player.getCards("e", (card) => {
          ...
          if (slot === "equip3_4") {
            return get
              .subtypes(card)
              .some((subtype) => subtype === "equip3" || subtype === "equip4")
          }
  ...
  addToExpansionMultiple: [
    ...
    async (event, trigger, player) => {
      ...
      for (const [id, cards] of Object.entries(event.gaining_map)) {
        const player2 = (_status.connectMode ? lib.playerOL : game.playerMap)[
          id
        ]
                ...
                if (evtmap[id][1].length) {
                  ...
                  if (event.log) {
                    game.log(
                      player2,
                      "将",
                      get.cnNumber(evtmap[id][1].length),
                      "张牌置于了武将牌上",
                    )
                  }
                }
        ...
        const waiting =
          typeof animateTime === "number"
            ? delay(get.delayx(animateTime, animateTime))
            : animateTime
        ...
        if (event.sortcard) {
          ...
          if (event.dialog) {
            ...
            for (const [i, choice] of event.choiceList.entries()) {
              event.dialog.add(
                `<div class="popup text" style="width:calc(100% - 10px);display:inline-block">${event.displayIndex !== false ? `选项${get.cnNumber(i + 1, true)}：` : ""}${choice}</div>`,
              )
            }
        ...
        function getTriggerTarget(event: GameEvent, triggered: Player[]) {
          ...
          for (const target of sortedTargets) {
            remainingTargets.set(
              target,
              (remainingTargets.get(target) || 0) + 1,
            )
          }
