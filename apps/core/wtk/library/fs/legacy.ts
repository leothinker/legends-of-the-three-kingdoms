 ...
 */
export function installLegacyFileSystemAPI(
  ...
  game.readFile = function readFile(
    ...
    fileSystem
      .read(fileName)
      .then((data) => callback(toExactArrayBuffer(data)), onerror)
  }
  ...
  game.ensureDirectory = function ensureDirectory(
    ...
    fileSystem
      .createDir(pathArray.join("/"), { recursive: true })
...
function checkPathType(
  ...
  fileSystem.stat(path).then(
    (info) =>
      callback?.(info === null ? -1 : info.type === expectedType ? 1 : 0),
    (error) => handleLegacyError(error, onerror),
  )
}
