...
export class Library {
    ...
    general: {
      ...
      config: {
        ...
        compatible: {
          ...
          init: true,
          intro:
            "提供部分即将废弃api的实现，使用老扩展时建议开启。<br/>注：这些api将在下个版本移除，扩展作者请关闭此选项以进行适配。（重启后生效）",
          restart: true,
        },
        ignore_error: {
          name: "无视报错",
        ...
        dev: {
          ...
          onclick(bool) {
            ...
            if (bool) {
              window.wtk_shijianInterfaces?.showDebugButton?.()
              lib.cheat.i()
            } else {
              window.wtk_shijianInterfaces?.hideDebugButton?.()
              delete window.cheat
              delete window.game
              delete window.ui
              delete window.get
      ...
      config: {
        theme: {
          ...
          onclick: async (theme) => {
            ...
            lib.announce.publish("WTK.Apperaence.Theme.onChanging", theme)
            ...
            ui.arena.show()
            lib.announce.publish("WTK.Apperaence.Theme.onChangeFinished", theme)
          },
        },
        layout: {
          name: "布局",
  ...
  filter = {
    ...
    ignoredHandcard: (card, player) =>
      Boolean(game.checkMod(card, player, false, "ignoredHandcard", player)),
    /**
     * 检测此牌是否可赠予
     *
     * @param { Card } card - 需要判断的牌
    ...
    characterDisabled(name, _characters, ...options) {
      ...
      if (_status.connectMode) {
        if (
          lib.configOL.banned.includes(name) ||
    ...
    characterDisabled2(name, ...options) {
      ...
      if (character[4]) {
        ...
        if (
          !options.includes("ignoreForibidden") &&
          character.isAiForbidden &&
          !_status.event.isMine?.()
        ) {
          return true
        }
        if (
          lib.characterFilter[name] &&
          !lib.characterFilter[name](get.mode())
        ) {
          return true
        }
      }
    ...
    targetEnabledx(card, player, target) {
      ...
      if (event.addCount_extra) {
        const usableTarget = game.checkMod(
          card,
          player,
          target,
          false,
    ...
    targetEnabled(card, player, target) {
      ...
      if (!info.singleCard || ui.selected.targets.length === 0) {
        const playerEnabled = game.checkMod(
          card,
          player,
          target,
          "unchanged",
        ...
        const targetEnabled = game.checkMod(
          card,
          player,
          target,
          "unchanged",
    ...
    wuxieSwap(event) {
      ...
      return Boolean(
        infoMap.player?.isUnderControl(true) &&
          !infoMap.player.hasSkillTag("noautowuxie") &&
          (!infoMap.targets || infoMap.targets.length <= 1) &&
          !infoMap.noai,
      )
    },
  }
