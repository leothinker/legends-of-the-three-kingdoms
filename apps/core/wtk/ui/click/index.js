...
export class Click {
  ...
  count(e) {
    ...
    function getVisibleCards(target) {
      const allShown =
        target.isUnderControl(true, game.me) ||
        (!game.observe &&
          game.me &&
          game.me.hasSkillTag("viewHandcard", null, target, true))
      if (allShown) {
      ...
      return {
        cards: target.getCards("h", (card) => filterCard(card)),
        allShown,
      }
    }
    ...
    function addCardsToCountDialog(container, cards) {
      ...
      if (
        container.buttons?.length === cards.length &&
        (cards.length === 0 ||
          container.buttons?.every(
            (button, index) => button.link === cards[index],
          ))
      ) {
        return
      }
