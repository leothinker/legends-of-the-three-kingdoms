  ...
  ;(function () {
    ...
    node._initLink = function () {
      ...
      var processExtension = function (exportext) {
        ...
        const waitingImage = Promise.allSettled([
          ...(dash1.imagePromises || []),
          ...(dash2.imagePromises || []),
        ])
        ...
        Promise.allSettled([waitingImage, waitDelay]).then(() => {
          ...
          const extension = {
            "extension.js": `import { lib, game, ui, get, ai, _status } from "wtk";\nexport const type = "extension";\nexport default function(){\n\treturn ${str} \n};`,
      ...
      buttonExport.onclick = function () {
        ...
        if (
          ...
          window.wtk_shijianInterfaces &&
          typeof window.wtk_shijianInterfaces.shareExtensionWithPassWordAsync == "function" &&
          ...
          game.readFile(
            ...
            () => {
              ...
              let result
              if (pwd === "" || pwd === null) {
                window.wtk_shijianInterfaces.shareExtensionAsync(extName)
              } else {
                window.wtk_shijianInterfaces.shareExtensionWithPassWordAsync(extName, pwd)
              }
            },
            () => {
      ...
      var dash1 = (function () {
        ...
        if (!list.length) {
          if (!lib.character["wtk_sunce"]) {
            lib.character["wtk_sunce"] = new Character({
              sex: "male",
              group: "wu",
              hp: 4,
              skills: ["jiang"],
          ...
          if (!lib.translate["wtk_sunce"]) {
            lib.translate["wtk_sunce"] = "孙策"
          }
          list.push(["wtk_sunce", lib.translate["wtk_sunce"]])
        }
  ...
  ;(function () {
    ...
    node._initLink = function () {
      ...
      if (source && source[index]) {
        ...
        extensionURL = lib.updateURL.replace(/wtk/g, "wtk-extension") + "/master/"
      }
      ...
      var downloadExtension = function (e) {
        ...
        game.checkFileList(files, function () {
          ...
          for (var i = 0; i < files.length; i++) {
            files[i] =
              extensionURL + that.info.name + "/" + files[i].slice(10 + that.info.name.length + 1)
          }
          var n1 = 0,
            n2 = files.length
          game.multiDownload(
