    ...
    if (flags & 1) {
      ...
      if (defaultFunction.prototype) {
        ...
        parent[name] = NativeWrapper.wrapApply(
          global === parent ? defaultFunction.bind(null) : defaultFunction,
          flags,
        )
      }
    }
  ...
  static boxCallback(unboxed, prototype) {
    ...
    if (!wrapped) {
      ...
      wrapped = ContextInvokerCreator(
        ...
        function (
          /** @type {any} */ thiz,
          /** @type {readonly any[]} */ args,
          /** @type {(new (...args: any) => any) | undefined} */ newTarget,
        ) {
          return newTarget
            ? // @ts-expect-error Sandbox
...
class Sandbox {
  ...
  static #compileCore(
    ...
    const raw = new thiz.#domainFunction(
      "_",
      `with(_){with(window){with(${contextName}){return(${applyName}(function(${parameters}){"use strict";// 沙盒代码起始\n${code}\n// 沙盒代码结束\n},${contextName}.this,${argsName}))}}}`,
    )
  ...
  exec2(code, context = null, readonly = true) {
    ...
    const compiled = Sandbox.#compileCore(
      this,
      code,
      context,
      null,
