...
本文根据 `apps/core/wtk/library/element/gameEvent.ts` 及其配套的 `GameEventManager`、content 编译器整理，用于后续阅读事件系统时作为上下文。
...
`arrangeTrigger` 的 content 在 `apps/core/wtk/library/element/content.ts`。
