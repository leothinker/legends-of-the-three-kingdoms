...
本章节将简略介绍三国杀使用的更换原画/更换皮肤的方式

# 一、changeSkin

三国杀为十周年谋定天下、手杀曹髦、势太史慈等多原画武将使用的更换原画的方式
以势魏延`pot_weiyan`为例
...
启用后，三国杀会读取位于`与原画所在文件夹同级的skin文件夹下以该武将id命名的文件夹A下的图片`作为该武将的可用皮肤
...
lib.character["sunce"] = [
  ...
  4,
  ["jiang", "hunzi", "zhiba"],
  ["zhu", "skinPath:image/skin/shen_sunce/"],
]
```
