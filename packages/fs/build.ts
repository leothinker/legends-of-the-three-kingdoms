...
async function main(platform) {
  try {
    ...
    if (!existsSync(blobPath)) {
      console.warn(
        `Warning: ${blobPath} not found. Proceeding anyway (postject will fail if file missing).`,
      )
    }
    ...
    // macOS (darwin) and Linux share most steps; mac has extra codesign steps & macho segment arg
      ...
      try {
        ...
        console.warn(
          "Failed to remove signature (maybe not signed or not supported). Continuing.",
          (e as Error).message,
        )
      }
      ...
      try {
        ...
        console.warn(
          "codesign failed. You may need to run the re-sign step manually or provide proper identity.",
          (e as Error).message,
        )
      }
